...
### D-4 — TripMate OpenAPI client ✅ **결정: kor-travel-geo와 동일방식**
- **결정 (2026-06-01 사용자)**: kor-travel-geo가 소비자에게 쓰는 방식 그대로 — **이원**:
  - **TripMate frontend(Next.js/TS)**: kor-travel-map `openapi.user.json` → **`openapi-typescript`
    ...
    대기 없이 지속 동기.
  - **TripMate backend(Python `apps/api`)**: **수기 httpx wrapper**(kor-travel-map의
...
- **결정 (2026-06-01 사용자)**: RustFS를 **유지** — dev는 docker-compose 서비스로
  ...
  `kor_travel_map` + `kor_travel_map_dagster` + RustFS **3종 묶음**(서로 독립, TripMate와
