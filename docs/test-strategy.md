...
본 문서는 `python-krtour-map` v2의 테스트 정책이다. 사용자 요청은
...
## 5. e2e 테스트 (`packages/krtour-map-admin/tests/`)

e2e/라우터 테스트는 **별도 패키지** `krtour-map-admin`의 FastAPI app을 띄워
검증한다 (ADR-020). 메인 패키지 `tests/` 트리에는 e2e 디렉토리가 없고, 디버그 UI
테스트는 `packages/krtour-map-admin/tests/`에 둔다 (`test_routers.py` /
`test_features_router.py` / `test_etl_routers.py` 등 — `TestClient` 기반, 대부분
DB 없이 의존성 override). 실행 환경은 메인 + 디버그 UI 둘 다 설치된 venv
(`uv pip install -e . -e packages/krtour-map-admin`). 실 DB round-trip은 메인
...
async def debug_client(pg_engine, file_store):
    ...
    from krtour.map_admin.app import build_app
...
def test_warns_when_binding_to_all_interfaces(caplog):
    from krtour.map_admin.app import warn_if_external_bind
...
.github/workflows/test.yml
  jobs:
    unit:
      ...
      steps:
        - pip install -e ".[dev]"
        - pytest tests/unit -q --cov=src/krtour/map --cov-fail-under=80
        - ruff check .
        - mypy src/krtour/map
        - lint-imports
    integration:
      ...
      steps:
        ...
        - pytest tests/integration -q --cov=src/krtour/map --cov-append --cov-fail-under=80
    fixture_replay:
      runs-on: ubuntu-latest
      steps:
        - pip install -e ".[dev]"
