...
class _FakeClient:
    def __init__(self, engine: _FakeEngine, *, settings: KorTravelMapSettings) -> None:
        self.engine = engine
...
async def test_build_offline_upload_store_uses_offline_upload_bucket() -> None:
    settings = KorTravelMapSettings(
        ...
        offline_upload_bucket="kor-travel-map-uploads",
    )
    ...
    assert store.bucket == "kor-travel-map-uploads"
    assert body == b"kor-travel-map-uploads:offline-uploads/u1/features.jsonl"


async def test_kor_travel_map_client_resource_disposes_engine(
    ...
    resource_fn = cast(
        ...
        resources.kor_travel_map_client_resource.resource_fn,
    )
...
def test_reverse_geocoder_resource_builds_and_closes_client(
    ...
    monkeypatch.setenv(
        "KOR_TRAVEL_MAP_KOR_TRAVEL_GEO_ADMIN_PROXY_SECRET",
        "geo-proxy-secret",
    )
    ...
    def _fake_client(
        ...
        *,
        ...
        assert admin_proxy_secret is not None
        assert admin_proxy_secret.get_secret_value() == "geo-proxy-secret"
...
def test_mois_license_records_resource_syncs_source_db_before_fetch(
    ...
    def _fake_sync(settings: KorTravelMapSettings, **_kwargs: object) -> None:
    ...
    monkeypatch.setattr(resources, "ensure_mois_source_db_fresh", _fake_sync)
...
def test_provider_record_guard_message_hides_secret(
    ...
    monkeypatch.setenv("KOR_TRAVEL_MAP_DATA_GO_KR_SERVICE_KEY", "super-secret-value")
    spec = {
