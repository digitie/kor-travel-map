...
DROP PROCEDURE feature.approve_feature_request_with_initial_state(uuid, jsonb, bigint);
...
BEGIN
    ...
    had_create := has_schema_privilege('ktm_feature_request_procedure_owner', 'feature', 'CREATE');
    IF NOT had_create THEN
        EXECUTE 'GRANT CREATE ON SCHEMA feature TO ktm_feature_request_procedure_owner';
    END IF;
    EXECUTE 'ALTER PROCEDURE feature.approve_feature_request_with_initial_state(IN p_request_id uuid, IN p_feature_payload jsonb, IN p_domain_command_id bigint, OUT o_outcome text, OUT o_feature_id uuid, OUT o_row_revision bigint, OUT o_existing_feature_id uuid) OWNER TO ktm_feature_request_procedure_owner';
    IF NOT had_create THEN
        EXECUTE 'REVOKE CREATE ON SCHEMA feature FROM ktm_feature_request_procedure_owner';
    END IF;
END
$t39_owner$;

-- head-schema.sql:24809-24810을 새 시그니처로 복원한다.
