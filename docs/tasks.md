...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#210(T-RV-37e
Docker 이미지 multi-stage/non-root/standalone 위생)까지 merged. 이번 PR은
`T-209e-a`로 backup/restore 독립 DB 묶음 중 cold backup script와 runbook을 먼저 닫고,
restore/admin router/hot-swap은 후속 T-209e-b/c로 남긴다.
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 hardening
전까지 구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은 `T-RV-20` router
schema 검증, `T-RV-19` admin UI 지도 선행 안정화, 또는 `T-RV-22/23/25` offline
upload 후속이다.
...
- **T-RV-19~T-RV-25, T-RV-27, T-RV-29~T-RV-35** — poi-cache list
  cursor/무한정 JSON, scope/policy 검증, offline orphan/멱등/상태 drift,
  노출, user OpenAPI 누출, D-4 codegen 미채택, savepoint,
