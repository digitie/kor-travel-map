...
후속 PR에서 해소했고, concierge측 P-01(concierge #83)과 source_entity_id 불변성 계약
테스트(concierge #85)까지 머지돼 **모든 후속이 종결**됐다(잔여 0).
...
- **concierge (그쪽 repo — 모두 완료)**: ~~P-01(`limit`에 `Query(ge=1,le=500)`로 명시적
