...
## 4. category (`docs/architecture/category.md` §4)
...
- `pyproject.toml` `providers` extras에 git URL 핀 (PR#25에서 활성화 —
  `@06da125f`).
- 본 라이브러리에 `docs/etl/knps-feature-etl.md` 신설 — feature 변환 계약
  (knps-api 측 `docs/etl/knps-feature-etl.md`와 정합, 본 PR#12).
...
자세한 근거 / 후속 작업은 `docs/adr/README.md` ADR-027 참조.
...
- 선·면 geometry는 `feature.feature_routes.geom`/`feature_areas.geom`이 정본이고
  둘 다 4326이다 (ADR-086). point 좌표는 core `coord`이고 `coord_5179` generated
