...
메인 패키지의 의존 방향: **dto → core → infra → providers → client → cli**
한 방향. `import-linter`가 CI에서 강제한다. `krtour_map.api`는 존재하지
않는다 (ADR-020).

`krtour-map-debug-ui` 패키지는 `krtour_map.client`만 import해서 함수 호출한다.
메인 패키지의 `infra/`/`providers/`를 직접 부르지 않는다.
...
19. **TripMate 도메인 모델을 본 라이브러리에 정의 금지** — 사용자/여행계획/POI는
    TripMate.
