"""typed subtype 분해 ③ — route·area subtype + geometry 정본 이동 (T-VN-35C, ADR-086).
...
없다. 35C 원문의 "parent/sibling 관계"는 이 판단으로 종결한다(ADR-086 §결정).
...
def upgrade() -> None:
    ...
    #
    # subtype이 kind별 값의 **유일한 정본**이 된다(shadow 병행 폐기 — ADR-086
