...
> kor-travel-map 정본 반영: **ADR-050~052 + tasks T-217a~g + CLAUDE.md**. 본 문서의
> KR-01→T-217b, KR-02→완료, KR-03→ADR-050 #2, KR-04→T-217d, KR-06→T-217f,
> 검토 보고서 §3.2 A-2(D-07)→T-217g로 task화됨.
> **추가 결정 사항(사용자 보정)**: kor-travel-concierge export 경로는 downstream 이름 없이
...
> (기준 커밋: krtour `0e45bd7` / TripMate `4a10a5b` / kor-travel-concierge `a443ca0`).
> **TripMate 측 상세**: [`tripmate-side-actions-2026-06-10.md`](tripmate-side-actions-2026-06-10.md).
> **kor-travel-concierge 측 상세**: 해당 repo `docs/cross-repo-consistency-actions-2026-06-10.md` (직접 전달 완료).
...
의존 순서(§3)에 따라 착수 가능하다. kor-travel-map 측 액션은 tasks T-217a~g로 추적.

### 2.1 kor-travel-map (KR-)

| ID | 우선 | 액션 | 의존 |
|---|---|---|---|
| KR-01 | 🟡 | **tombstone/reject → feature inactive 경로 구현**: `kor_travel_concierge_items_to_bundles()`의 skip(`providers/kor_travel_concierge.py:87`)을 deactivate 연동으로 보강(MOIS Step C 동형). 1단계로 skip 건수 WARN/admin 이슈 노출도 가능 | D-03 |
| KR-02 | 🟡 | **신규 task 등록**: KR-01(라이프사이클), 제보 수신 API(D-02 승인 시), provider 신선도 대시보드(D-07 승인 시)를 `docs/tasks.md`에 T-2xx로 | D-02/03/07 |
| KR-03 | 🟡 | **kor-travel-concierge 계약 링크 정리**: D-04 확정 후 `docs/rest-api.md`(또는 external-apis 절)에서 kor-travel-concierge 정본 계약 문서로 링크 + 소비 측 기대(fetcher의 `{items,has_more,next_cursor}`/`X-API-Key`/env 키) 요약 | D-04 |
...
| KR-05 | ⚪ | **geocoding R&R 1줄 명시**: kor-travel-concierge 후보 지오코딩(제안) vs krtour kor-travel-geo 재검증(정본 확정)의 역할 차이를 architecture.md 또는 ADR-049 보강에 기록 | — |
...
| KR-08 | 🟡 | **T-066 완료 후 live fetch smoke**: Dagster `kor_travel_concierge_youtube_features` live 전환 — cursor 영속(`provider_sync_state`)·페이지네이션 가드·재시도 멱등성 검증. T-212e(실데이터 full reload) 항목에 합류 | TA-01 |
...
### 2.3 kor-travel-concierge (TA- — 상세는 해당 repo 문서)

| ID | 우선 | 액션 (요약) | 의존 |
|---|---|---|---|
| TA-01 | 🔴 | **T-066 export API 구현**: `GET /api/v1/features/{snapshot\|changes}` — krtour fetcher 기대치(`X-API-Key`, `limit`/`cursor`, `{items,has_more,next_cursor}`, cursor 안정성) 체크리스트 준수 | — |
...
M3 공급 사슬 완결 ───────────── TA-01(T-066) → KR-08(live smoke) · KR-01(D-03 승인 시)
   └ 게이트: krtour Dagster가 kor-travel-concierge 실데이터를 cursor 증분 포함 정상 적재
...
1. **연동 정본 1곳 + view 구조** (KR-04): 계약은 공급자 repo가 정본, 소비자 repo 문서는
   ...
   TripMate·kor-travel-concierge 측 문서도 동일 형식으로 통일.
