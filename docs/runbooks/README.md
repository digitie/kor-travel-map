...
`kor-travel-map`을 편집하는 **AI 에이전트 공용** runbook. Claude Code / OpenAI
...
| 파일 | 범위 |
|------|------|
| [agent-workflow.md](./agent-workflow.md) | 표준 1-PR 작업 흐름 (worktree → 브랜치 → Linux/WSL 편집·검증 → PR → CI green → 머지 → 동기화) + 갱신 필수 문서 |
...
| [admin-feature-live-acceptance.md](./admin-feature-live-acceptance.md) | #741/#785/T-VN-15 owned Feature production live lane, 핀된 SHA 체크아웃 + C7과 같은 runtime preflight(ADR-102), SIGKILL-safe supervisor·별도 recovery |
...
|-------------|---------------|----------------------|
| OpenAI Codex | `F:\dev\kor-travel-map-codex` | `sandbox/codex` |
| Claude Code | `F:\dev\kor-travel-map-claude` | `sandbox/claude` |
| Google Antigravity 2.0 | `F:\dev\kor-travel-map-antigravity` | `sandbox/antigravity` |

- **worktree는 영속**, 작업마다 그 안에서 **브랜치만 새로** 딴다
  (`git switch -c feat/<topic> main`). 메인 trunk(`F:\dev\kor-travel-map`)는
...
| 항목 | 정책 | 근거 |
|------|------|------|
| Git/codegraph 실행 | **Linux/WSL** (`/mnt/f/dev/kor-travel-map-*`). Windows `git.exe`/Node/npm은 일반 개발에 사용하지 않음. | AGENTS.md, dev-environment.md |
| 테스트 실행 | **Linux/WSL** 기본. 성능·격리 필요 시에만 WSL ext4로 rsync 후 PostGIS/testcontainers 구동. | dev-environment.md |
| `scripts/*.sh` 실행 | WSL Bash. PowerShell은 `.sh` 직접 실행 대신 WSL 위임. | dev-environment.md §2.3 |
...
| import 루트 | `from kortravelmap import ...` (flat 금지) | ADR-022 |
