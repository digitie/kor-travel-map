...
- [ ] T-VN-M05-ACTIVATION — baseline PostGIS digest를 Compose에 고정해 병합한 Map `e6c08e25…`는 PinVi `admin`·`full` pair에 재결박됐다. Manager #251의 trusted release 밖 root-owned runtime pin registry에서 읽기 전용 terminal seed `cbb577d3…`을 보존한 채 Map→PinVi 순서로 회전하고 `pin verify`가 통과한 final pinset `a3f6a8f3…`의 새 candidate에서만 n150 isolated M04/M05 live mutating E2E와 activation attestation을 실행한다.
