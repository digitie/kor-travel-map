...
- [ ] **다음**: mois source DB cursor iterator(`collect_and_load_mois_license_features_bulk`)
  — `run_mois_license_bulk_job`를 mois source DB(`iter_open_place_records`) 스트림에
  연결(현재는 in-memory snapshot iterable). 또는 CLI mutex(SPRINT-4 §2.8,
  `src/krtour/map/cli/`) 신설.
