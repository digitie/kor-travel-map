...
- [ ] T-VN-M05-ACTIVATION — Map `b8d108bd…`·PinVi `50c875f5…`·pinset `22563762…`은 registry/public-copy gate 뒤 n150 isolated M04/M05 launcher를 정확히 한 번 실행했고 고정 분류 `runtime_http_failed`로 terminal 차단됐다. cleanup은 통과했으며 raw HTTP·컨테이너·환경 출력은 읽지 않는다. `a3f6a8f3…`의 `launcher_preflight`와 이 candidate의 pinset·Manager source·output leaf는 재실행하지 않는다. Manager #253의 HTTP 호출 단계 고정 분류 보정, 이 기록의 새 Map revision과 PinVi 후속 provenance revision을 atomic `pin rotate-pair`로 새 pinset에 함께 결박한다. terminal current의 role별 회전은 거부되고 source pair preflight도 ledger claim보다 먼저 실행된다. 두 전문 적대 재리뷰와 최신 CI를 통과한 그 새 candidate에서만 n150 isolated M04/M05 live mutating E2E와 activation attestation을 정확히 한 번 실행한다.
