...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#216(T-201b-a F6 opening_hours
정합성 검사)까지 merged. 코드 리뷰 후속 흐름으로,
이번 PR은 `T-RV-19` POI/cache target cursor와 request schema 상한을 닫는다.
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 hardening
전까지 구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은
`T-201b-b` F5 provider SLA 정합성 또는 `T-RV-22/23/25` offline upload 후속이다.
