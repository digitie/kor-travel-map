...
- [x] CLI mutate 명령 ①  `krtour-map import mois <records-file>` — NDJSON snapshot
  ...
  776 passed.)
- [ ] **다음**: CLI mutate 명령 ② `krtour-map dedup-merge <feature_id>` — manual
  merge. **merge primitive 없음**: 생존 feature로 supersede + source_link 재지정 +
  `dedup_review_queue` 상태 갱신하는 infra 1차 함수부터 설계 필요(별도 PR).
- [ ] Step B incremental + dataset_key cursor (`ProviderSyncState`). 또는 실적재 후
  dedup false-positive 측정(가중치 조정 ADR-016).
