...
수정했다. 나머지(C-04~C-08, P-01)는 **오늘 활성 버그 0**의 latent 하드닝으로 후속 분리.
...
| C-04 | MED | inactivate 매칭 키(고정 상수, exact SQL) ↔ upsert 저장 키(payload+normalize) — 오늘 일치, 향후 alias 시 silent miss | ⏭ 후속(map) |
| C-05 | MED | operation open-world 분류(`!=upsert`→전부 inactivate) — 새 operation 추가 시 live feature 파괴적 비활성화 | ⏭ 후속(map) |
| C-06 | LOW | 페처 anti-stall 가드(커서 비전진/누락) 미테스트 | ⏭ 후속(테스트) |
| C-07 | LOW | `base_url`에 경로 segment 있으면 httpx 절대경로 join이 삭제 | ⏭ 후속(문서) |
| C-08 | INFO | producer 유래 golden 픽스처 부재 → 스키마 drift 미감지 | ⏭ 후속(테스트) |
| P-01 | LOW | **concierge측**: `limit`에 `Query(ge=1,le=500)` 바운드 없음(silent clamp) | ⏭ concierge repo |
...
- **map**: C-04(inactivate 키를 upsert와 동일 정규화로 유도 또는 실제 기록 키 사용),
  C-05(operation 폐쇄 분류 — upsert만 적재, reject/tombstone만 inactivate, 그 외 skip+log),
  C-07(base_url scheme+host[:port]만 문서화), C-06/C-08(페처 가드 테스트 + producer golden
  conformance 테스트).
- **concierge**: P-01(`limit`에 `Query(ge=1,le=500)`로 명시적 422 계약). 또한
  `source_record.source_entity_id`가 한 후보의 upsert·reject/tombstone export에서 byte 동일
  하다는 계약 테스트 권장(map inactivate 조인 전제).
