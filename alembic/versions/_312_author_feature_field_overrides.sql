-- feature.author_feature_field_overrides — route geometry 쓰기를 보조 relation으로 옮긴다 (ADR-099 2단계).
--
-- 판정: 교체(route geometry 쓰기 한정)
--
-- ## 왜 이 파일이 있는가
--
-- 312가 `feature_routes.geom`을 지운다. 그런데 plpgsql 본문은 `pg_depend`를 만들지
-- 않아 **DROP COLUMN이 막히지 않는다** — 실패는 호출 시점에 42703
-- (`column "geom" of relation "feature_routes" does not exist`)으로만 난다.
--
-- 2026-09-19 적대 리뷰가 blocker로 잡았다. `load_bundle`은 `feature_inserted=True`
-- 일 때에도 field patch를 부르므로 **1회차 route 적재**가 바로 죽는다 — 즉
-- "DB 재생성 후 재적재"가 첫 적재부터 막힌다.
--
-- ## 무엇만 바뀌는가
--
-- `UPDATE feature.feature_routes`에서 `geom = CASE ... END,` 절 하나를 떼어
-- `UPDATE feature.feature_route_geometries`로 옮겼다. **판정식은 한 글자도 바뀌지
-- 않는다** — override field path는 여전히 `'route.geom'`이다(그 문자열은 컬럼
-- 참조가 아니라 `ops.feature_override_field_paths`의 registry 키다). 바뀌는 것은
-- `ELSE` 절의 컬럼 alias 하나뿐이다.
--
-- 시그니처가 같으므로 `CREATE OR REPLACE`로 낸다 — 309처럼 DROP+CREATE를 하면
-- 소유자·GRANT를 다시 걸어야 하고, 그 자리를 빠뜨리면 조용히 권한이 사라진다.
--
-- **순서는 subtype → geometry로 고정한다.** `sync_subtype_public_ready`와
-- `feature_subtype.write_subtype`이 같은 순서를 쓴다 — 셋이 어긋나면 40P01 교착
-- 창이 열린다.

SET ROLE ktm_feature_state_procedure_owner;

CREATE OR REPLACE PROCEDURE feature.author_feature_field_overrides(IN p_feature_id uuid, IN p_expected_row_revision bigint, IN p_principal text, IN p_reason_code text, IN p_command_id bigint, IN p_values jsonb, IN p_geometry_wkt jsonb, OUT o_feature_id uuid, OUT o_row_revision bigint, OUT o_command_id bigint, OUT o_applied_field_count integer)
