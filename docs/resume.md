...
- #767 구현을 `fix/t-vn-21r-benchmark-correctness`에 준비했다. Tier-2 harness는
  seed/skip-seed 모드 모두 DB public ID 200개로 batch를 구성하고, viewport별
  `matched_rows`/`returned_rows`/최소 cardinality를 fail-closed로 검증한다.
  shared read는 최상위 Plan 누적값으로 단일화했다.
- 고정 ID 부재·199건 부족·모든 viewport 수량·root/child·Append/parallel plan 회귀
  테스트를 작성했다. **다음 한 작업**: 같은 적대적 리뷰어 1명의
