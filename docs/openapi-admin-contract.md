...
- OpenAPI 산출물은 admin/debug/ops를 포함한 전체 admin spec
  `packages/kor-travel-map-admin/openapi.json`과 TripMate/user-facing subset spec
  `packages/kor-travel-map-admin/openapi.user.json` 두 개다.
...
> **현행 코드 정합 상태(2026-06-06)**: **모든 admin/ops/debug/tripmate 성공 응답이
> `{data, meta}` envelope로 통일 완료**(T-DA-15/16/18). list는 `data.{items,
...
| `ops` | `/ops` | import job 조회, metrics, consistency, `GET /ops/health-deep`(DB/PostGIS readiness; public `/health` liveness와 분리), `GET /ops/system-logs`·`GET /ops/api-call-logs`·`GET /ops/import-job-events`(운영 로그 조회, keyset cursor). api-call-log 적재는 `KOR_TRAVEL_MAP_ADMIN_API_CALL_LOG_ENABLED` opt-in middleware(기본 off, best-effort) |
...
| GET | `/admin/issues/{issue_key}` | 이슈 상세. provider raw 주소, kor-travel-geo 후보, 좌표, 지도 표시 데이터 포함 |
| PATCH | `/admin/issues/{issue_key}` | `resolve`, `ignore`, `reopen`, `retry_geocode`, `retry_reverse_geocode`, `apply_kor_travel_geo_address`, `manual_override` |
...
처리 모드는 `KOR_TRAVEL_MAP_ADMIN_FEATURE_CHANGE_REVIEW_MODE`로 정한다.
...
`KOR_TRAVEL_MAP_ADMIN_BACKUP_COMMAND_ENABLED=true`와 요청 body `execute=true`가 모두 있어야
실행된다.
...
없이 `409`를 반환한다. 응답은 공통 error envelope를 사용한다.
...
`KOR_TRAVEL_MAP_ADMIN_DAGSTER_URL`과 `KOR_TRAVEL_MAP_ADMIN_DAGSTER_GRAPHQL_URL`은
`http`/`https` scheme만 허용하고, host는
`KOR_TRAVEL_MAP_ADMIN_DAGSTER_ALLOWED_HOSTS` allowlist에 있어야 한다. 기본 allowlist는
...
`KOR_TRAVEL_MAP_ADMIN_DAGSTER_REPOSITORY_NAME`, repository location 이름은
`KOR_TRAVEL_MAP_ADMIN_DAGSTER_REPOSITORY_LOCATION_NAME`으로 명시 override할 수 있다.
...
기본 `--profile admin`은 기존 `packages/kor-travel-map-admin/openapi.json`만 생성/검증한다.
TripMate/user subset만 갱신할 때는 다음을 쓴다.

```bash
python packages/kor-travel-map-admin/scripts/export_openapi.py \
  --profile user \
  --output packages/kor-travel-map-admin/openapi.user.json
```
