...
**open PR 없음** (main PR#142 이후까지 merged, Sprint 4(4a+4b) 완료). 다음 작업은
새 feature branch에서 **ADR-045 독립 프로그램화**(Docker compose + admin-first
OpenAPI + 독립 Dagster) / Sprint 5(MOIS-sibling provider + Phase 2)로 시작한다.
...
3. **ADR-045 독립 프로그램 전환 반영** (다음 한 작업) — krtour-map은 Docker 독립
   프로그램 +
   독립 DB/Dagster + TripMate OpenAPI 연동을 기준으로 구현한다. Admin API는
   `docs/openapi-admin-contract.md`, POI/cache target은
   `docs/poi-cache-update-targets.md` 기준.
4. **문서 최신성 유지** — 현재 geocoding endpoint 정본은 REST `/v1/address/*`,
   로컬 포트는 `http://127.0.0.1:8888`, frontend 현재 기준은 Next.js 16 +
   `maplibre-vworld-js#v0.1.2`.
5. **검증 기준** — WSL unit/integration/live pytest + Windows Playwright e2e + GitHub
   Actions green 후 머지.
...
> 차단 의사결정은 `docs/adr045-open-decisions.md`(D-1~D-16). 각 task는 1-PR.
...
- [ ] T-206a-geo — (형제 repo `python-kraddr-geo`, 별도 PR) `POST /v2/regions/
  within-radius` 엔드포인트(`tl_scco_sig` 교차, D-11). **선행**: sig_cd↔sigungu_code
  코드체계 1건 검증. T-206a 선행 의존.
...
**Phase 3 — FastAPI 라우터 (debug-ui 패키지)**
...
**Phase 4 — Dagster (TripMate 복사→구체화)**
- [ ] T-208a — `packages/krtour-map-dagster/` 골격 + definitions + import-linter layer.
- [ ] T-208b — resources(DB/client/provider 9 + kraddr-geo/rustfs, D-15).
- [ ] T-208c — provider load asset 9종(ADR-034 순서, concurrency_key lock).
...
**Phase 5 — Docker / 배포**
- [ ] T-209a — `docker-compose.yml` (서비스 6종, §2, 포트 D-21).
- [ ] T-209b — 기동 순서(postgres→alembic→dagster init→api, D-2).
- [ ] T-209c — Dockerfile 3종(GDAL/PostGIS client).
- [ ] T-209d — `docs/runbooks/docker-app.md` + `deploy.md`.
...
- [ ] T-210c — TripMate `apps/etl` Dagster 자산 → krtour-map 이관(T-208).
...
- **accepted (text on main)**: ADR-001 ~ ADR-045 (전부). PR#16에서 027~034,
  ...
  2026-06-01).
- **001~045 accepted**. **다음 후보 번호 = ADR-046**:
  - **ADR-046+** — 신규 provider 추가 절차 표준 (체크리스트)
  - (필요 시) `krtour-map-debug-ui` → admin/api 패키지 rename ADR (ADR-045 §1 후속)
  - (필요 시) OpenAPI versioning / TripMate client generation 정책 ADR
