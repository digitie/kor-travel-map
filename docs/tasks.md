...
- [/] T-VN-M05-ROLE-CATALOG-RESET — `31fe73ad…` n150 candidate는 정확히 한 번 실행되어 `target_not_isolated` terminal로 보존하며 재시도하지 않는다. PinVi draft PR [#500](https://github.com/digitie/pinvi/pull/500)의 immutable `0bc1b5e8…`은 raw DB 값 없이 identity·namespace·extension·object·role dependency의 fixed enum만 receipt에 기록한다.
- [/] T-VN-M05-MANAGER-PIN-ROTATION — Manager draft PR [#243](https://github.com/digitie/kor-travel-docker-manager/pull/243)은 PinVi result receipt의 transaction·pinset·inode binding과 세분 fixed enum을 strict parse하고 durable terminal journal에 보존한다. PinVi `0bc1b5e8…`·Map `9c64e862…`의 다음 pinset은 `6af965d5…`이다.
