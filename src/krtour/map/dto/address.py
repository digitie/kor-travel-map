...
``krtour.map.dto.coordinate.Coordinate``가 단일 source.
...
정규화/파싱 helper는 ``krtour.map.core.address`` (분리 — dto는 stdlib만).
...
class Address(BaseModel):
    ...
    호출자 또는 ``krtour.map.core.address``의 helper(`normalize_bjd_code` 등)에서.
