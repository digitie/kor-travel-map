...
- **Provider ⑤ KNPS** 14 dataset (`providers/knps.py`) — Point/place 5건
  (visitor_centers/restrooms/campgrounds/shelters/cultural_resources) ✅ 구현
  (`knps_point_records_to_bundles`, ADR-028/034). SHP(area)/route(LineString)
  geometry parsing은 후속(pyshp+shapely). `python-knps-api` `06da125f`.
...
- [x] `src/krtour/map/providers/` — standard_data / kma / opinet / krex /
      visitkorea (enrichment, PR#51) / knps (Point/place 5건) — 6 provider
- [ ] `src/krtour/map/providers/` — knps SHP/route geometry / krheritage
      (Sprint 3) / mois (Sprint 4)
