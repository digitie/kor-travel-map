...
- 차단 해소: T-206b/d, T-208e, T-207a. 반영: `openapi-admin-contract.md §6` +
  plan §2 + `pinvi-rest-api.md`(409 명시).

### D-7 — `/features/{id}` admin↔사용자 응답 분리 ✅ **결정: 분리**
- **결정: 엔드포인트 분리** — `/features/*`(공개, `pinvi-rest-api.md §2` 정제
  필드, 원문/이력 제외) + `/admin/features/*`(source_records 원문 · dedup history ·
  consistency 참조 포함). OpenAPI drift gate도 admin/user 이원(D-3와 연동).
- 차단 해소: T-207e, `pinvi-rest-api.md`. 반영: 두 문서 + OpenAPI.

### D-11 — `sigungu_by_radius` 경계 소스 ✅ **결정: kor-travel-geo에 신규 엔드포인트 추가**
- **결정**: 시군구 경계를 kor-travel-map에 적재하지 않고, **kor-travel-geo**(시군구 경계
  polygon `tl_scco_sig` + PostGIS + v2 REST 이미 보유)에 **신규 엔드포인트를 추가**
  하고 kor-travel-map이 REST로 호출한다. (kor-travel-map `feature.sigungu_boundaries` 신설
  불필요 → T-205b 취소.)
- **kor-travel-geo 신규 엔드포인트 (형제 repo `kor-travel-geo` 별도 PR)**:
  `POST /v2/regions/within-radius` — 기존 v2(POST+JSON) 패턴. 요청/응답/Python
  헬퍼·debug route 정본은 `docs/architecture/regions-within-radius.md`.
...
- **kor-travel-map 측**: `resolve_sigungu_by_radius`가 위 엔드포인트를 `KorTravelGeoRestClient`
  로 호출 → `code` 목록 → `WHERE sigungu_code = ANY(:codes)`로 feature 조회.
- **🟢 기타 코멘트 (확정)**:
  - **코드 체계 = 동일 (사용자 확인 2026-06-01)**: kor-travel-geo `tl_scco_sig.sig_cd`
    (5자리)와 kor-travel-map `feature.sigungu_code`(5자리)는 **같은 체계**. 별도 매핑·
    변환 불필요 — kor-travel-geo 반환 `code`를 그대로 `WHERE sigungu_code = ANY(...)`에
    사용한다.
  ...
  - **대안 검토 기록**: 기존 `/v2/reverse`에 radius 옵션을 얹는 방안은 "한 점→한
    ...
    엔드포인트 늘려도 됨).
- 차단 해소: T-206a. 신규 task: T-206a-geo(kor-travel-geo 엔드포인트, 별도 repo).

### D-14 — offline upload 파일 저장 위치 ✅ **결정: RustFS 무제한 보존**
- **결정: RustFS 버킷(`kor-travel-map-uploads`)에 저장, 보존 만료 없음(무제한)**. 자동
...
- **결정**: 코드에 인증 로직 없음(ADR-005 유지). 운영 인증은 **infra 계층**
  (Cloudflare Tunnel SSO + IP allowlist). PinVi 서비스 토큰은
  **`X-Kor-Travel-Map-Service-Token`** 헤더 pass-through(앱 미검증, "인증된 요청만 도달"
  가정). 반영: ADR-005 amendment, `pinvi-rest-api.md §1`, reverse proxy(T-209).
...
### D-4 — PinVi OpenAPI client ✅ **결정: kor-travel-geo와 동일방식**
- **결정 (2026-06-01 사용자)**: kor-travel-geo가 소비자에게 쓰는 방식 그대로 — **이원**:
  - **PinVi frontend(Next.js/TS)**: kor-travel-map `openapi.user.json` → **`openapi-typescript`
    codegen**(`types/api.gen.ts`) + 폼은 **수동 Zod mirror**. `gen-types.mjs` 류
    스크립트로 생성하고 **CI에서 `openapi.user.json` diff + 산출물 diff 게이트**(kor-travel-geo
    `docs/agent-guide.md`/`frontend-package.md` 패턴). 자동 생성이라 schema 안정화
    대기 없이 지속 동기.
  - **PinVi backend(Python `apps/api`)**: **수기 httpx wrapper**(kor-travel-map의
    `KorTravelGeoRestClient`처럼 직접 작성, codegen 아님). `integrations/kor_travel_map_
...
- 반영: T-210d(backend httpx wrapper) / T-210e(frontend openapi-typescript +
  CI diff), `pinvi-rest-api.md §5`. D-3(versioning/이원 schema)과 정합.
...
- **결정**: **docker env(`KOR_TRAVEL_MAP_<PROVIDER>_*`) → Dagster `ConfigurableResource`**.
...
- **결정 (2026-06-01 사용자)**: RustFS를 **유지** — dev는 docker-compose 서비스로
  ...
  `kor_travel_map` + `kor_travel_map_dagster` + RustFS **3종 묶음**(서로 독립, PinVi와
  분리). offline upload(D-14 무제한)도 RustFS에 보존.
- **포트 기준 (2026-06-03 사용자)**: 로컬 RustFS는 S3 API `12101`, console `12105`.
- 반영: T-209a(compose RustFS 서비스), T-209e/ADR-040 amendment(백업 3종).

### D-9 — `kor-travel-map-admin` 패키지 rename ✅ **결정: 지금 `kor-travel-map-admin`으로**
- **결정 (2026-06-01 사용자)**: 보류 않고 **즉시 rename** — `kor-travel-map-admin`
  → **`kor-travel-map-admin`** (Python namespace `kortravelmap_debug_ui` →
  `kortravelmap.api`, settings env prefix `KOR_TRAVEL_MAP_DEBUG_UI_` → `KOR_TRAVEL_MAP_API_`,
  npm/Docker 이미지명, openapi.json 경로 동반). FastAPI api + Next.js admin frontend을
  담는 패키지이며 Dagster는 별도 `kor-travel-map-dagster`. 라우터 prefix(`/debug` vs
...
### D-13 — shadcn/ui ↔ `@kor-travel-map/map-marker-react` 공존 ✅ **결정: 권고대로**
- **결정**: **분리 유지** — shadcn은 일반 UI primitive, `@kor-travel-map/map-marker-react`는
...
- **결정**: API schema 변경은 CHANGELOG **`### API` 섹션** + SemVer version 태깅
  (D-3 연동). lib(`kor-travel-map`)와 배포 API를 같은 SemVer로 정렬하되 API
