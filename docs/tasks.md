...
- [ ] **T-DA-18** (LOW, API 일관성, **DA-D-03 추가 발견**) `POST /ops/dagster/nux-seen`
  응답이 아직 flat bare(`{status, dagster_url, graphql_url, checked_at, seen, errors}`).
  T-DA-16 dagster/summary 통일 중 발견. `{data, meta}`로 통일 필요(프런트
  `useMarkDagsterNuxSeen`는 본문 미소비라 영향 작음). 별도 소규모 PR.
