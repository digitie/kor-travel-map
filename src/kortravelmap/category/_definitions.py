"""장소 카테고리 체계 (``kortravelmap.category`` 내부 데이터).

이 모듈은 장소 카테고리 코드 체계를 Python enum/dataclass로 반영한다.
...
class PlaceCategoryTier1Code(StrEnum):
    """``place-schema.md``에서 가져온 kor-travel-map Tier 1 category code."""
...
class PlaceCategoryCode(StrEnum):
    """현재 구현된 kor-travel-map 8자리 category code.

    enum 값은 place schema의 ``AABBCCDD`` 계층과 현재 운영 seed를 함께
...
class PlaceCategory:
    """place category seed 한 행.

    속성:
        code: 전체 8자리 ``AABBCCDD`` place category code.
        ...
        sort_order: category seed 정렬값.
        is_active: 서비스에서 활성 category인지 여부.
...
"""``place-schema.md``에서 가져온 kor-travel-map Tier 1 표시명."""
...
"""``place-schema.md``에서 가져온 kor-travel-map Tier 2 표시명."""
...
"""place category code를 Mapbox Maki icon 이름으로 매핑한 table."""
...
def get_category(code: str | PlaceCategoryCode) -> PlaceCategory:
    """8자리 code에 해당하는 place category를 반환한다.
    ...
    예외:
        KeyError: ``code``가 현재 구현된 category seed 집합에 없을 때 발생한다.
    """
...
def is_known_category_code(code: str | PlaceCategoryCode) -> bool:
    """``code``가 현재 category seed 집합에 있으면 ``True``를 반환한다."""
...
def iter_categories(
    *,
    ...
    """place category를 seed 순서대로 순회한다.

    인자:
        depth: 선택 category depth filter. 예를 들어 ``4``를 넘기면 현재 Tier 4로
            정의된 leaf category만 순회한다.
        active_only: 비활성 category를 건너뛸지 여부. 현재 category seed row는 모두
...
def format_category_tree(
    *,
    ...
    """place category 전체 또는 일부를 트리 문자열로 반환한다.
