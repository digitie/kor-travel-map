"""``kortravelmap.core.types`` — KST aware datetime helper re-export.

본 모듈은 ``kortravelmap.dto._time``의 ``KST`` / ``kst_now``를 그대로 re-export
한다. 정의는 ``dto/_time.py``에 있다 (의존 방향 보존: ``dto → core → infra``,
core는 dto를 import 가능하나 그 반대는 import-linter 위반 — ADR-002).

호출 측은 본 모듈 또는 ``kortravelmap.core`` package re-export를 통해 사용:

    from kortravelmap.core import KST, kst_now
...
from kortravelmap.dto._time import KST, kst_now
