    ...
    return HTTPException(
        ...
        detail={
            ...
            "message": (
                ...
                "KRTOUR_MAP_ADMIN_BACKUP_COMMAND_ENABLED=true 설정 후 실행하세요."
            ),
            "details": {},
        },
    )
...
def _script_path(settings: AdminSettings, script: Path) -> Path:
...
async def create_backup(
    ...
    env = {
        ...
        "KRTOUR_MAP_BACKUP_ID": backup_id,
        "KRTOUR_MAP_BACKUP_ALLOW_RUNNING": "1" if payload.allow_running else "0",
    }
    plan = _command_plan(
        settings=settings,
...
async def restore_backup(
    ...
    env = {
        ...
        "KRTOUR_MAP_RESTORE_RECREATE": "1" if payload.recreate else "0",
        "KRTOUR_MAP_RESTORE_SKIP_CHECKSUM": "1" if payload.skip_checksum else "0",
        "KRTOUR_MAP_RESTORE_SKIP_RUSTFS": "1" if payload.skip_rustfs else "0",
    }
    plan = _command_plan(
        settings=settings,
