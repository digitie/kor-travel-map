...
class RustfsFileStore:
    ...
    bucket: str
    prefix: str = "features"
    public_base_url: str | None = None        # http://cdn.example.com/kor-travel-map
...
KOR_TRAVEL_MAP_OBJECT_STORE_BUCKET=kor-travel-map
KOR_TRAVEL_MAP_OBJECT_STORE_REGION=us-east-1
...
`KOR_TRAVEL_MAP_OBJECT_STORE_PUBLIC_BASE_URL`을 CDN URL로 설정하면 frontend가
직접 그 URL로 이미지 로드:
```
https://cdn.tripmate.example.com/kor-travel-map/features/visitkorea/f_.../primary-0-d2a4f1e7.jpg
```
...
- import: `kor_travel_map.files` → `kortravelmap.files`, `kor_travel_map.rustfs` →
  `kortravelmap.rustfs`.
- async-only: upload 헬퍼는 `async def`.
- backend 추상화 강화: `storage_backend`가 `'rustfs'`/`'s3'`/`'minio'` 등 자유
  값 허용 (v1은 `'rustfs'`만 허용). ADR-015 미러.
- env prefix: `KOR_TRAVEL_MAP_RUSTFS_*` 유지 (호환) + 새 `KOR_TRAVEL_MAP_OBJECT_STORE_*`
  alias 추가.
