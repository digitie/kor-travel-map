...
| `knps_shelters` | 국립공원 대피소 공간데이터 | `place` | 전용 코드 미존재 — **`LODGING_MOUNTAIN_SHELTER`** 신설 권고 | 산장/긴급 대피소 |
| `knps_access_restrictions` | 국립공원 입산통제구간 | `notice` (+ area) | n/a (notice는 `notice_type`으로) | `notice_type=forest_access_restriction` 신규 (notice-feature-etl.md 확장) |
| `knps_vegetation_zones` | 국립공원 식생도 | `area` | 전용 코드 미존재 — **`NATURE_ECOLOGY`** 신설 검토 | 학술용. 멸종위기종 위치는 `status='hidden'` 검토 |
| `knps_species_habitats` | 멸종위기종 서식지 | `area` | 동일 | 보안상 마스킹 필요 |
...
| `knps_fire_alerts` | 산불경보 / CCTV stream | `notice` | n/a | `notice_type=forest_fire_alert` |
...
### 11.6 카테고리 확장 후보 (ADR-026 신규 후보)

KNPS dataset 통합 시 카테고리/도메인 확장 필요:

| 추가 후보 | 위치 | 사유 |
|----------|------|------|
| `SAFETY_HAZARD_ZONE` | 새 Tier 1 `08` `SAFETY` 또는 기존 `01020100` 하위 | 위험지역 (낙석/급류 등) |
| `LODGING_MOUNTAIN_SHELTER` | `03080000` 또는 `03030000` 하위 | 대피소/산장 (캠핑장과 구분) |
| `WEATHER_MOUNTAIN_STATION` | `kind=weather`용 anchor (실제 카테고리는 `01050300` 대용 가능) | 산악 관측소 |
| `NATURE_ECOLOGY` | `01020000` 하위 | 식생/서식지 학술 자료 |
| `area_kind=hazard_zone` | `kraddr.base.domains` | AreaDetail 변종 |
| `notice_type=forest_access_restriction` | `docs/notice-feature-etl.md` 확장 | 입산통제 |
| `notice_type=forest_fire_alert` | 동일 | 산불경보 |

각 확장은 별도 ADR (예: ADR-026) + 본 doc §11.6 갱신.
...
3. **본 라이브러리에 통합**:
   - ADR-026 (카테고리 확장) + ADR-027 (`python-knps-api` provider 등록)
