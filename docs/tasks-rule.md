...
- **아카이브 파일명**: `archive/{resume,journal}-YYYY-MM[a|b|c].md`. 한 파일이 **220 KB**를 넘으면
  같은 달 안에서 `a`/`b`/`c`로 더 쪼개 **모든 파일이 단독으로 읽히게** 한다.
...
- **새 엔트리는 항상 live 문서 상단에** 추가한다. 아카이브 파일은 원칙적으로 읽기 전용 이력이다.
  ...
  220 KB 경계 초과를 해소하는 섹션 단위 경계 재조정뿐이다. 이 경우 섹션 본문은 그대로 보존하고
  변경 근거를 live 문서에 기록한다.
