...
4. Map은 consumer-independent service contract로 event를 pull delivery하고 principal별 ack를
   append-only로 보존한다. 소비자는 자기 DB transaction 안에서 참조 rebind/detach receipt와
   영향 row를 commit한 뒤에만 ack한다. Map은 특정 소비자 이름을 식별자·role·환경변수·route에
   넣지 않는다.
...
- `0231_m05_manual_provider_dedup` migration, 전용 procedures/ACL/bootstrap/restore/root를
  구현한다.
