...
- shadcn/ui: Button, Input, Select, Dialog, Sheet, Tabs, Table, Badge, Toast,
  Form, DropdownMenu 등 공통 UI primitive. `Table` primitive는 DataTable의 표시 계층으로만 쓴다.
...
- `/ops/logs`: T-221e에서 system/API log에 더해 `ops.import_job_events` 전역 stream을
  붙였다. `provider`/`dataset_key`/`job_id`/`level` 필터와 job 상세 링크를 제공한다.
- `/debug/explain`: T-221e 재판정 결과 admin REST/UI로 열지 않는다. raw SQL 입력
  화면은 운영 콘솔 blast radius가 크고, 성능 검증은 통합 테스트 EXPLAIN gate와
  운영 DB read-only 세션/runbook으로 충분하다. 필요해지면 별도 task에서 allowlist
  query catalog 방식으로만 재검토한다.
- `/debug/fixtures`: 별도 REST/UI를 만들지 않는다. 현재 `/debug/etl`의 fixture/live
  변환 preview와 `docs/debug-fixture-workflow.md`의 파일 기반 capture/replay가 정본이다.
  provider별 fixture 저장 자동화가 필요해지면 provider 라이브러리 쪽 fixture 도구와 함께
  별도 task로 연다.
...
4. 주소
   ...
   - kor-travel-geo REST v2 reverse/geocode 재검증 버튼.
   - provider 원천 주소와 좌표 기준 kor-travel-geo 주소 비교 결과.
...
import job event stream을 provider/API/job 실패 확인용으로 조회한다. system/API 호출 로그는
각각 `GET /ops/system-logs`, `GET /ops/api-call-logs`에서 조회하고, admin UI는
`/ops/logs` 한 화면의 탭으로 묶는다.
...
| `missing_address` | 주소 없음 또는 kor-travel-geo 정규화 실패 | geocode/reverse 재시도, manual address 입력 |
| `missing_bjd_code` | 법정동코드 없음 | kor-travel-geo 보강, manual code 입력 |
| `provider_address_mismatch` | provider 주소와 좌표 기준 kor-travel-geo 주소가 서로 다른 장소로 보임 | 지도 비교, 좌표/주소 수정, source ignored |
| `provider_address_partial_match` | 시군구/읍면동은 맞지만 상세 주소가 불완전하거나 다름 | kor-travel-geo 결과 채택, manual address 보정 |
...
결측/정합성 이슈 처리. 주소/좌표 이슈는 kor-travel-geo 재시도와 수동 override를 모두
지원해야 한다.
...
`apply_kor_travel_geo_address`는 좌표 기준 kor-travel-geo reverse 결과를 그대로 정본으로
...
- `KOR_TRAVEL_MAP_OFFLINE_UPLOAD_MAX_BYTES`를 넘는 파일은 `413`으로 거절한다.
...
3. schema/header 확인.
4. 좌표 범위 확인.
5. `bjd_code`가 없으면 주소 geocode, 그래도 안 되면 좌표 reverse로 kor-travel-geo 보강.
6. DTO `FeatureBundle` 변환 가능성 확인.
7. issue summary 생성.

`KOR_TRAVEL_MAP_KOR_TRAVEL_GEO_BASE_URL=http://127.0.0.1:12501`가 설정된 경우 admin API는
kor-travel-geo REST v2 `POST /v2/geocode`, `POST /v2/reverse`를 호출한다. 설정이 없거나
결과에 법정동코드가 없으면 좌표가 있는 행은 `reverse_geocode_failed`,
좌표·주소가 모두 없는 행은 `missing_address` issue가 된다.
...
- load job이 실제 실행되면 `AsyncKorTravelMapClient.run_offline_upload_load_job()`이
