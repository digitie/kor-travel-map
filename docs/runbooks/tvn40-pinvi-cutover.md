...
1. `MAP_SHA=<40C 머지 커밋 40자>`를 확정한다. 이 절차의 release provenance는
   40C 머지 커밋을 사용하지만, T-VN-40 receipt의 n150 isolated acceptance는 별도의
   detached source pair를 증언할 수 있다. 두 값을 재실행 없이 서로 바꾸지 않는다.
   receipt의 Map/PinVi source SHA가 현재 checkout의 branch/tag에 없더라도 설치기가
   `origin`에서 exact SHA를 fetch하므로, 해당 SHA가 원격 object 조회로 재현 가능한지
   먼저 확인한다.
