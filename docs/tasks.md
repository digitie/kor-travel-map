...
- [ ] **T-DA-15** (MED, API 일관성, **DA-D-03 = 전면 통일**) list 응답 셰입 이원화
  ...
  - [x] `/admin/offline-uploads` → 동일 envelope (2026-06-06).
  - [ ] `/admin/poi-cache-targets` 잔여.
- [ ] **T-DA-16** (MED, API 일관성, **DA-D-03 = 전면 통일**) 단건 응답 envelope 불일치
  ...
  - [x] `/admin/offline-uploads/{id}` → `{data, meta}` (2026-06-06).
  - [ ] `/admin/poi-cache-targets/{id}`, `/ops/dagster/summary`, `/ops/metrics`,
    `/debug/mois-license/{id}`, `/ops/import-jobs/{job_id}`(meta 추가) 잔여.
