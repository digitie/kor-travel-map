...
`kor-travel-map`의 **디버그 REST API + admin 운영 UI** 별도 Python 패키지.
...
- **패키지명**: `kor-travel-map-admin` (PyPI distribution) / `kortravelmap.admin` (Python import, ADR-022)
- **위치**: `kor-travel-map` 저장소 내 `packages/kor-travel-map-admin/`
  (monorepo)
...
- **운영 형태**: Docker에서 실행되는 kor-travel-map 독립 프로그램의 API/admin UI.
...
uvicorn kortravelmap.admin.app:app --host 127.0.0.1 --port 12301 --reload
```
...
### Backend (`KOR_TRAVEL_MAP_ADMIN_*`)

| 변수 | 기본값 | 설명 |
|------|--------|------|
| `KOR_TRAVEL_MAP_ADMIN_HOST` | `127.0.0.1` | uvicorn 바인드 host (외부 노출 금지) |
| `KOR_TRAVEL_MAP_ADMIN_PORT` | `12301` | uvicorn 포트 |
| `KOR_TRAVEL_MAP_ADMIN_RELOAD` | `false` | dev 모드 hot-reload |
| `KOR_TRAVEL_MAP_ADMIN_CORS_ALLOW_ORIGINS` | `http://localhost:12305` | Next.js dev 서버 |
...
| `KOR_TRAVEL_MAP_ADMIN_BACKUP_ROOT` | `data/backups` | `/admin/backups`가 조회하는 backup artifact root |
| `KOR_TRAVEL_MAP_ADMIN_BACKUP_PROJECT_ROOT` | `.` | backup/restore script 상대 경로를 해석하고 command를 실행할 project root |
| `KOR_TRAVEL_MAP_ADMIN_BACKUP_SCRIPT_PATH` | `scripts/docker-backup.sh` | backup command plan이 호출하는 script path |
| `KOR_TRAVEL_MAP_ADMIN_RESTORE_SCRIPT_PATH` | `scripts/docker-restore.sh` | restore command plan이 호출하는 script path |
| `KOR_TRAVEL_MAP_ADMIN_BACKUP_COMMAND_ENABLED` | `false` | `POST /admin/backups`, `POST /admin/restore/{backup_id}`의 host command 실행 허용 여부. false면 plan-only |
| `KOR_TRAVEL_MAP_ADMIN_BACKUP_COMMAND_TIMEOUT_SECONDS` | `1800` | opt-in host command 실행 timeout |
| `KOR_TRAVEL_MAP_ADMIN_RESTORE_APP_DB` | `kor_travel_map_restore` | staging restore app DB 기본값 |
| `KOR_TRAVEL_MAP_ADMIN_RESTORE_DAGSTER_DB` | `kor_travel_map_dagster_restore` | staging restore Dagster DB 기본값 |
| `KOR_TRAVEL_MAP_ADMIN_RESTORE_RUSTFS_VOLUME` | `kor-travel-map-rustfs-restore` | staging restore RustFS volume 기본값 |
| `KOR_TRAVEL_MAP_ADMIN_FRONTEND_DIST` | (auto) | static export 모드 시 `frontend/out/` 경로 |
| `KOR_TRAVEL_MAP_ADMIN_FEATURE_CHANGE_REVIEW_MODE` | `require_review` | place/event feature 추가·수정·삭제 요청 처리 모드. `require_review` 또는 `immediate` |
...
| `NEXT_PUBLIC_KOR_TRAVEL_MAP_ADMIN_API` | 개발 기본 `http://127.0.0.1:12301` | 백엔드 base URL. production에서는 명시 필수 |
