...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#235(T-RV-31/32/33 router/executor
정확성)까지 merged했고, 이번 PR은 `T-209b-a` Dagster schedule/run/event storage
PostgreSQL 전환을 닫는다. `T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라
production 레벨 hardening 전까지 구현하지 않고 문서 추적만 유지한다. 남은 T-RV 실행
품질 묶음(`T-RV-34/35`)은 별도 T-RV 백로그에서 계속 추적한다.
TripMate `docs/krtour-map-requirements.md` 대조 후속은 `T-213a~h`로 별도 등록되어 있다
(`docs/reports/tripmate-requirements-reconcile-2026-06-06.md`). Codex 현재 다음 후보는
**T-201b Phase 2 dry-run report** 또는 **T-209 Docker/daemon polish**다. ADR-045 관련
잔여 task가 닫히면 T-212 전체점검 묶음으로 넘어간다. T-207b는 사용자 결정에 따라
구현하지 않는다.
