"""``kortravelmap.admin.app`` — FastAPI application factory.
...
uvicorn 설정은 ``AdminSettings``(``KOR_TRAVEL_MAP_ADMIN_*`` env) 또는 호출자가
...
from kortravelmap.admin import __version__
...
async def _record_api_call_safe(
    *,
    ...
    try:
        ...
        from kortravelmap.admin.db import _get_engine
...
def create_app(settings: AdminSettings | None = None) -> FastAPI:
    ...
    라우터/DTO 변경 시 ``packages/kor-travel-map-admin/openapi.json`` drift
...
``uvicorn kortravelmap.admin.app:app``로 직접 실행, ``scripts/export_openapi.
