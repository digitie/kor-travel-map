...
- **Provider ⑤ KNPS** 14 dataset (`providers/knps.py`) — Point/place 5건 +
  ...
  **knps-api 책임**(ADR-028 Amendment I / ADR-044). ⚠️ CSV 컬럼명 best-guess —
  ...
  (통계 3건은 feature 본문 X.)
- **geocoding (kraddr-geo v2 연동)** (`krtour.map.geocoding`) ✅ — 좌표↔주소
  보강. kraddr-geo를 import하지 않고 v2 응답(`reverse_v2`/`geocode_v2`) structural
  Protocol로 소비. 순수 변환 `reverse_v2_to_address`/`geocode_v2_to_coordinate` +
  비동기 콜러블 팩토리 `kraddr_geo_{reverse,address}_geocoder` +
  `cached_reverse_geocoder`(좌표 메모이즈).
...
- **Provider ⑥ krheritage** (`providers/krheritage.py`) ✅ — 국가유산 place +
  ...
  **남은 것**: 미디어 file_sources(FeatureFileSource DTO 미구현) / GIS spca 면적
  보강(area_square_meters) / knps 사찰↔krheritage temple dedup(§2.5).
