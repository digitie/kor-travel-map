...
def _item_set_vector(
    items: tuple[curation_repo.CurationServiceItemSnapshot, ...],
) -> list[dict[str, str]]:
    return [
        {
            "curation_item_id": item.curation_item_id,
            "row_revision": str(item.row_revision),
            "item_payload_hash": curation_snapshot_sha256(_item_payload(item)),
        }
        for item in items
    ]
...
@service_router.get(
    ...
    responses={200: {"headers": _ETAG_HEADER}, 304: {"description": "ETag 일치"}},
...
async def get_curation_item_detail_snapshot(
    ...
    if request.headers.get("if-none-match") == strong_etag:
...
@service_router.get(
    ...
    responses={
        ...
        304: {"description": "첫 page collection ETag 일치"},
...
async def get_curation_collection_detail_snapshot(
    ...
    item_set_hash = curation_snapshot_sha256(_item_set_vector(snapshot.items))
    identity_payload = _collection_identity_payload(
        snapshot,
        item_set_hash=item_set_hash,
    )
    ...
    if cursor is None:
        ...
        cursor_payload = _decode_cursor(cursor, key=key)
        if (
            ...
            or str(cursor_payload["item_set_hash"]) != item_set_hash
        ):
            raise HTTPException(
                status_code=409,
                detail="curation collection snapshot changed; restart from first page",
            )
        last_item_id = str(cursor_payload["last_item_id"])
        try:
            start = next(
                index + 1
                for index, item in enumerate(snapshot.items)
                if item.curation_item_id == last_item_id
            )
        except StopIteration as exc:
            raise HTTPException(
                status_code=409,
                detail="curation collection cursor item is no longer present",
            ) from exc

    page = snapshot.items[start : start + page_size]
    complete = start + len(page) == len(snapshot.items)
    next_cursor = None
    ...
    return CurationCollectionDetailSnapshot.model_validate(
        {
            **identity_payload,
            "etag": collection_etag,
            "items": [_item_view(item) for item in page],
