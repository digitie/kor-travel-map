...
- 메인 import root를 `kortravelmap`으로 이동하고 권장 예시를
  ...
  `kortravelmap.admin`, `kortravelmap.dagster` namespace로 맞췄다.
...
- 구 `kortravelmap` / `kortravelmap.admin` / `kortravelmap.dagster` /
...
- `.env.example`, `scripts/load-env.sh`, `docker-compose.yml`, Dockerfile expose,
  `AdminSettings`, main settings, frontend 기본 URL/Playwright 설정을 새 포트로 정렬.
...
- **Backend schema**: `GET /admin/features/change-requests` meta에 `review_mode`를 추가해
  `KOR_TRAVEL_MAP_ADMIN_FEATURE_CHANGE_REVIEW_MODE` 값을 빈 큐에서도 표시한다.
...
- `/admin/features`에 `POST`, `/admin/features/{feature_id}`에 `PATCH`/`DELETE`,
  ...
  `KOR_TRAVEL_MAP_ADMIN_FEATURE_CHANGE_REVIEW_MODE=require_review`, 설정이 `immediate`면 같은
...
- **검증**: `pytest -s packages/kor-travel-map-admin/tests/test_dagster_router.py -q` 8 passed,
...
- **Admin API**: `/admin/backups`, `/admin/backups/{backup_id}`,
  ...
  실행은 기본 plan-only이며 `KOR_TRAVEL_MAP_ADMIN_BACKUP_COMMAND_ENABLED=true` opt-in에서만
  host command를 실행한다.
...
- **Offline upload**: upload create의 `detected_encoding`은 `None`으로 저장해 parser
  ...
  `KOR_TRAVEL_MAP_ADMIN_DAGSTER_REPOSITORY_NAME` /
  `KOR_TRAVEL_MAP_ADMIN_DAGSTER_REPOSITORY_LOCATION_NAME` 설정으로 이동한다.
...
- **SSRF guard**: `KOR_TRAVEL_MAP_ADMIN_DAGSTER_ALLOWED_HOSTS` allowlist를 추가하고
...
- **OpenAPI**: `packages/kor-travel-map-admin/openapi.json`과 `openapi.user.json`을
...
- **Export profile**: `packages/kor-travel-map-admin/scripts/export_openapi.py`에
  `--profile admin|user|all`을 추가했다. 기본 admin profile은 기존
  `packages/kor-travel-map-admin/openapi.json`을 유지한다.
- **User spec**: `packages/kor-travel-map-admin/openapi.user.json`을 추가했다. 포함 경로는
...
- **Repo/OpenAPI**: `feature_repo.get_feature_rows_by_ids`,
  `feature_repo.search_features`를 추가하고 `packages/kor-travel-map-admin/openapi.json`을
  재생성했다.
...
- **OpenAPI**: `packages/kor-travel-map-admin/openapi.json`을 재생성하고 계약 문서를
  갱신했다.
...
- **OpenAPI/DB**: `alembic 0010`으로 `ops.feature_overrides`를 추가하고
  `packages/kor-travel-map-admin/openapi.json`을 갱신했다.
...
- **OpenAPI**: `packages/kor-travel-map-admin/openapi.json`을 재생성했다.
...
- **원인**: `.env`의 로컬 `KOR_TRAVEL_MAP_ADMIN_DAGSTER_URL=http://127.0.0.1:12302`이
  compose interpolation에 그대로 사용되어 API 컨테이너 안에서 자기 자신을 조회.
- **수정**: Docker compose는 `KOR_TRAVEL_MAP_DOCKER_ADMIN_DAGSTER_URL`을 읽어 API 컨테이너
  내부 `KOR_TRAVEL_MAP_ADMIN_DAGSTER_URL`로 주입. 기본값은 `http://dagster:12302`.
...
- **Backend**: `GET /ops/dagster/summary` 추가. `KOR_TRAVEL_MAP_ADMIN_DAGSTER_URL`
...
- **검증**: Dagster router unit test, admin backend 전체 pytest, ruff,
  `mypy --strict -p kortravelmap.admin`, frontend type-check/lint/build 통과. OpenAPI
...
- **스크립트**: `scripts/load-env.sh`가 `.env`의 provider key를
  `KOR_TRAVEL_MAP_ADMIN_*`/`NEXT_PUBLIC_*`로 매핑. `stop-fixed-ports.sh`,
...
## 2026-05-31 (codex) — kor-travel-geo 포트 정합 + 라이브 검증 준비

**작업**: 사용자 지시 "라이브러리 최신버전을 기준으로 업데이트"에 따라 로컬
`F:\dev\kor-travel-geo` 최신 `main`과 `docs/ports.md`를 확인하고, 지오코딩 REST
기본 연동 포트를 공식 FastAPI backend `http://127.0.0.1:12201`로 정렬함. 기존
Next proxy(`13088/api/proxy`) 또는 컨테이너 예제(`kor-travel-geo:8080`)는 테스트 기본값과
문서 예시에서 제거했다.

- `packages/kor-travel-map-admin/settings.py`: `KOR_TRAVEL_MAP_ADMIN_KOR_TRAVEL_GEO_BASE_URL`
  기본값을 `http://127.0.0.1:12201`로 지정. 명시적으로 `None`을 주면 기존처럼
  `/debug/geocoding/*` 503 응답.
- `tests/*live*.py`: geocoding live 기본 URL을 `http://127.0.0.1:12201`로 정렬.
...
- 로컬 `maplibre-vworld-js` 최신 tag `v0.1.2`도 확인해 frontend와
  `@kor-travel-map/map-marker-react`의 git URL 핀을 `#v0.1.2`로 올림. Next.js 16에서
...
**검증**:
- WSL ext4 샌드박스 `/home/digitie/dev/kor-travel-map`에 NTFS 원본 rsync 후
  editable install 성공 (`gdal-3.8.4` wheel).
- `pytest tests/unit`: 642 passed.
- `pytest packages/kor-travel-map-admin/tests -m "not live"`: 113 passed / 45 deselected.
- kor-travel-geo live 12201 geocoding/debug/provider tests: 45 passed.
- `pytest tests/integration`: 35 passed. `test_dedup_with_kor_travel_geo_live.py`: 5 passed.
...
- `docs/dev-environment.md`: 제목과 본문 첫 정책 설명에 Windows Git 원본 +
  WSL 실행 모델을 명시.
- `AGENTS.md`, `docs/codegraph-worktree.md`: 남아 있던 `~/dev/kor-travel-map-*`
  예시를 `F:\dev\kor-travel-map-*` 기준으로 정리.

## 2026-05-31 (antigravity) — 개발 정책 NTFS 메인레포 전환 및 에이전트 워크트리 재설정

**작업**: 개발 및 형상관리의 중심을 WSL ext4에서 NTFS(`F:\dev\kor-travel-map`)로 전면 이전함. WSL ext4는 가상/컨테이너 가속 테스트(PostGIS testcontainers) 실행을 위한 **샌드박스**로 역할을 재규정함. 이에 따라 에이전트별 worktree를 NTFS상에 신설 및 프리픽스를 `kor-travel-map-`으로 개정하고 로컬 키값(`.env`)을 동기화 완료함. 정책 관련 문서 3종을 전면 정비하여 PR#110 머지 완료.

- **산출물**:
  - `kor-travel-map-codex` (worktree): F:\dev\ 하위에 `sandbox/codex` 브랜치로 신설 및 `.env` 키값 복사.
  - `kor-travel-map-claude` (worktree): F:\dev\ 하위에 `sandbox/claude` 브랜치로 신설 및 `.env` 키값 복사.
  - `kor-travel-map-antigravity` (worktree): F:\dev\ 하위에 `sandbox/antigravity` 브랜치로 신설 및 `.env` 키값 복사.
- **문서 및 설정 개정**:
...
**작업**: 사용자 지시에 따라 `maplibre-vworld-js` 프로젝트의 스타일(`react-doctor.config.json`) 및 에이전트별 MCP 설정 파일(`.gemini/mcp.json`, `antigravity.json`, `claude.json`, `.codex/config.toml`)을 가져와서 현재 프로젝트의 worktree 경로(`F:\dev\kor-travel-map-*`)에 맞춤 보정 후 적용. PR #107 생성 후 성공적으로 머지 및 리모트/로컬 동기화 완료.

- **산출물**:
  - `react-doctor.config.json` (프로젝트 루트): React 정적 분석 규칙 및 스타일 일관성 검사 예외 설정 복사.
  - `.gemini/mcp.json` (새로 생성): Antigravity 에이전트용 `codegraph` (`cwd: F:\\dev\\kor-travel-map-antigravity`), Playwright, Sequential Thinking MCP 서버 등록.
  - `antigravity.json` (프로젝트 루트): Antigravity용 MCP 구성 동기화.
  - `claude.json` (프로젝트 루트): Claude Code용 `codegraph` (`cwd: F:\\dev\\kor-travel-map-claude`) 등 MCP 설정 동기화.
  - `.codex/config.toml` (프로젝트 루트): Codex용 `codegraph` (`cwd: F:\\dev\\kor-travel-map-codex`) 등 MCP 설정 동기화.
- **배포 및 통합**:
...
- DB 적재: `infra/feature_repo.py` raw SQL 3-table upsert / `infra/dedup_repo.py`
  + `ops.dedup_review_queue` / `AsyncKorTravelMapClient` 오케스트레이터.
...
- 데이터 통로: geocoding **python API → REST API v2** 전환(httpx 주입,
  TYPE_CHECKING-only) — kor-travel-geo DB/패키지 의존 0.
...
- **PR#92 — workspace 루트 + frontend WSL 기동 + 라이브 e2e + 버그 fix**: 저장소에
  ...
  `"@kor-travel-map/map-marker-react": "workspace:*"`(pnpm/yarn 문법) → npm 호환 `"*"`.
...
큐(#121/#122) + kor-travel-geo 호출 python API → REST API v2 전환(#123). 5 PR 머지.
...
- **PR#89 (#122) — `AsyncKorTravelMapClient` 오케스트레이터**: placeholder 진입점에
...
- **PR#90 (#123) — geocoding python API → REST API v2 전환**: 기존 geocoding은
  ...
  현 kor-travel-geo에 **존재하지 않음**(미존재). 실제 REST API(`/v1/address/*`,
  ...
  `cached_reverse_geocoder`) 유지 → provider 무영향. `KOR_TRAVEL_MAP_KOR_TRAVEL_GEO_BASE_URL`
...
**작업**: 사용자 "krheritage 진행". `src/kortravelmap/providers/krheritage.py` 신설.
...
(-p kortravelmap, 46 files) / import-linter 4 kept / coverage gate green.
...
보강은 불가('global' bucket 고정). + kor-travel-geo v2는 async(`reverse_v2`).
...
(-p kortravelmap, 45 files) / import-linter 4 kept / openapi --check 0 green.

**경계**: kor-travel-geo client 수명·실제 호출은 호출자(TripMate/Dagster). 본 lib는
async 변환 + geocoder 주입 지점까지. DB write(적재) 경로는 여전히 feature_repo.

**다음**: provider ⑥ krheritage 또는 실 DB 적재 오케스트레이션.

## 2026-05-29 (claude) — KNPS provider 재검토 + kor-travel-geo v2 함수 연동(`kortravelmap.geocoding`)
...
**② kor-travel-geo v2 함수 연동 — `kortravelmap.geocoding` 신설**: kor-travel-geo가
...
- 응답 DTO: `*V2Response`(status: "OK"/"NOT_FOUND"/"ERROR", candidates:
  ...
  `Point`(x=lon,y=lat).
- 구현: kor-travel-geo를 import하지 않고 **structural Protocol**로 소비(ADR-006,
  ...
  팩토리 `kor_travel_geo_reverse_geocoder`/`kor_travel_geo_address_geocoder`(client는
  `KorTravelGeoClient` Protocol, 수명은 호출자). async 타입 별칭 `AddressGeocoder`/
  `ReverseGeocoder`(docs §2).
- import-linter layers에 `kortravelmap.geocoding`(providers↔infra 사이) 추가.
- 테스트 17건(매핑/confidence/fallback/잘못된코드/status/팩토리). 실제 kor-travel-geo
  없이 fake v2 응답으로 검증. `KorTravelGeoClient` Protocol 구조 적합성도 mypy 확인.

**검증(로컬)**: unit+lint **554** / ruff / mypy --strict(-p kortravelmap, 45 files) /
import-linter 4 kept green.

**경계**: 데이터 정합성 1차 책임은 kor-travel-geo(ADR-044). 본 모듈은 v2 응답 신뢰·
...
- debug-ui `db.py` — `get_session` FastAPI 의존성 (메인 lib `KorTravelMapSettings.
...
- apihub는 `authKey`(apihub.kma.go.kr)로 인증 — data.go.kr `serviceKey`와 **별개
  키** → `settings.kma_apihub_key` (`KOR_TRAVEL_MAP_ADMIN_KMA_APIHUB_KEY`) +
...
**신규** (2):
- `src/kortravelmap/providers/visitkorea.py` (~290 line) — datagokr 1차로 적재된
...
**작업**: PR#48 — agent worktree 접두사 `geo-*` → `kor-travel-map-*` 일괄 rename
+ `docs/tasks.md` 최신화 (순수 docs).

**worktree rename** (사용자 지시 — `kor-travel-map-` 접두사):
- `geo-codex`/`geo-claude`/`geo-antigravity` → `kor-travel-map-codex`/
  `kor-travel-map-claude`/`kor-travel-map-antigravity`.
...
- `docs/codegraph-worktree.md` §2 rationale 재작성 — 접두사를 본 저장소
  (`kor-travel-map`) 이름에서 따와 한 머신의 여러 저장소 worktree를 1:1
  식별. (이전 `geo-*`는 형제 `kor-travel-geo`와 모호.)
...
- 각 provider repo (`python-kma-api`/`python-opinet-api`/…)의 `.env`에 박힌
  키 이름을 그대로 가져오고, prefix `KOR_TRAVEL_MAP_ADMIN_`만 붙여 디버그 UI
  `.env`로 옮긴다.
- 예: `python-kma-api/.env`의 `KMA_SERVICE_KEY=...` → 디버그 UI의
  `.env`에 `KOR_TRAVEL_MAP_ADMIN_KMA_SERVICE_KEY=...`로 저장.
- ADR-005 + ADR-035: 운영 시 Cloudflare Tunnel/SSO 뒤. `SecretStr` 보호
  (plaintext 로그/JSON 노출 방지).

**신규 파일** (2):
- `packages/kor-travel-map-admin/.env.example` (8 provider 키 자리 + 컨벤션
  주석)
- `packages/kor-travel-map-admin/src/kortravelmap/admin/etl_live.py` (~270
  line):
...
- `src/kortravelmap/admin/settings.py`: 8 `SecretStr | None` field 추가
  (kma/opinet/datagokr/visitkorea/krex/knps/airkorea/krforest)
- `src/kortravelmap/admin/routers/etl.py`:
...
- `cd packages/kor-travel-map-admin && python -m pytest -q` → **21 passed**
  (PR#46 18 + PR#47 3)
- `ruff` All checks passed
- `mypy --strict src packages/kor-travel-map-admin/src` → **no issues found in
  48 source files**
...
**변경 — 본 lib** (3):
- `src/kortravelmap/providers/kma.py`:
...
- `src/kortravelmap/providers/krex.py`: `TRAFFIC_NOTICE_CATEGORY`
  `"06010000"`(PARKING 오용) → `"99000000"` (notice placeholder) 정정
- `src/kortravelmap/providers/__init__.py` — kma 9 신규 re-export
...
**Verification**:
- `pytest tests/ packages/kor-travel-map-admin/tests/ --ignore=tests/integration
...
**신규 파일** (2):
- `src/kortravelmap/providers/krex.py` (~520 line):
...
**변경 파일** (2):
- `src/kortravelmap/providers/__init__.py` — krex 18 신규 식별자 re-export
...
- `ruff check src/ tests/ packages/kor-travel-map-admin/` → All checks passed
- `mypy --strict src/kortravelmap packages/kor-travel-map-admin/src/krtour/
...
**신규 파일 — backend** (2):
- `packages/kor-travel-map-admin/src/kortravelmap/admin/etl_fixtures.py`
  (~340 line):
  ...
  - `list_providers()` / `list_datasets(provider)` / `run_fixture_preview(
    provider, dataset)`
- `packages/kor-travel-map-admin/src/kortravelmap/admin/routers/etl.py`
  (~150 line):
...
**신규 파일 — frontend** (2):
- `packages/kor-travel-map-admin/frontend/src/api/etl.ts` — TanStack Query
  hook: `useProviders` (60s staleTime), `useEtlPreviewMutation`
- `packages/kor-travel-map-admin/frontend/src/app/etl/page.tsx` — provider/
  dataset/source 선택 UI + Preview 실행 버튼 + 결과 JSON 표시

**변경 파일** (4):
- `packages/kor-travel-map-admin/src/kortravelmap/admin/app.py` —
  `etl_router` include
- `packages/kor-travel-map-admin/src/kortravelmap/admin/routers/__init__.py`
  — re-export
- `packages/kor-travel-map-admin/frontend/src/app/page.tsx` — `/etl` 링크
- `packages/kor-travel-map-admin/openapi.json` — drift gate baseline 재생성

**테스트**:
- `packages/kor-travel-map-admin/tests/test_etl_routers.py` (13 case):
...
- `python packages/kor-travel-map-admin/scripts/export_openapi.py --check`
  → exit 0

**디버그 서버 사용 흐름** (사용자가 지금 바로):
1. `pip install -e packages/kor-travel-map-admin` (PR#35 시점에 1회만 필요)
2. `uvicorn kortravelmap.admin.app:app --host 127.0.0.1 --port 8087`
3. browser → `http://127.0.0.1:8087/docs` (Swagger UI) 또는 `/debug/etl/
   providers`로 매트릭스 확인
4. 또는 frontend `cd packages/kor-travel-map-admin/frontend && npm run dev` →
...
**변경 파일** (3):
- `src/kortravelmap/providers/opinet.py`:
...
- `src/kortravelmap/providers/__init__.py` — opinet 6 신규 re-export
...
**신규 파일** (4):
- `src/kortravelmap/dto/price.py` (~140 line) — `PriceValue` DTO
...
- `src/kortravelmap/providers/opinet.py` (~170 line)
...
**변경 파일** (4):
- `src/kortravelmap/dto/_enums.py` — `PriceDomain` enum 5값 (opinet_gas_station/
  rest_area_food/rest_area_fuel/toll_fee/admission_fee)
- `src/kortravelmap/dto/__init__.py` — `PriceDomain` + `PriceValue` re-export
- `src/kortravelmap/core/ids.py` — `make_price_value_key(*, feature_id,
  provider, price_domain, product_key, observed_at)` (`pv_{sha1[:20]}`,
  PRICE_VALUE_KEY_HASH_LENGTH=20)
- `src/kortravelmap/core/__init__.py` — 2 신규 re-export
- `src/kortravelmap/providers/__init__.py` — opinet 5 신규
...
- `src/kortravelmap/providers/__init__.py` — 2 신규 re-export
...
- `pyproject.toml [providers]` extra:
  ...
  - Sprint 그룹화 + 코멘트로 Protocol 박힌 라이브러리 표시:
    - kor-travel-geo (on-demand geocoder, ReverseGeocoder Protocol)
...
**신규 파일** (3):
- `src/kortravelmap/core/weather.py` (~150 line) — pure helpers:
...
- `src/kortravelmap/core/__init__.py` — 5 신규 re-export
- `src/kortravelmap/providers/__init__.py` — kma 2 신규 (Protocol + 함수)
...
**신규 파일** (4):
- `src/kortravelmap/dto/weather.py` (~220 line) — `WeatherValue` DTO
...
- `src/kortravelmap/providers/kma.py` (~270 line):
...
**변경 파일** (6):
- `src/kortravelmap/dto/_enums.py` — `WeatherDomain` / `ForecastStyle` /
  `TimelineBucket` 3 enum 추가 (총 26 값)
- `src/kortravelmap/dto/__init__.py` — re-export 4 신규
- `src/kortravelmap/core/ids.py` — `make_weather_value_key` + `WEATHER_VALUE_
  KEY_HASH_LENGTH=20` 추가. `wv_{sha1[:20]}` 포맷. timeline_bucket 제외.
- `src/kortravelmap/core/__init__.py` — 2 신규 re-export
- `src/kortravelmap/providers/__init__.py` — kma 5 신규
...
**신규 파일** (3):
- `src/kortravelmap/core/address.py` (~280 line) — kraddr-base 흡수 utility:
...
**변경 파일** (6):
- `src/kortravelmap/dto/address.py` — `Address` 모델 풍부화:
  ...
  - kraddr-base의 `LegalAddress` / `RoadAddress` / `AddressRegion`을 한
    모델로 통합 (분리 모델 안 만듦)
- `src/kortravelmap/core/__init__.py` — 8 신규 식별자 re-export
- `src/kortravelmap/providers/standard_data.py` — `_item_to_bundle`에서 utility
  적극 활용:
...
- `src/kortravelmap/infra/models.py` — comment "kraddr.base.Address" →
  "kortravelmap.dto.Address (ADR-041)"
...
**Verification (local)**:
- `pytest tests/ packages/kor-travel-map-admin/tests/ --ignore=tests/
...
- `core/address.py` 모듈 docstring + `dto/address.py` docstring 두 곳에서
  명시. 좌표 DTO는 `kortravelmap.dto.coordinate.Coordinate` 단일 source.
...
- `next.config.ts` (TS로 마이그레이션, `next.config.js` 삭제) — `transpile
  Packages: ["@kor-travel-map/map-marker-react"]` + `productionBrowserSourceMaps:
  false` + `poweredByHeader: false`
- `tsconfig.json` — Next.js 15 권장 + paths `"@/*": ["./src/*"]`
- `src/api/client.ts` — fetch wrapper, `BASE_URL` (env `NEXT_PUBLIC_KOR_TRAVEL_MAP
...
**변경 파일** (5):
- `packages/kor-travel-map-admin/frontend/package.json` — `_comment_dependencies`
...
- `packages/kor-travel-map-admin/frontend/.env.example` — default backend port
  `8600` → `8087` (PR#35 AdminSettings.port 기본과 정합)
- `packages/kor-travel-map-admin/frontend/README.md` — Sprint 1 skeleton note →
  PR#36 진입 status로 정정 + ADR-043 명시
- `docs/sprints/SPRINT-2.md` §2.5 frontend block — PR#36 merged + 다음 단계
  매핑

**삭제 파일** (1):
- `packages/kor-travel-map-admin/frontend/next.config.js` — `next.config.ts`로
  대체
...
**컨텍스트**: `packages/kor-travel-map-admin/`는 Sprint 1까지 `pyproject.toml`
...
**신규 파일** (7):
- `packages/kor-travel-map-admin/src/kortravelmap/admin/__init__.py` —
  namespace package + `__version__ = "0.2.0-dev"`.
- `py.typed` — PEP 561 marker.
- `settings.py` — `AdminSettings` (pydantic-settings, host/port/log_level/
  debug_routes_enabled, `KOR_TRAVEL_MAP_ADMIN_*` env prefix).
...
- `tests/test_routers.py` — pytest + httpx ASGI 테스트 6건 (200 OK / schema
  ...
  매칭).
- `packages/kor-travel-map-admin/openapi.json` — drift gate baseline (FastAPI
  `app.openapi()` 직접 결과, sort_keys + indent=2).

**변경 파일** (4):
- `pyproject.toml` `tool.mypy.mypy_path` — `"src"` → `"src:packages/kor-travel-map-
...
- `info.title = "kor-travel-map-admin"`, `info.version = "0.2.0-dev"`,
...
**신규 파일** (2):
- `src/kortravelmap/providers/standard_data.py` (~340 line)
...
**변경 파일** (3):
- `src/kortravelmap/providers/__init__.py` — `standard_data` re-export (`__all__`
  4 식별자 + 4 상수).
...
- `mypy --strict src/kortravelmap` → Success: no issues found in 32 source files.
- `lint-imports` → 4 contracts kept, 0 broken.
...
- ADR 035+ 진행, npm 게시 보류 (`@kor-travel-map/map-marker-react`), 나머지 수용
...
- ADR-043 `@kor-travel-map/map-marker-react` npm 게시 보류 (ADR-029 supersede)
...
→ `~/dev/geo-antigravity`. `geo-*` 접두사는 형제 저장소 (`kor-travel-geo`
...
- `codegraph init -i` (F:\dev\kor-travel-map) → `.codegraph/codegraph.db`
  SQLite (WAL 모드, 1.31 MB) 생성. 64 파일 / 719 노드 / 1,205 edge.
- `codegraph status` → "Index is up to date".
- `codegraph query make_feature_id` → `src/kortravelmap/core/ids.py:73` 정확
...
**신규 파일** (4):
- `src/kortravelmap/core/providers.py` (~120 line) — `CANONICAL_PROVIDER_NAMES`
...
- `src/kortravelmap/core/scoring.py` (~270 line) — ADR-016:
...
- `src/kortravelmap/core/__init__.py` — providers/scoring 18 신규 식별자
  re-export.
...
- `python -m mypy --strict -p kortravelmap` → Success, **31 source files**.
...
- `src/kortravelmap/providers/visitkorea/__init__.py` — 축제 변환 함수
  (`festival_to_bundle`).
- `src/kortravelmap/infra/feature_repo.py` — raw SQL `_SQL` 상수 + upsert
  (ADR-004 + ADR-013).
...
**작업**: PR#28 Sprint 2 prep — `src/kortravelmap/infra/models.py` (SQLAlchemy 2
...
**신규 파일** (8):
- `alembic.ini` — Alembic config. DSN은 env.py가 `KOR_TRAVEL_MAP_PG_DSN`에서
...
- `src/kortravelmap/infra/models.py` (~290 line) — `Base` declarative +
...
- `src/kortravelmap/infra/__init__.py` — Base/metadata/FeatureRow/SourceRecord
...
- `python -m ruff check src/ tests/ alembic/` → All checks passed.
- `python -m mypy --strict -p kortravelmap` → Success, **29 source files**
  (infra/models.py 신규).
- `import-linter` → **4 contracts kept, 0 broken**.
...
- §11 에러 변환 — `KorTravelMapError` 베이스 (PR#20)로 통합.
...
- `.venv/bin/python -m mypy --strict -p kortravelmap` → Success, 28 source files.
...
- `src/kortravelmap/infra/models.py` — SQLAlchemy 2 declarative + GeoAlchemy2
...
**신규 파일** (4):
- `src/kortravelmap/dto/source.py` (~150 line):
...
- `src/kortravelmap/dto/bundle.py` (~80 line):
...
**주요 변경 파일**:
- `src/kortravelmap/core/ids.py` — `make_source_record_key` (`sr_{sha1[:20]}`,
...
- `src/kortravelmap/core/__init__.py` — re-export 신규 helper + length constant
  (12 → 15 식별자).
- `src/kortravelmap/dto/__init__.py` — `SourceRecord` / `SourceLink` /
...
**작업**: PR#25에 WSL ext4 base 정책 문서 명시 sweep. `kor-travel-geo`
패턴 미러.

**컨텍스트**: 사용자가 "kor-travel-geo와 똑같이 wsl이 베이스임을 문서에
명시하고 PR25에도 코멘트와 함께 md 파일 업데이트 반영" 지시. kor-travel-geo 측
...
- `README.md` — `## 💻 개발 환경 (PC, WSL)` 신규 섹션 (책임 비책임 다음).
  ...
  reference. kor-travel-geo `README.md:21-33`과 1:1 미러.
...
- `AGENTS.md` §"개발 환경 정책 (PC, WSL)" — 첫 단락 끝에 "형제 라이브러리
  (kor-travel-geo/kraddr-base/knps-api 등)와 동일 정책" 명시.
...
- README/CLAUDE/AGENTS/SKILL/dev-environment 5개 파일이 WSL 정책을 동일하게
  "박는" 형태 — kor-travel-geo와 동일 cardinality.
...
- `pyproject.toml` providers extras — knps git URL 주석 갱신 (`@06da125f` commit
  pin + keyless 비고).
- `src/kortravelmap/dto/{area,route}.py` — `protected_area` area_kind,
...
- 로컬: `.venv/bin/python -m ruff check src/ tests/`, `git diff --check`,
  `compileall src/kortravelmap`, DTO smoke 통과.
...
**변경 파일** (6):
- `src/kortravelmap/dto/_time.py` — `check_aware_datetime(value)` 공용 helper
  추가. docstring에 ADR-019 정책 (aware = any tz, naive = reject) 명시.
- `src/kortravelmap/dto/feature.py`:
...
- `src/kortravelmap/dto/notice.py` — `valid_start_time/valid_end_time` aware
  validator 추가.
- `src/kortravelmap/dto/urls.py` — `RawDataRef.fetched_at` aware validator 추가.
- `src/kortravelmap/dto/__init__.py` — `KST`/`kst_now`/`check_aware_datetime`
  공개 API 추가.
...
- `python -m mypy --strict -p kortravelmap` → Success, 26 source files
- import-linter → 4 contracts kept, 0 broken
...
- `src/kortravelmap/dto/_time.py` — `KST` / `kst_now()` 정의 (이전 `core/
...
- `pyproject.toml`:
  ...
  - `layers` 계약에서 `kortravelmap.cli` 제거 (모듈 미존재 — Sprint 4~5
    추가 시 다시 박음)
- `src/kortravelmap/core/types.py` — KST/kst_now 정의 → `dto/_time` re-export
  shim. 공개 API (`from kortravelmap.core import kst_now`)는 그대로.
- `src/kortravelmap/dto/feature.py` — `from ..core import kst_now` →
  `from ._time import kst_now` (의존 방향 보존)
- `src/kortravelmap/providers/__init__.py` — docstring 표 줄바꿈 (E501 해소)
...
- 해소: 정의를 `dto/_time.py`로 이전 (dto 레이어 내부). `core/types.py`는
  re-export shim — 호출 측 (`from kortravelmap.core import kst_now`) 코드
  변경 0.
...
- ADR-031 — `openapi.yml` workflow + `export_openapi.py --check` drift
  ...
  에서 활성화.
- ADR-032 — ci.yml에서 `--cov=src/kortravelmap`, coverage XML upload.
...
**작업**: Sprint 1 PR#21 — `src/kortravelmap/infra/` skeleton: `crs.py`
...
**신규 파일** (6):
- `src/kortravelmap/infra/crs.py` (~140 line):
...
- `src/kortravelmap/infra/db.py` (~150 line):
  ...
  - `SecretStr` 입력 자동 처리 (KorTravelMapSettings.pg_dsn 직접 주입 가능)
...
**변경 파일** (3):
- `src/kortravelmap/infra/__init__.py` — 9 식별자 re-export (crs 6 + db 3),
...
- `python -m ruff check src/kortravelmap/infra/ tests/unit/test_crs.py
  tests/unit/test_db.py tests/integration/` → All checks passed.
- `python -m mypy --strict -p kortravelmap.infra` → Success, no issues
...
**작업**: Sprint 1 PR#20 — `src/kortravelmap/core/` 예외 계층 + ADR-009
...
**신규 파일** (4):
- `src/kortravelmap/core/exceptions.py` (~110 line) — `KorTravelMapError` 베이스 +
...
- `src/kortravelmap/core/ids.py` (~130 line) — ADR-009 결정적 ID 생성:
...
**변경 파일** (1):
- `src/kortravelmap/core/__init__.py` — PR#19에서 추가된 `KST`/`kst_now`와
...
- `python -m ruff check src/kortravelmap/core/ tests/unit/test_exceptions.py tests/unit/test_ids.py`
  → all checks passed.
- `python -m mypy --strict -p kortravelmap.core` → Success.
...
**다음**: PR#20 사용자 review/merge → PR#21 (`src/kortravelmap/infra/` skeleton
...
**작업**: Sprint 1 PR#19 — `src/kortravelmap/dto/` Feature + 5 detail kind
...
**신규 파일** (13):
- `src/kortravelmap/core/types.py` — `KST` / `kst_now()` (ADR-019)
- `src/kortravelmap/dto/_enums.py` — `FeatureKind` 7종 / `FeatureStatus` 6종
  / `SourceRole` 8종 (StrEnum)
- `src/kortravelmap/dto/coordinate.py` — `Coordinate` (Korea bounds validator
  [124, 132] × [33, 39.5], frozen)
- `src/kortravelmap/dto/address.py` — `Address` (basic, kraddr-base 통합은
  Sprint 2)
- `src/kortravelmap/dto/urls.py` — `FeatureUrls` + `RawDataRef`
- `src/kortravelmap/dto/opening_hours.py` — `OpeningTime`/`OpeningPeriod`/
  `SpecialOpeningDay`/`FeatureOpeningHours` (Google Places 호환)
- `src/kortravelmap/dto/place.py` — `PlaceDetail`
- `src/kortravelmap/dto/event.py` — `EventDetail` (날짜 순서 validator)
- `src/kortravelmap/dto/notice.py` — `NoticeDetail` + **NOTICE_TYPES 14건**
  (ADR-027 `access_restriction`/`fire_alert` 포함) + `normalize_notice_type`
  + 한/영 alias map (입산통제/해수욕장폐장/산불경보 등)
- `src/kortravelmap/dto/route.py` — `RouteDetail` + ROUTE_TYPES 9종 +
  `normalize_route_type` (lenient unknown → 'route' fallback)
- `src/kortravelmap/dto/area.py` — `AreaDetail` + AREA_KINDS 12종 (ADR-027
  **hazard_zone** 포함)
- `src/kortravelmap/dto/feature.py` — `Feature` 본체:
...
**변경 파일** (2):
- `src/kortravelmap/dto/__init__.py` — placeholder → 38 공개 식별자
  re-export
- `src/kortravelmap/core/__init__.py` — `KST`/`kst_now` re-export
...
**다음**: PR#19 review/merge → PR#20 `src/kortravelmap/core/` 본격 구현
...
**작업**: Sprint 1 PR#18 — `src/kortravelmap/category/` 144건 코드 이전
...
**신규 파일** (2):
- `src/kortravelmap/category/_definitions.py` (~2110줄) — kraddr-base 사본 +
  ADR-027 패치:
...
**변경 파일** (2):
- `src/kortravelmap/category/__init__.py` — `_definitions`에서 14 공개
  식별자 re-export.
- `docs/category.md`:
...
**다음**: PR#18 사용자 review/merge → PR#19 (`src/kortravelmap/dto/` —
...
**작업**: Sprint 1 PR#17 — `src/kortravelmap/` PEP 420 scaffolding. **첫 실제
Python 코드 commit**.
...
**신규 파일** (13):
- `src/kortravelmap/__init__.py` — `__version__ = "0.2.0-dev"` + 공개 API
  주석 + ADR 참조 (002/003/020/022/030/034).
- `src/kortravelmap/py.typed` — PEP 561 marker (빈 파일).
- `src/kortravelmap/settings.py` — `KorTravelMapSettings(BaseSettings)`:
  ...
  - env prefix `KOR_TRAVEL_MAP_`, `.env` 로딩, `extra="ignore"`
...
- `tests/lint/test_no_namespace_init.py` (3 케이스):
  ...
  - `src/kortravelmap/__init__.py`는 존재
  - `src/kortravelmap/py.typed`는 존재
- `tests/unit/test_smoke_import.py` (5 케이스):
  - `import kortravelmap` + `__version__` 노출
  - 6 layer subpackage 모두 import 가능
  - `KorTravelMapSettings()` 기본값 적용
  - `KOR_TRAVEL_MAP_*` 환경변수 우선
...
- `python -c "import kortravelmap; print(kortravelmap.__version__)"` →
  `0.2.0-dev`
- `KorTravelMapSettings()` 인스턴스 생성 + `pg_dsn` SecretStr 마스킹 확인
...
- ADR-029 (`@kor-travel-map/map-marker-react` npm 패키지): accepted
...
**비목표 (본 PR#16)**: 실제 `src/kortravelmap/` 코드 작성 / testcontainers
...
"from kortravelmap import ... 사용 금지 — 항상 from kortravelmap import ...").
...
- `CLAUDE.md §5 #2`: "`from kortravelmap import ...` 사용 금지 — 항상
  `from kortravelmap import ...`" → "`from kor_travel_map import ...` (flat) 사용
  금지 — 항상 `from kortravelmap import ...`".
- `AGENTS.md §"DO NOT" #18`: 동일 패턴 + "src/kortravelmap/ 디렉토리 만들지
  말 것 — src/kortravelmap/" → "src/kor_travel_map/ 디렉토리 만들지 말 것 —
  src/kortravelmap/".
- `SKILL.md §4 #20`: 동일 패턴.
...
- 본 라이브러리 `kortravelmap.providers.knps`에서 파싱 권고 — provider
...
"React + Vite"로 박았던 것이 잠정 가설이었고, `kor-travel-geo-ui`와 TripMate
...
- `docs/decisions.md`:
  ...
  - ADR-025 §근거 — kor-travel-geo-ui 일관 + TripMate `apps/web` 동일 stack
    명기.
  - ADR-025 §결과(긍정/부정) — Vite → Next.js로 정정.
  - **ADR-025 §사용자 보강 (2026-05-25, 2차) — 빌드 도구 정정** 신설:
    `next dev --port 8610`, App Router, `NEXT_PUBLIC_*` env, `@kor-travel-map/
...
- `docs/debug-ui-package.md` §14 전체 갱신:
  - §14.1 기술 스택 — Framework Next.js 15 (App Router) 추가, 빌드 도구
    Vite 행 삭제, 공통 마커 `@kor-travel-map/map-marker-react` (ADR-029) 추가.
  ...
  - §2 디렉토리 트리 — `vite.config.ts`/`index.html`/`src/main.tsx`/`pages/`
    ...
    markerColor는 `@kor-travel-map/map-marker-react`로 이전 명기.
...
- `docs/tripmate-integration.md` §14.5:
  ...
  - `@kor-travel-map/map-marker-react` 사용 명기.
  - 작업 분담에 "Next.js 그대로 유지, 마커 import만 교체" 명기.
- `packages/kor-travel-map-admin/README.md`:
...
- `packages/kor-travel-map-admin/frontend/package.json` — 전체 교체:
  - `vite`/`@vitejs/plugin-react`/`vitest` → `next`/`eslint-config-next`/
    `@types/node`.
  - scripts: `vite` → `next dev/build/start/lint`.
  - dependencies: `next`/`@kor-travel-map/map-marker-react` (workspace) 추가.
- `packages/kor-travel-map-admin/frontend/.env.example`:
  - `VITE_*` → `NEXT_PUBLIC_*`.
  - 주석에 Next.js env 규약 (NEXT_PUBLIC_ vs server-only) 명기.
- `packages/kor-travel-map-admin/frontend/.gitignore`:
  - `dist/` 삭제, `.next/`/`out/`/`next-env.d.ts` 추가, `.vite/` 삭제.
- `packages/kor-travel-map-admin/frontend/README.md`:
  - 기술 스택 표 Next.js 행 추가, Vite 삭제, env `NEXT_PUBLIC_*`.
  - 개발 명령 `next dev --port 8610`.
  - 빌드 / 운영 옵션 3가지 추가.
  - 페이지 표를 App Router route (`/features/[id]` 등)로 변경.
  - categoryMaki 매핑은 `@kor-travel-map/map-marker-react` 사용 (ADR-029) 명기.
- `packages/kor-travel-map-admin/frontend/next.config.js` 신설:
  - reactStrictMode + transpilePackages (`@kor-travel-map/map-marker-react`)
  - 운영 옵션(`output: 'standalone'/'export'`, `basePath`, `rewrites`)은
    주석 처리 — 운영자 결정 후 활성화.
- `docs/tasks.md` — §폐기/재해석 — T-100 "Next.js 미채택" 기록은 잘못됨
  명기, ADR-025 2차 보강으로 채택 확정.

**핵심 인사이트**: kor-travel-geo-ui = Next.js이고 TripMate `apps/web` = Next.js
이므로 디버그 UI도 Next.js가 자연. 1차에서 Vite로 박았던 것은 SPA의 단순함
가정에서 비롯됐으나, 운영 일관성 (학습 곡선 통일 + `@kor-travel-map/map-marker-react`
...
**결정 / 신규 ADR**:
- **ADR-029 (proposed)**: `@kor-travel-map/map-marker-react` npm 패키지 추출. MIT
...
- `packages/kor-travel-map-admin/scripts/export_openapi.py` 신설 — ADR-031
...
- `docs/decisions.md`:
  ...
  - **ADR-026 신규**: TripMate 사용자 UI도 `maplibre-vworld` 채택 (SPEC V8
    ...
    npm 패키지 추출은 후속 ADR.
- `docs/external-apis.md`:
  - 환경변수 카탈로그에 `KOR_TRAVEL_GEO_VWORLD_API_KEY` 항목 추가 (공유 키 명기).
...
- `packages/kor-travel-map-admin/frontend/.env.example`:
  - VITE_VWORLD_API_KEY 주석 — "= $KOR_TRAVEL_GEO_VWORLD_API_KEY 값과 동일" 박음.
- `packages/kor-travel-map-admin/frontend/README.md`:
...
- `docs/debug-ui-package.md` §2 디렉토리 + §14 신설 (~120 lines):
  ...
  - §14.1 기술 스택 (maplibre-vworld v1.0.0, ISC license, React 19, Vite,
    포트 8610)
  - §14.2 환경변수 (`VITE_VWORLD_API_KEY`, `VITE_KOR_TRAVEL_MAP_ADMIN_API`)
  ...
  - §14.6 OpenAPI → TypeScript 동기 (kor-travel-geo ADR-015 미러)
  - §14.7~14.8 e2e + 외부 노출 안전
- `packages/kor-travel-map-admin/README.md` — Frontend 절 추가 + env 표 분리
  (Backend / Frontend)
- **NEW**: `packages/kor-travel-map-admin/frontend/`:
...
- **ADR-025** — 디버그 UI frontend는 `maplibre-vworld-js` 채택.
  ...
  - 선언형 React → 상태 동기 단순.
  - `kor-travel-geo-ui`와 동일 stack (React + Vite + TS) → 운영 일관.
  ...
  - VWorld API key는 `kor-travel-geo`의 `KOR_TRAVEL_GEO_VWORLD_API_KEY` 공유
    또는 별도 `VITE_VWORLD_API_KEY`.

**의사결정 (사용자 위임 — 검토 부탁)**:
- VWorld API key 발급 정책: `kor-travel-geo`와 공유 vs 디버그 UI 전용 별도
  발급 (운영자 결정).
...
- 라이선스 호환성: ISC + BSD-3 + GPL-3.0 모두 호환 (GPL-3.0이 가장 strict이라
  배포 시 GPL 준수).
- `kor-travel-geo-ui` Next.js 패턴과 비교했을 때 디버그 UI는 SPA로 충분 (Vite 만
  사용, Next.js SSR 불필요).
...
- import 경로 `kortravelmap.providers.mois`, loader `kortravelmap.mois`, dataset_key
  prefix `mois_*`.
...
- `docs/feature-db-initialization.md` (T-010) — schema 부트스트랩 + Alembic +
  KorTravelMapSettings + AsyncKorTravelMapClient 생성 + healthz.
...
**결정**: 14개 docs는 v1 패턴을 v2 기준 (kortravelmap namespace, async-only, 함수
...
**작업**: PR-only 룰 추가 + namespace 재명명 (`kor_travel_map` → `kortravelmap`) +
kraddr-base category 모듈 이전 결정 + kor-travel-geo 패턴 보강.

**변경 파일**:
- `docs/decisions.md` — ADR-021 (PR-only), ADR-022 (`krtour` namespace),
  ADR-023 (category 이전) 3건 추가.
- `AGENTS.md` — 식별자 표 (Python import → `kortravelmap`, category 모듈 출처
...
- `README.md` — Python import 경로, 디렉토리(`src/kortravelmap/` + namespace
  설명), 문서 지도에 `docs/category.md` 추가.
- `pyproject.toml` — `packages.find` (`kortravelmap*` + `namespaces=true`),
  `package-data`, `import-linter` root_package + layers + forbidden 계약 갱신,
  coverage source.
- `packages/kor-travel-map-admin/pyproject.toml` + `README.md` — namespace 정합.
- 일괄 docs 갱신 (rename script): `architecture`, `backend-package`, `decisions`,
  ...
  `CHANGELOG`.
- `docs/category.md` (신규) — `kortravelmap.category` 모듈 사양서 11절.
...
- **ADR-022** `krtour` PEP 420 implicit namespace 채택 — `kor-travel-map`은
  `kortravelmap`으로 import, `kor-travel-map-admin`는 `kortravelmap.admin`로
  import. 같은 namespace를 share. `src/krtour/__init__.py` 금지.
- **ADR-023** kraddr-base의 category 모듈 (`kraddr.base.categories`, ~2072줄,
  141 enum)을 `kortravelmap.category`로 이전. 코드 이전은 코드 작성 단계 진입 시
  별도 PR. 라이선스 호환 (둘 다 GPL-3.0-or-later).

**발견**:
- kor-travel-geo ADR-015도 `kraddr` implicit namespace 채택 → 패턴 정합.
- kor-travel-geo의 batch DAG + consistency_reports 패턴(ADR-017)이 본 라이브러리의
  Sprint 5 운영에 유용 → T-200/T-201로 백로그 추가.
- 변수 이름 `kor_travel_map_client`(snake_case)는 변경 안 함 — Python 식별자 명명
  규약과 import path는 별개.
...
- `pyproject.toml` — `[api]` extra 제거 (ADR-020 §후속). `import-linter`에 두
  번째 계약 추가 (`kor_travel_map`에서 fastapi/uvicorn/starlette import 금지).
- `.env.example` — `KOR_TRAVEL_MAP_DEBUG_API_*` → `KOR_TRAVEL_MAP_ADMIN_*` 갱신 +
  주석.
- `docs/test-strategy.md` — e2e 코드 예시의 `from kortravelmap.api.app import ...`
  → `from kortravelmap.admin.app import ...`.
- `packages/kor-travel-map-admin/pyproject.toml` (신규) — 별도 패키지 pyproject.
- `packages/kor-travel-map-admin/README.md` (신규) — 패키지 README.

**결정**: **ADR-020** — 디버그 UI는 별도 Python 패키지 `kor-travel-map-admin`로
분리. monorepo 안 `packages/kor-travel-map-admin/`에 위치. 메인 라이브러리에서
FastAPI/Uvicorn 의존성 제거. ADR-005의 위치 부분(`kortravelmap.api`)은 본 ADR로
...
- `import-linter`의 `forbidden` 계약으로 메인 패키지의 FastAPI import를 CI에서
  자동 차단 가능.
- v1의 `packages/kor-travel-map-admin/` 디렉토리 패턴(monorepo Python 서브패키지)
  과 일관됨.
...
- ADR-001 ~ ADR-019 19건 박음. 핵심:
  ...
  - **ADR-007** 의존 스택 — kor-travel-geo와 동일.
...
**발견**:
- `kor-travel-map-spec.docx` (저장소 루트, 약 80쪽)는 v1 산출물 + SPEC V8 정합 +
  kor-travel-geo 디시플린 종합 reference로 유용.
...
- kor-travel-geo와 동일 스택 (PostgreSQL + PostGIS + SQLAlchemy 2 async + GeoAlchemy2
...
- Debug UI 패키지 (packages/kor-travel-map-admin)
- Extensive docs 수정
- `kor-travel-map-spec.docx` (AI 에이전트용 사양 80쪽)

**결정**: 사용자 요청 — v1 보존, main 재시작, orphan 히스토리, origin force-push.

**발견**: `~$kor-travel-map-spec.docx` Word lock 파일을 `.gitignore`에 추가.
