...
코드는 NTFS 드라이브(`F:\dev\python-krtour-map`)에서 형상관리와 컴파일이 이루어지나, PostGIS testcontainers나 Linux Native CLI가 필요한 통합/e2e 테스트 수행 시에는 소스코드를 WSL 내의 ext4 디렉토리로 복사(rsync)하여 가속 테스트를 수행합니다.
