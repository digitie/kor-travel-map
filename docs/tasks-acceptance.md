...
6. [x] **KMA 격자 재활성화의 전제조건이 적혀 있다.** (2026-09-14)
   ...
   **그리고 2026-09-14 지시로 KMA·에어코리아는 쿼터 평가 대상에서 빠졌다.** 둘 다
   꺼져 있고 큐 경계가 그 결정을 강제한다. 재활성화는 그 지시를 되돌리는 일이지 이
   조문이 판단할 것이 아니다 — 이 조문이 한 일(그 결정에 필요한 숫자를 적어 두는 것)은
   기록으로 남는다.
...
1. [ ] `docs/tasks-acceptance.md`가 읽기 한도(220KB) 아래로 내려간다.
...
전제와 근거는 `docs/etl/upstream-quota.md` §5가 정본이다 — 여기 복제하지 않는다.
**KMA·에어코리아는 평가 대상이 아니다**(2026-09-14 지시) — 꺼져 있고 큐 경계가 그
결정을 강제한다. 이 task는 **끄지 않은 provider의 총량**이다.

1. [ ] 큐 경로에 provider별 일일 예산이 있다.
2. [ ] 정책의 rate limit이 강제되거나 지워진다.
3. [ ] targeted refresh가 필요한 scope만 깨운다.
4. [ ] 큐 경로의 분자를 어디서 읽는지 적혀 있다.
