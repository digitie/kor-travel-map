...
- kor-travel-map `curated_features` 1건은 TripMate `curated_trip_plans` 1건으로
  복사된다. 하위 장소·정류점·추천 POI는 TripMate `curated_plan_pois`가 받는다.
- TripMate는 kor-travel-map DB를 직접 읽지 않는다. TripMate는 kor-travel-map REST API를 호출해
...
### 3.6 `feature.curated_tripmate_copy_snapshots`

T-223c-2부터 Dagster `curated_features` group이 TripMate copy snapshot을 물리화한다.
TripMate는 여전히 REST를 호출해 복사하며, 이 테이블은 kor-travel-map 내부 cache다.
...
| `snapshot` | `/tripmate-copy`와 같은 닫힌 JSON payload |
...
호출한다. 사용자용 TripMate public client와 `kor-travel-concierge`는 직접 write하지 않는다.
...
TripMate는 `feature_id`를 파싱하지 않는다. kor-travel-map item에 `feature_id`가 없는
...
5. Admin UI에 curated 후보 선택/해제, source rule, TripMate copy preview를 붙인다. **완료(T-223c-3)**.
6. TripMate는 kor-travel-map REST를 호출해 `curated_trip_plans` /
