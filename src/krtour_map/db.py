...
Index("ix_feature_event_details_dates", feature_event_details.c.starts_on, feature_event_details.c.ends_on)
...
feature_opening_periods = Table(
    ...
    Column("timezone", Text, nullable=False, default="Asia/Seoul"),
    Column("payload", JSON, nullable=False, default=dict),
    CheckConstraint("start_weekday >= 0 AND start_weekday <= 6", name="ck_feature_opening_periods_weekday"),
    CheckConstraint("length(start_time) = 4", name="ck_feature_opening_periods_start_time"),
    CheckConstraint("duration_minutes > 0", name="ck_feature_opening_periods_duration_positive"),
    CheckConstraint(
        "duration_minutes <= 10080",
        name="ck_feature_opening_periods_duration_week",
    ),
)

Index(
    "ix_feature_opening_periods_weekday_time",
...
def feature_to_row(feature: Feature) -> dict[str, Any]:
    ...
    return {
        ...
        "longitude": coord.lon if coord is not None else None,
        "latitude": coord.lat if coord is not None else None,
        "geom": None,
        "address": feature.address.model_dump(mode="json", exclude_none=True),
        "legal_dong_code": feature.address.legal_dong_code,
        "road_name_code": None,
        "road_address_management_no": None,
        "admin_dong_code": None,
        "sido_code": feature.address.sido_code,
        "sigungu_code": feature.address.sigungu_code,
        "urls": feature.urls.model_dump(mode="json", exclude_none=True),
        ...
        "detail": feature.detail or {},
        "raw_refs": [
            ref.model_dump(mode="json", exclude_none=True) for ref in feature.raw_refs
        ],
...
def feature_from_row(row: Mapping[str, Any]) -> Feature:
    ...
    if row.get("longitude") is not None and row.get("latitude") is not None:
        coord = {"lat": row["latitude"], "lon": row["longitude"]}
    return Feature(
        ...
        coord=coord,
        address=dict(row.get("address") or {}),
        ...
        raw_refs=[
            RawDataRef.model_validate(ref) for ref in list(row.get("raw_refs") or [])
        ],
...
def source_link_to_row(link: SourceLink) -> dict[str, Any]:
    ...
    return {
        ...
        "source_role": str(link.source_role),
...
def event_detail_to_row(detail: EventDetail) -> dict[str, Any]:
    ...
    if detail.opening_hours is not None:
        payload.setdefault(
            "opening_hours",
            detail.opening_hours.model_dump(mode="json", by_alias=True, exclude_none=True),
        )
    return {
...
def event_detail_from_row(row: Mapping[str, Any]) -> EventDetail:
    ...
    opening_hours_payload = payload.get("opening_hours")
    return EventDetail(
        feature_id=str(row["feature_id"]),
        ...
        opening_hours=(
            FeatureOpeningHours.model_validate(opening_hours_payload)
            if isinstance(opening_hours_payload, Mapping)
            else None
        ),
...
def opening_period_to_row(
    ...
    *,
    ...
    """Convert an opening period into a normalized DB row."""

    return {
        "feature_id": feature_id,
        "period_index": period_index,
        "start_weekday": period.open.day,
        "start_time": period.open.time_str,
        "duration_minutes": period.duration_minutes,
        "timezone": timezone,
        "payload": period.model_dump(mode="json", by_alias=True, exclude_none=True),
    }


def opening_period_from_row(row: Mapping[str, Any]) -> OpeningPeriod:
    """Convert a `feature_opening_periods` row into an `OpeningPeriod` DTO."""

    start = _opening_time_from_minute(
        int(row["start_weekday"]) * 24 * 60 + _hhmm_to_minutes(str(row["start_time"]))
    )
    duration = int(row["duration_minutes"])
    if duration >= 7 * 24 * 60:
        return OpeningPeriod(open=start)
    close = _opening_time_from_minute(
        int(row["start_weekday"]) * 24 * 60
        + _hhmm_to_minutes(str(row["start_time"]))
        + duration
    )
    return OpeningPeriod(open=start, close=close)


def opening_hours_to_period_rows(
    feature_id: str,
    opening_hours: FeatureOpeningHours,
) -> list[dict[str, Any]]:
    """Convert regular opening-hours periods into `feature_opening_periods` rows."""
    ...
    return [
        ...
        for index, period in enumerate(opening_hours.periods)
    ]


def special_opening_day_to_row(feature_id: str, special_day: SpecialOpeningDay) -> dict[str, Any]:
    """Convert a special opening day into a `feature_special_days` row."""
    ...
    return {
        ...
        "periods": (
            [
                period.model_dump(mode="json", by_alias=True, exclude_none=True)
                for period in special_day.periods
            ]
            if special_day.periods
            else None
        ),
        "payload": special_day.model_dump(mode="json", by_alias=True, exclude_none=True),
    }


def special_opening_day_from_row(row: Mapping[str, Any]) -> SpecialOpeningDay:
    """Convert a `feature_special_days` row into a `SpecialOpeningDay` DTO."""
    ...
    return SpecialOpeningDay(
        ...
        periods=(
            [OpeningPeriod.model_validate(period) for period in periods]
...
def _opening_time_from_minute(minute: int) -> OpeningTime:
    normalized = minute % (7 * 24 * 60)
    day, minute_of_day = divmod(normalized, 24 * 60)
    hour, minute_value = divmod(minute_of_day, 60)
    return OpeningTime(day=day, time=f"{hour:02d}{minute_value:02d}")
