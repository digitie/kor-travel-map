...
wrapper가 아니므로, TripMate가 Dagster resource로 넘긴 reverse geocoder callable을 호출하고
그 결과를 `Feature.address`에 병합한다.
...
세부 기준은 [Address geocoding and match report](address-geocoding.md)를 따른다.
...
- `notice_type`: `traffic_accident`, `roadwork`, `road_closure`, `heavy_rain_warning`,
  ...
  `coastal_isolation` 등. 한국어/영어 alias는 `normalize_notice_type()`으로 canonical 값에 맞춘다.
...
세부 ETL 주기와 타입 기준은 [Notice feature ETL](notice-feature-etl.md)을 따른다.
