...
**진행 중**: ADR-045 독립 프로그램화 후속. **즉시 다음 작업은 `T-209b-a`로,
Dagster가 `.dagster/schedules/schedules.db-*` SQLite 파일을 내부 schedule storage로
사용하는 경로를 PostgreSQL `krtour_map_dagster` 기반 storage로 전환한다.**
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 hardening
전까지 구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은
`T-209b-a` 완료 후 `T-201b-d` F8 file object orphan 정합성 또는 `T-RV-29/30`
OpenAPI/user spec + frontend generated type drift다.
...
- ~~**T-RV-23** offline upload checksum idempotency + load TOCTOU.~~
  ...
  Failure로 기록한다.
- **T-RV-27, T-RV-29~T-RV-35** — admin API 노출, user OpenAPI 누출,
  D-4 codegen 미채택, savepoint,
  sensor 멱등/retry, asset materialize/RetryPolicy.
  (리포트 §2)
...
- [~] T-201 — **`ops.feature_consistency_reports` 도입** (T-201a Phase 1 ✅ 2026-05-29: alembic 0003 + `infra/consistency.py` F1~F3 관측; T-201b Phase 2 = F4~F8 + Dagster 게이트, Sprint 5)
  ...
  - T-201b 진행: F4 dedup backlog WARN은 Sprint 4b에서 추가됨. 2026-06-05 Codex는
    ...
    WARN을 큐 저장 baseline 대비 현재 `core.scoring` 재계산 기준으로 정렬했다.
    남은 Phase 2 케이스는 F8 + dry-run report.
