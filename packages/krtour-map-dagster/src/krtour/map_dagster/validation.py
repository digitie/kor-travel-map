...
def validate_feature_bundle_address(
    ...
    좌표가 있는 feature는 kraddr-geo reverse 결과로 ``bjd_code``가 있어야 한다.
    ...
    if feature.coord is not None and address.bjd_code is None:
        issues.append(
            FeatureAddressIssue(
                ...
                severity="error",
                message="좌표가 있지만 kraddr-geo reverse 결과 법정동코드가 없음.",
    ...
    if address.sigungu_name and _compact(address.sigungu_name) not in normalized_provider:
        issues.append(
            FeatureAddressIssue(
                ...
                severity="error",
                message="provider 주소 문자열과 좌표 기준 kraddr-geo 시군구명이 다름.",
                provider_address=provider_address,
                bjd_code=address.bjd_code,
                sigungu_code=address.sigungu_code,
            )
    ...
    if address.sido_name and _compact(address.sido_name) not in normalized_provider:
        issues.append(
            FeatureAddressIssue(
                ...
                severity="warning",
                message="시군구는 맞지만 provider 주소에 kraddr-geo 시도명이 명시되지 않음.",
                provider_address=provider_address,
                bjd_code=address.bjd_code,
                sigungu_code=address.sigungu_code,
            )
