...
ADR-025에서 frontend는 Next.js + MapLibre/VWorld로 정의. state 관리 라이브러리는
미정. Sprint 2/3에서 라우터/UI 추가가 본격 시작되므로 표준 박음:
...
- 디버그 UI에서 검증한 state 패턴을 지도 컴포넌트와 공통 UI에도 그대로 이식 가능.
