...
`python-mcst-api`의 파일데이터 CSV 12 dataset을 place `Feature`로 적재하는 ETL
정본(T-220 재배선, #395). 구 KCISA OpenAPI/ODCloud 경로(T-220a~c)는 provider
재편(provider #6/#7/#9)으로 폐기됐다 — KCISA OpenAPI(`api.kcisa.kr`)는 공인
...
| 항목 | 값 |
|------|----|
| provider | `python-mcst-api` (`@ba471ee`, origin/master) |
| client | `mcst.FileDataClient` — **keyless**(다운로드 페이지 스크레이핑 → 최신 CSV) |
| dataset_key | `mcst_<slug>` 12종 — 메타표 `krtour.map.providers.mcst.MCST_FILE_DATASETS` |
| Feature.kind | place |
...
- **split_coord** (1종): `FCLTY_NM`/`FCLTY_ROAD_NM_ADDR`/`FCLTY_LA`(위도)/
  `FCLTY_LO`(경도)/`TEL_NO`.
- **korean_address** (1종): `지역`/`이름`/`사업자`/`소재지`/`구분`/`홀` 한국어
  컬럼, 좌표 없음 — `지역`+`소재지` 합성 주소 단서.

category는 전부 **기존 코드**(T-220a 결정 재사용 — 신설 불요). 신규 2종도
기존 코드로 흡수: 아동서점은 서점류와 동일 계열(01040000), 골프장은
...
`krtour.map.providers.mcst.MCST_EXCLUDED_FILE_DATASETS`에 코드로도 보존:
...
|---|---|---|
| `feature_place_mcst_culture` | `mcst_culture_records` (파일데이터 12) | `30 4 * * 2` |

구 `feature_place_mcst_libraries` asset/`mcst_library_records` resource는 제거.

dataset당 1 run 상한 `KRTOUR_MAP_MCST_MAX_ITEMS_PER_DATASET`(기본 50000 — 실측
