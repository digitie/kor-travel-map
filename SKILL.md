...
| REST API 패키지 | `kor-travel-map-api` (별도 Python 패키지, `packages/kor-travel-map-api/`, ADR-055) |
| Admin UI 패키지 | `kor-travel-map-admin` (`packages/kor-travel-map-admin/frontend/`) |
...
packages/kor-travel-map-api/        ← REST API Python 패키지 (ADR-055)
  ...
  src/kortravelmap/
    api/
      ...
      routers/   — public/admin/ops/debug 엔드포인트
      settings.py
...
7. 관련 ADR (`docs/decisions.md` 001~059 모두 accepted; 다음 후보 060)
