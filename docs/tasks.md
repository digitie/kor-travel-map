...
- [ ] T-VN-M05-ACTIVATION — baseline PostGIS digest를 Compose에 고정해 병합한 Map `e6c08e25…`·PinVi `932fb140…`의 pinset `a3f6a8f3…`은 installed-wheel project-root preflight에서 terminal로 끝나 root registry의 `launcher_preflight` evidence로 차단됐다. Docker·Compose·DB·driver ledger 전이었어도 같은 pinset·Manager source·output leaf는 재실행하지 않는다. Manager #253의 `sys.prefix` trusted-root 회귀 보정과 이 Map 기록의 새 revision, PinVi의 후속 provenance revision을 atomic `pin rotate-pair`로 새 pinset에 결박한다. terminal current의 role별 회전은 거부되고 source pair preflight도 ledger claim보다 먼저 실행된다. 두 전문 적대 재리뷰과 최신 CI를 통과한 그 새 candidate에서만 n150 isolated M04/M05 live mutating E2E와 activation attestation을 정확히 한 번 실행한다.
