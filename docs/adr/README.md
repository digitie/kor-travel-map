...
| ADR-003 | TripMate ↔ 라이브러리는 함수 호출 (REST 없음) *(superseded)* | [003-tripmate-library-function-call-no-rest.md](003-tripmate-library-function-call-no-rest.md) |
...
| ADR-012 | 공간 쿼리는 입력 좌표 1회 변환, 반경은 `coord_5179`(meter) 컬럼 | → 개발 규칙 ([SKILL.md §4](../../SKILL.md)) |
...
| ADR-019 | KST aware datetime만 허용 (`kst_now()`) | → 개발 규칙 ([SKILL.md §4](../../SKILL.md)) |
...
| ADR-021 | main에 직접 push 금지 — 모든 변경은 PR (branch + review) | → 개발 규칙 ([SKILL.md §4](../../SKILL.md)) |
...
| ADR-026 | TripMate 사용자 UI도 `maplibre-vworld` 채택 (SPEC V8 v8_3 supersede) | [026-tripmate-user-ui-maplibre-vworld.md](026-tripmate-user-ui-maplibre-vworld.md) |
...
| ADR-030 | 라이브러리 in-memory 캐시 금지 (immutable 카탈로그 예외) | → 개발 규칙 ([SKILL.md §4](../../SKILL.md)) |
...
| ADR-036 | `maplibre-vworld-js` 라이브러리 분리 + v0.1.0 — 공통 기능은 상류, TripMate 전용만 본 저장소 | [036-maplibre-vworld-js-library-split.md](036-maplibre-vworld-js-library-split.md) |
...
| ADR-039 | CLI 중복 실행 차단 — 비-동시-실행 명령은 lock으로 보호 | → 개발 규칙 ([SKILL.md §4](../../SKILL.md)) |
...
| ADR-043 | `@kor-travel-map/map-marker-react` npm 게시 보류 — 모노레포 내부 share로만 | [043-map-marker-react-npm-publish-withheld.md](043-map-marker-react-npm-publish-withheld.md) |
...
| ADR-045 | kor-travel-map은 Docker 독립 프로그램으로 운영하고 TripMate는 OpenAPI로 연동 | [045-docker-standalone-program-tripmate-openapi.md](045-docker-standalone-program-tripmate-openapi.md) |
| ADR-046 | ADR-045 이행은 호환 shim 없이 정본 방향으로 전환하고 주소는 kor-travel-geo REST v2로 통일 | [046-no-compat-shim-geo-rest-v2.md](046-no-compat-shim-geo-rest-v2.md) |
| ADR-047 | kor-travel-map 로컬 포트는 docker-manager 기준 API 12701, admin UI 12705, Dagster 12702로 고정 | [047-fixed-local-ports-docker-manager.md](047-fixed-local-ports-docker-manager.md) |
...
| ADR-049 | TripMate-agent YouTube 장소 후보는 `kor-travel-concierge-youtube` provider로 pull한다 *(superseded)* | [049-concierge-youtube-provider-pull.md](049-concierge-youtube-provider-pull.md) |
| ADR-050 | TripMate-agent feature export 계약을 보강한다 — 경로 중립화·정본 위치·노출 정책·철회 라이프사이클 | [050-concierge-export-contract-hardening.md](050-concierge-export-contract-hardening.md) |
| ADR-051 | TripMate 사용자 feature 제안의 kor-travel-map 반영은 기존 admin feature change API를 전송 구간으로 쓴다 | [051-user-suggestion-via-admin-change-api.md](051-user-suggestion-via-admin-change-api.md) |
| ADR-052 | RustFS 버킷은 당분간 공유하되 prefix 소유권을 명문화하고, 추후 전용 버킷으로 분리한다 | [052-rustfs-shared-bucket-prefix-ownership.md](052-rustfs-shared-bucket-prefix-ownership.md) |
| ADR-053 | `kor-travel-concierge` provider identity를 `kor-travel-concierge`로 clean cut하고 TripMate 직접 연동을 제거한다 | [053-concierge-provider-identity-clean-cut.md](053-concierge-provider-identity-clean-cut.md) |
...
| ADR-056 | T-108 운영 배포 자동화 — N150/Odroid 병행 multi-platform build | [056-multi-platform-build.md](056-multi-platform-build.md) |
