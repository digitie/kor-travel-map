...
본 문서는 `kor-travel-map` (메인 패키지)의 외부 표면(public API) reference다.
사용자 시나리오와 함수 시그니처 중심이다.

> **ADR-054/055**: 메인 distribution은 `kor-travel-map`, Python import
...
> 본 문서의 `kortravelmap` 예시는 T-226c/d/e 적용 전 현재 코드 기준이다.

> **ADR-045 주의**: 메인 패키지의 Python API는 kor-travel-map 독립 프로그램 내부
> API/Dagster/테스트에서 사용하는 public Python 표면이다. PinVi 운영 연동은
> 이 Python API를 직접 import하지 않고 OpenAPI로 한다. Admin/OpenAPI 계약은
> `docs/architecture/openapi-admin-contract.md`를 기준으로 한다.

REST/OpenAPI backend는 **별도 Python 패키지** `kor-travel-map-api` (ADR-055)에
있고, admin UI는 `packages/kor-travel-map-admin/frontend/`가 소유한다. 사양은
`docs/architecture/debug-ui-package.md`를 따른다. 메인 라이브러리는 FastAPI 의존이 없다.

## 1. 라이브러리 진입점

> **구현 현황 (2026-06-03)**: `AsyncKorTravelMapClient`는 구현되어 있으며,
> kor-travel-map API/Dagster 내부 구현과 테스트에서 사용하는 public Python 표면이다.
> PinVi 운영 코드는 이 client를 직접 import하지 않고 OpenAPI를 호출한다.

### 1.1 `AsyncKorTravelMapClient`

```python
from kortravelmap import AsyncKorTravelMapClient, KorTravelMapSettings
from sqlalchemy.ext.asyncio import create_async_engine

settings = KorTravelMapSettings()  # KOR_TRAVEL_MAP_* 환경변수 자동 로드
engine = create_async_engine(settings.pg_dsn.get_secret_value())

async with AsyncKorTravelMapClient(engine=engine, settings=settings) as client:
    ...
```
...
- 라이브러리는 engine을 스스로 생성하지 않는다. 호출자가 주입하고 lifecycle을
  관리한다.
- kor-travel-map API/Dagster가 필요 resource(provider client, kor-travel-geo, RustFS)를
  구성하고, client에는 DB transaction 경계만 맡긴다.
- 라이브러리 settings는 자체 환경변수 `KOR_TRAVEL_MAP_*`만 읽는다. provider API 키는
...
| `FeatureBundle` | provider 변환 결과 (Feature + Detail + SourceRecord + SourceLink + FeatureFileSource list) | dto.bundle |
...
상세 필드는 `docs/architecture/feature-model.md`.
...
이들은 `core/`에 정의되어 import-linter가 허용한다. ADR-045 이후 PinVi 운영
코드는 이 함수를 직접 import하지 않고 OpenAPI를 호출한다. 직접 import는
kor-travel-map API/Dagster 내부 구현과 테스트에서만 사용한다.
...
class KorTravelMapSettings(BaseSettings):
    ...
    # Sentry (선택; kor-travel-map 운영 환경이 주입)
...
`docs/architecture/debug-ui-package.md` 참조. 본 문서에서는 다루지 않는다.

요약:
- 별도 Python 패키지 `kor-travel-map-api` (`packages/kor-travel-map-api/`).
- admin frontend `kor-travel-map-admin` (`packages/kor-travel-map-admin/frontend/`).
- 메인 라이브러리를 import해서 `AsyncKorTravelMapClient` 함수 호출.
- 인증 없음, 내부망 전용 (ADR-005 + ADR-020).
- 엔드포인트, 응답 셰입, OpenAPI 생성은 `docs/architecture/debug-ui-package.md`.

## 3. 사용 시나리오

### 3.1 kor-travel-map Dagster asset (적재)

```python
from kortravelmap import AsyncKorTravelMapClient

@asset(group_name="features", retry_policy=...)
async def feature_event_visitkorea_festivals(ctx, visitkorea, kor_travel_map_client):
    items = list(visitkorea.search_festival(eventStartDate="20260501"))
    bundles = kor_travel_map_client.providers.visitkorea.festival_to_bundles(items)
    result = await kor_travel_map_client.load_feature_bundles(bundles)
    ctx.log.info("loaded", extra=result.as_metadata())
    return result
```

### 3.2 kor-travel-map FastAPI 라우터 (조회)
...
async def list_features_in_bounds(
    ...
    zoom: int = 12,
    kor_travel_map_client: AsyncKorTravelMapClient = Depends(...)
):
    cluster_unit = _cluster_unit_from_zoom(zoom)
    features = await kor_travel_map_client.features_in_bounds(
        min_lon=min_lon,
        min_lat=min_lat,
        max_lon=max_lon,
        max_lat=max_lat,
...
`docs/architecture/debug-ui-package.md` 참조. 호출 예시는 거기 박혀 있다.

## 4. 의존성/주입 계약

| 주입 대상 | 타입 | 책임자 |
|---------|------|--------|
| `engine` | `sqlalchemy.ext.asyncio.AsyncEngine` | kor-travel-map API/Dagster |
| `file_store` | `FileStore` Protocol (boto3 S3 호환 client 래핑) | kor-travel-map API/Dagster |
| `kor_travel_geo_client` | `KorTravelGeoRestClient` 또는 `ReverseGeocoder` (선택) | kor-travel-map API/Dagster |
| `providers` | `dict[str, AsyncProviderClient]` (선택) | kor-travel-map Dagster |
| `settings` | `KorTravelMapSettings` | 라이브러리 자체 / 호출자 override |
...
class ValidationError(KorTravelMapError): ...           # DTO Pydantic validation
...
class ProviderError(KorTravelMapError): ...             # provider 호출 실패
class FileStoreError(KorTravelMapError): ...
class ImportJobConflictError(KorTravelMapError): ...    # advisory lock 미획득
```
...
- PII 마스킹은 호출자(PinVi Sentry `before_send` hook) 책임.

## 7. 성능 보장

`docs/architecture/performance.md`에 정의된 SLA:
...
| bulk COPY (10k feature_price_values) | < 5s | psycopg.copy_* |

부하 테스트는 nightly (`tests/integration` `-m slow`).

## 8. 호환성 정책

- Public API (위 §1.2의 `AsyncKorTravelMapClient` 메서드) 변경은 minor version
  bump.
- DTO 필드 추가는 patch.
...
CLI는 `typer`. 인증 없음 (로컬 전용). 운영 schedule은 kor-travel-map 독립 Dagster에서.

## 11. 비책임 (다시 확인)

- 사용자/여행계획/POI 도메인 — PinVi
- JWT/OAuth/세션/권한 — 네트워크 계층 또는 호출자
- 이메일 발송 — PinVi (Resend)
- WebSocket 실시간 동기 — PinVi
- PinVi 사용자 UI 페이지 — PinVi
- Dagster orchestration (job 정의, scheduler, daemon) — kor-travel-map 독립 Dagster 패키지
