...
`pyproject.toml`의 `[tool.coverage.run]`에 source = `src/krtour_map`, `branch =
...
.github/workflows/test.yml
  jobs:
    unit:
      ...
      steps:
        - pip install -e ".[dev]"
        - pytest tests/unit -q --cov=src/krtour_map --cov-fail-under=80
        - ruff check .
        - mypy src/krtour_map
        - lint-imports
    integration:
      ...
      steps:
        ...
        - pytest tests/integration -q --cov=src/krtour_map --cov-append --cov-fail-under=80
    fixture_replay:
      runs-on: ubuntu-latest
      steps:
        - pip install -e ".[dev]"
