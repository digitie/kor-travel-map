# (아카이브) 300 이전 백업·복원 운영 기록

> **동결 2026-09-08.** 여기 적힌 절차는 커밋 `b2543d68`("refactor: establish application
> 300 baseline boundary")가 `docs/backup-restore.md`에서 **삭제한 원문**이다(1020줄 → 94줄).
> 그 절차가 부르는 `scripts/docker-restore.sh`·`docker-restore-swap.sh`·
> `docker-restore-verify.sh`는 현재 본문 없이 exit 2로 끝나고, retire된 HTTP URI는
> `410 RESTORE_UNSUPPORTED`를 낸다. **이 문서의 명령을 그대로 실행하지 말 것.**
>
> 현행 정본은 [`docs/backup-restore.md`](../backup-restore.md) — backup은 audit-only이고
> restore/swap은 300 baseline 정책상 지원하지 않는다(2026-08-26 소유자 결정).
>
> 그럼에도 남기는 이유는 **실측으로만 알 수 있었던 것들이 여기에 있기 때문이다.**
> §8.1 ④ "role·ACL을 다시 맞춘다 — 여기가 제일 잘 빠진다"와 §10의 드릴 함정 목록이
> 그것이다. 2026-09-08 M05-2 C·D단계가 같은 결함을 **다시** 만났고
> (`--no-owner --no-privileges`가 소유권 질문 자체를 불가능하게 하고, 복원본의 lease가
