...
이 문서는 `kor-travel-map`의 누적 ADR이다. 결정이 뒤집힐 때도 이전 기록은
...
- **Amendment (2026-06-02, ADR-045 D-1)**: ADR-045로 API가 TripMate(외부)에도
  ...
  **infra 계층**(Cloudflare Tunnel SSO + IP allowlist)이 책임지고, TripMate 서비스
  토큰은 **`X-Kor-Travel-Map-Service-Token`** 헤더로 pass-through한다(앱은 검증하지 않고
...
- **Amendment (2026-06-08, D-1 "B안" defense-in-depth)**: 운영 인증의 **1차 책임은
  ...
  - `service_token`(`KOR_TRAVEL_MAP_ADMIN_SERVICE_TOKEN`, opt-in) 설정 시 **service read
    ...
    강제하지 않는다**(operator는 proxy SSO). (`/tripmate/*` namespace는 제거됨 — kor-travel-map은
...
- **근거**: SPEC V8 §R-1, `kor-travel-geo` AGENTS.md "제공자 API 사용 원칙".
...
- **상태**: accepted
- **날짜**: 2026-05-24
- **결정자**: 사용자 + Claude (kor-travel-geo Critical 룰 11 미러)
...
- **근거**: kor-travel-geo `docs/data-model.md` "공간 쿼리 가이드" 검증.
...
- **Amendment (2026-06-01, ADR-045 D-9)**: 패키지를 `kor-travel-map-admin` →
  **`kor-travel-map-admin`** 으로 rename(Python namespace `kortravelmap_debug_ui` →
  `kortravelmap.admin`, settings env prefix `KOR_TRAVEL_MAP_DEBUG_UI_` →
  `KOR_TRAVEL_MAP_ADMIN_`, frontend `kor-travel-map-admin-frontend`, openapi.json 경로
...
- **후속**:
  ...
  - `packages/kor-travel-map-admin/pyproject.toml` 신규.
...
- **근거**:
  - `kor-travel-geo`의 실제 운영 관습과 정합 (그쪽도 PR 기반).
...
- **결정**:
  ...
  - 별도 패키지 `kor-travel-map-admin`(ADR-020)의 Python import는
    `kortravelmap.admin` (sibling under `krtour` namespace, 별도 distribution이
    같은 namespace를 공유). 디렉토리 layout: `packages/kor-travel-map-admin/src/
...
- **결정** (2차 보강 적용 — Vite → Next.js):
  ...
  - 백엔드 API 경유: 환경변수 `NEXT_PUBLIC_KOR_TRAVEL_MAP_ADMIN_API` (Next.js
...
- **사용자 보강 (2026-05-25, 2차) — 빌드 도구 정정 Vite → Next.js**:
  3. **디버그 UI frontend = Next.js (App Router)**. 1차 결정의 "React + Vite"
     ...
     - Env 규약: `NEXT_PUBLIC_*` 만 브라우저 노출. `VITE_*` 미사용. 1차 결정의
       `VITE_VWORLD_API_KEY` / `VITE_KOR_TRAVEL_MAP_ADMIN_API`는 각각
       `NEXT_PUBLIC_VWORLD_API_KEY` / `NEXT_PUBLIC_KOR_TRAVEL_MAP_ADMIN_API`
       로 정정.
...
- **컨텍스트**: `docs/performance.md §9.1`에 이미 "in-memory 캐시 두지 않는다"
  ...
  영구히 제거한다.
- **결정**:
  - **본 라이브러리(`kortravelmap`)는 in-memory 캐시를 두지 않는다**. `core/`
    / `infra/` / `providers/` / `client/` 어디에도 `cachetools` /
    `async-lru` / `aiocache` / 수동 `dict` 캐시 금지.
  - **Narrow 예외 (모듈 레벨 `functools.cache` 한정 허용)**:
    1. `kortravelmap.category` Tier 1~4 PlaceCategoryCode 카탈로그 (144건,
       릴리스 단위 immutable).
...
- **결정**:
  ...
  - `packages/kor-travel-map-admin/openapi.json`과
    `packages/kor-travel-map-admin/openapi.user.json`을 저장소에 커밋.
  - `packages/kor-travel-map-admin/scripts/export_openapi.py` 신설 (이미
    `docs/debug-ui-package.md §8`에 사양 박힘).
  - `.github/workflows/openapi.yml` — admin/user 이원 `--profile all --check` drift 게이트:
    ```yaml
    - run: python packages/kor-travel-map-admin/scripts/export_openapi.py \
             --profile all --check
    ```
...
- **후속**:
  - `packages/kor-travel-map-admin/scripts/export_openapi.py` 작성 (코드 작성
    단계).
...
- **후속**:
  ...
  - `docs/debug-ui-package.md` §14 — `categoryMaki.ts`를 `@kor-travel-map/map-marker-react`
    에서 import하도록 명기.
...
본 라이브러리는 향후 `kor-travel-map` CLI를 제공한다 (Sprint 4~5). 일부 CLI 명령은
동시 실행이 부적절:

- `ktmctl import <provider>` — 같은 provider+dataset_key를 두 워커가
  동시에 적재하면 source_record_key UNIQUE 충돌 + import_jobs 상태 불일치.
- `ktmctl dedup-merge <feature_id>` — manual merge가 동시에 두 명에 의해
  실행되면 master 선정 충돌.
- `ktmctl backup` / `restore` — ADR-040의 hot-swap 절차 도중 두 번째
  실행이 들어오면 데이터 손상.
...
- 위 부류 CLI 명령에 **PostgreSQL advisory lock** (`pg_try_advisory_lock`)을
  기반으로 한 mutex 가드 박음.
- lock key naming: `hash(f"kor-travel-map:{command}:{scope}")` (예: `import:
...
- 동시 실행을 허용해도 무방한 read-only 명령(예: `ktmctl status`,
...
- `src/kortravelmap/cli/mutex.py` 신설 (Sprint 4 진입 시) — `with mutex_lock
  (session, key)` async context manager.
- `src/kortravelmap/cli/` 첫 명령(예: `import`)부터 본 mutex 적용.
...
- `packages/kor-travel-map-admin/src/kortravelmap/admin/routers/admin_backups.py`.
...
- `docs/dagster-boundary.md`를 TripMate-owned Dagster에서 kor-travel-map-owned Dagster로
  갱신.
...
1. **구 모델 호환 shim 금지**
   ...
   - TripMate 직접 import, 공유 DB, TripMate-owned Dagster 경로를 유지하기 위한
     adapter도 만들지 않는다.
   - 문서와 코드의 정본은 `kor-travel-map-admin`, `kortravelmap.admin`,
     `KOR_TRAVEL_MAP_ADMIN_*`, OpenAPI, 독립 DB/Dagster다.
...
1. kor-travel-map standalone의 로컬/개발/compose 기본 포트는 다음으로 고정한다.
   - API(FastAPI `kor-travel-map-admin`): `12301`
...
3. `.env`의 기존 provider service key 이름은 `scripts/load-env.sh`와
   `docker-compose.yml`에서 `KOR_TRAVEL_MAP_ADMIN_*`/`NEXT_PUBLIC_*` 환경변수로 매핑한다.
   평문 키는 git에 커밋하지 않는다.
...
`KOR_TRAVEL_MAP_ADMIN_FEATURE_CHANGE_REVIEW_MODE`). 보안 스킴(P1-B)은 #314로 이미 해소.
...
- 2단 검토 유지: TripMate admin 1차 검토 → kor-travel-map `change-requests` 큐
  (`KOR_TRAVEL_MAP_ADMIN_FEATURE_CHANGE_REVIEW_MODE`에 따라 kor-travel-map 운영자 최종
  승인 또는 immediate 적용).
...
- **잔여 합의 5건 확정 (2026-06-11, T-217c — 코드 실측 기반, 소비 계약은
  ...
  1. **review_mode**: 기본 `require_review` 유지 — TripMate 1차 검수가 있어도
     ...
     `KOR_TRAVEL_MAP_ADMIN_FEATURE_CHANGE_REVIEW_MODE=immediate` 전환 가능(전역 설정).
...
| Admin import | `kortravelmap.admin` | `kortravelmap.admin` |
...
- 소비자 전파는 T-226e에서 수행한다: generated client/TripMate 문서, README quickstart,
  examples/snippets, migration guide를 갱신한다.
