# ADR-045: kor-travel-map은 Docker 독립 프로그램으로 운영하고 외부 경계는 OpenAPI
...
초기 v2 설계는 `kor-travel-map`을 외부 소비자가 같은 Python process에서 import하는
하부 라이브러리로 정의했다. 그러나 admin 기능 범위가 feature 전체 운영, provider
강제 적재, 중복/결측 검토, offline upload, Dagster 기반 업데이트 큐까지 커지면서
다음 요구가 확정됐다.

- kor-travel-map은 Docker에서 실행되는 **독자 프로그램**이어야 한다.
- DB도 공유 DB가 아니라 kor-travel-map이 소유하는 독립 PostgreSQL/PostGIS DB다.
- Dagster도 kor-travel-map 프로그램 안에 둔다.
- 외부 소비자와의 통신은 OpenAPI 기반 HTTP API로 한다(라이브러리 직접 import 금지).
- OpenAPI는 우선 admin UI를 기준으로 설계하고, 외부 연동 시 필요한 사용자/서비스
  API를 보완·확장한다.
...
2. **외부 경계**
   - 외부 소비자는 `kor-travel-map`을 직접 import하지 않고, kor-travel-map DB에도
     직접 연결하지 않는다.
   - 외부 경계는 OpenAPI(HTTP)로만 노출한다 — feature 조회/상세/업데이트 요청은
     생성된 OpenAPI client로 호출한다.
   - `feature_id`는 외부 참조 값으로 노출되며, 본 레포 밖에서의 저장/FK 처리는
     소비자 책임으로 본 레포가 규정하지 않는다.

3. **OpenAPI 우선순위**
   - 1차 OpenAPI는 admin UI가 쓰는 API를 기준으로 작성한다.
   - admin API는 `/admin/...`, `/ops/...`, `/features/...`, `/debug/...` prefix를
     사용한다.
   - 외부 사용자/서비스 연동 API는 admin API를 재사용하되, 필요한 응답 경량화,
...
5. **독립 Dagster**
   - Dagster asset/job/schedule은 kor-travel-map 프로그램 소유다.
...
6. **OpenAPI 기반 feature update 요청**
   - admin UI 또는 외부 소비자는 OpenAPI로 feature update request를 만들 수 있다.
   - 지원 scope:
...
- admin/운영 기능이 커지면 소비자 process 안에 라이브러리로 끼워 넣는 방식은
  배포·장애 격리·DB 소유권이 흐려진다.
- 독립 DB와 OpenAPI는 kor-travel-map의 데이터/운영 책임을 외부 경계와 명확히 나눈다.
...
- OpenAPI를 admin UI 기준으로 먼저 만들면 실제 운영 화면이 API 계약을 계속 검증한다.
  외부 연동 API는 이후 필요한 공개 범위에 맞춰 얇게 확장하면 된다.

### 결과 (긍정)

- kor-travel-map 배포, DB migration, provider key, Dagster schedule을 외부와 독립적으로
  운영할 수 있다.
- 외부 소비자는 HTTP/OpenAPI client만 알면 되므로 언어·프로세스·DB 경계가 명확하다.
- admin UI와 외부 소비자가 같은 OpenAPI 기반 계약을 공유하므로 schema drift를 줄인다.
...
- 기존 문서의 "함수 직접 호출/공유 DB" 표현을 ADR-045 기준으로 정리해야 한다.
...
- `docs/architecture/dagster-boundary.md`를 kor-travel-map-owned Dagster 모델로 갱신.
