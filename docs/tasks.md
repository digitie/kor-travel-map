...
- [ ] T-VN-M05-ACTIVATION — `a3f6a8f3…`·`22563762…`·`c700bd2e…`·`fa28a6e7…`에 이어 Map `73150672…`·PinVi `d8dc386d…`·Manager `c31c8448…`·pinset `5512ce12…`도 최신 CI·정확한 head 전문 적대 리뷰 두 건·trusted clean release·atomic `pin rotate-pair`·registry/public-copy gate 뒤 n150 isolated M04/M05 launcher를 정확히 한 번 실행했다. launcher는 exit 1이고 권위 있는 고정 결과는 `launcher_safe_result_unavailable`이었다. raw HTTP·컨테이너·환경 출력과 output leaf는 열지 않으며, 다섯 pinset·각 source pair·Manager source·output leaf는 어떤 이유로도 재실행하지 않는다. root registry의 exact unconditional terminal entry와 public-copy gate도 확인했다. 다음 실행은 이 terminal 기록을 포함한 **새** Map revision, 새 PinVi provenance revision, 새 Manager source를 atomic `pin rotate-pair`로 함께 결박한 fresh pinset에서만 허용된다. terminal current의 role별 회전은 거부되고 source pair preflight도 ledger claim보다 먼저 실행된다. 최신 CI와 전문 적대 리뷰 두 건 뒤에만 n150 isolated M04/M05 live mutating E2E와 activation attestation을 정확히 한 번 실행한다.
