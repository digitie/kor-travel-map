...
- [ ] T-VN-M05-ACTIVATION — baseline PostGIS digest를 Compose에 고정해 병합한 Map `e6c08e25…`는 PinVi `admin`·`full` pair에 재결박됐다. Manager PR #253 source `02cc8de…`의 trusted release 밖 root-owned runtime pin registry에서 읽기 전용 terminal seed `cbb577d3…`을 보존한 채 atomic `pin rotate-pair` 한 번으로 final pinset `a3f6a8f3…`을 만든다. terminal current의 role별 회전은 거부되고 source pair preflight도 ledger claim보다 먼저 실행된다. 이 새 candidate에서만 n150 isolated M04/M05 live mutating E2E와 activation attestation을 실행한다.
