...
1. [ ] `krheritage` 포털에서 일일 한도 확인(sweep당 ~3,950요청인데 분모가 없다).
2. [ ] `opinet` 포털 — 쓰고 있는 1,500/일이 실측이 아니고 예산 600이 그 위에 있다.
3. [ ] `krex` · `mois`/localdata 포털에서 확인.
4. [ ] 결과를 §2 "이 표에 없는 provider" 표에 출처와 함께 적는다.
5. [ ] **그다음에** 큐 예산 장치가 필요한지 판단한다.
