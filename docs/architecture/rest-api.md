...
> **범위**: kor-travel-map **전 표면**(user/PinVi + admin + ops + debug)의 **단일 계약 정본**
> (ADR-048 #9). `docs/architecture/pinvi-rest-api.md`는 PinVi **소비 매핑 view**로 수렴하고 계약
...
> 본 문서가 전 표면 계약 정본이다. `docs/architecture/pinvi-rest-api.md`는 PinVi 소비 매핑 view이며
> 계약 세부는 이 문서를 따른다.
...
| 2개 계약 doc | `rest-api.md` 단일 정본 + pinvi-rest-api.md 소비 view | 🔁 수렴(T-216g) |
...
- **정본 수렴(T-216g)**: 본 문서를 전 표면 단일 계약 정본으로, `docs/architecture/pinvi-rest-api.md`는
