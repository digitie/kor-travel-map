...
- **Provider ⑤ KNPS** 14 dataset (`providers/knps.py`) — Point/place 5건 +
  ...
  `Feature.geom` 필드 + `feature_repo` geom 적재 (ADR-028/034/012). **본 lib
  KNPS 변환은 사실상 완료** — SHP→WKT 디코딩은 **knps-api 책임**(ADR-028
  Amendment I / ADR-044, 본 lib는 record Protocol 소비). `python-knps-api`
  `06da125f`. (통계 3건은 feature 본문 X.)
