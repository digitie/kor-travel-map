...
|------------|----------------------|
| ChatGPT Codex | `kor-travel-map-codex` |
| Claude Code (본 SDK 포함) | `kor-travel-map-claude` |
| Google Antigravity 2.0 | `kor-travel-map-antigravity` |

`kor-travel-map-` 접두사는 본 저장소(`kor-travel-map`) 이름에서 따왔다 — 한 머신에서 여러 저장소의 worktree가 `F:\dev\` 아래 공존할 때 어느 저장소의 worktree인지 1:1로 식별하기 위함. (이전엔 `kor-travel-map-*`을 썼으나 통일성을 위해 `kor-travel-map-*`로 프리픽스를 일치시켰다.)

워크트리 디렉토리는 메인 repo 디렉토리의 **형제**(sibling)로 생성한다. 본 저장소 표준 위치(`F:\dev\kor-travel-map\`) 기준:

```
F:\dev\
├── kor-travel-map/            # main (메인 작업 디렉토리, 'main' 브랜치)
├── kor-travel-map-v1/         # 기존 v1 보존 worktree (참고용)
├── kor-travel-map-codex/      # ChatGPT Codex 전용
├── kor-travel-map-claude/     # Claude Code 전용
└── kor-travel-map-antigravity/ # Google Antigravity 2.0 전용
```

NTFS (`F:\dev\`) 하위에 동일 구조로 둔다. 메인 코드의 원본이 NTFS에 위치하고,
순수 Git을 제외한 실행은 WSL에서 `/mnt/f/dev/kor-travel-map-<agent>` 경로로
...
wsl
cd /mnt/f/dev/kor-travel-map-claude
uv venv --python 3.11
...
```powershell
# 0) 자기 worktree에 들어와 있다고 가정 (Claude는 F:\dev\kor-travel-map-claude/)
cd F:\dev\kor-travel-map-claude
...
uv run ruff check .
uv run mypy --strict src/kortravelmap
```
mypy --strict src/kortravelmap
lint-imports
...
codegraph impact src/kortravelmap/dto/feature.py
...
```powershell
cd F:\dev\kor-travel-map-<agent>
C:\Python314\python.exe scripts\mcp_telegram_start.py login
```

tracked MCP 설정 파일은 다음을 포함한다:

- `.codex/config.toml` — ChatGPT Codex, `cwd=F:\dev\kor-travel-map-codex`
- `claude.json` — Claude Code, `cwd=F:\dev\kor-travel-map-claude`
- `antigravity.json`, `.gemini/mcp.json` — Google Antigravity,
  `cwd=F:\dev\kor-travel-map-antigravity`
...
- **worktree 본체** (코드/`.codegraph/`): NTFS (`F:\dev\kor-travel-map-*`).
- **Git 외 실행**: WSL에서 `/mnt/f/dev/kor-travel-map-*` 경로를 직접 사용한다.
...
(`F:\dev\kor-travel-map\`)를 그대로 쓴다. `kor-travel-map-*` worktree는 **각 AI
