...
2. [x] **분자가 있다.** (2026-09-13) upstream 진입점 대부분이 요청을 세고 그 수가
   ...
   **적대 리뷰 다섯 판이 이 조문을 만들었다.** 1차: 배선은 맞는데 커버리지가
   없었다(세는 자리 셋뿐). 2차: krheritage detail이 실제의 ~1%를 냈고 큐 runner가
   계수기를 열지 않았으며 "실패 경로에도 실린다"가 거짓이었다. 3차: **2차 수정이
   게이트만 통과시켰다** — MOIS Phase A는 resource init 시점이라 계수가 항상
   no-op이었고, 초크포인트도 유일하지 않았다. 4·5차: 정정한 문장이 다른 파일에서
   되살아났고, 대체 문구 자체가 또 검증 불가였다. 서사는 `docs/journal.md`
   2026-09-13에 있다.

   **정적 검사가 (2차 blocker)를 통과시킨 것이 이 판의 교훈이다.** 그 검사는
   "계수 호출 자리가 있는가"만 본다 — 루프 밖 1회도, 도달 불가 분기도 초록이다.
   그래서 층을 나눴다: 정적 검사는 "빠진 진입점이 없는가"만 지키고(우주는 명시
   목록, 하한은 선언에 정확히), 효과는 **가짜 client로 N번 부르게 하고 계수가
   N인지 재는** 런타임 테스트가 본다.
