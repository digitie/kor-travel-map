...
> `T-213a~h`에 등록했다. 일반 좌표 기준 `/features/nearby`(T-213b)와 category
> catalog `GET /categories`(T-213f)는 **구현됨**(user spec 포함). 남은 미구현:
> provider last-sync(`T-213g`), public `/health`/`/version`(`T-213h`),
> weather card(`T-213e`)는 아직 user spec에 없다.
...
- **200**: `{ "data": { "count": int, "items": [FeatureSummary], "cluster_unit":
  ...
  `cluster_unit=null`을 반환한다. 서버 집계 구현 여부는 `T-213c`에서 결정한다.
...
### 3.6 `GET /providers/{provider}/last-sync` — 데이터 신선도 (후속 `T-213g`)
...
- **200**: `{ "data": { "provider", "dataset_key", "sync_scope",
  ...
  **404** 미존재 scope.
- **현재 상태**: `provider_sync.provider_sync_state` repo helper는 있으나 본 endpoint는
  `openapi.user.json`에 없다. `T-213g`에서 client/helper/API를 함께 고정한다.
...
- `kind=price`/`weather`는 feature 본문 대신 `GET /features/{id}/prices` /
  `/weather`(시계열) 별도 — 사용자 노출 범위는 `T-213e`에서 결정. media는
  `FeatureDetail.urls` + RustFS 공개 URL.
