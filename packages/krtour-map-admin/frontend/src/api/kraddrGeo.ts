...
const KRADDR_GEO_BASE_URL = publicUrlEnv(
  process.env.NEXT_PUBLIC_KRADDR_GEO_BASE_URL,
  "NEXT_PUBLIC_KRADDR_GEO_BASE_URL",
  "http://127.0.0.1:12201",
);

export interface KraddrGeoPoint {
  x?: number | null;
  y?: number | null;
  lon?: number | null;
  lat?: number | null;
...
export interface KraddrGeoAddress {
...
export interface KraddrGeoRegion {
...
export interface KraddrGeoCandidate {
  address?: KraddrGeoAddress | null;
  confidence?: number | null;
  distance_m?: number | null;
  match_kind?: string | null;
  point?: KraddrGeoPoint | null;
  region?: KraddrGeoRegion | null;
}

export interface KraddrGeoResponse {
  candidates: KraddrGeoCandidate[];
  status: string;
}

export interface KraddrGeoCoord {
  lat: number;
  lon: number;
}

export interface KraddrGeoCodes {
...
async function postKraddrGeo<T>(
  path: "/v2/geocode" | "/v2/reverse",
  body: Record<string, unknown>,
): Promise<T> {
  const response = await fetch(`${KRADDR_GEO_BASE_URL}${path}`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `kraddr-geo ${path} 실패 (HTTP ${response.status})${detail ? ` ${detail}` : ""}`,
    );
  }
...
export function reverseGeocode(
  coord: KraddrGeoCoord,
): Promise<KraddrGeoResponse> {
  return postKraddrGeo<KraddrGeoResponse>("/v2/reverse", {
    lon: coord.lon,
    lat: coord.lat,
    include_region: true,
...
export function geocodeAddress(
  ...
  return postKraddrGeo<KraddrGeoResponse>("/v2/geocode", {
    fallback: "api",
    ...(type === "road"
...
export function kraddrCandidateToCoord(
  candidate: KraddrGeoCandidate | null | undefined,
): KraddrGeoCoord | null {
...
export function kraddrCodesFromCandidate(
  candidate: KraddrGeoCandidate | null | undefined,
): KraddrGeoCodes {
...
export function kraddrCandidateToAddressRecord(
  candidate: KraddrGeoCandidate | null | undefined,
...
function regionName(region: KraddrGeoRegion | null | undefined): string | undefined {
  return firstText(
