...
주소 검증 finding의 `dedupe_key`는
`provider + dataset_key + source_entity_type + source_entity_id + violation_type` 전체의
`av2_<sha256>` 68-byte 값이다. `detected_at`은 최초 탐지 시각, `last_seen_at`은 최신
recurrence 시각이다. recurrence 때 실제 `feature_id`/`source_record_key`도 최신 target으로
갱신하며, 연결 대상이 삭제돼도 `SET NULL`로 ledger 행은 보존한다.
