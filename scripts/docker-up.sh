...
services=(postgres dagster-db-init db-role-bootstrap db-migrate-to-m01-bootstrap-boundary db-role-bootstrap-m01 dagster-storage-migrate api frontend dagster dagster-daemon)
...
if [[ "$external_infra" == "true" ]]; then
  ...
  services=(postgres dagster-db-init db-role-bootstrap db-migrate-to-m01-bootstrap-boundary db-role-bootstrap-m01 rustfs rustfs-init dagster-storage-migrate api frontend dagster dagster-daemon)
  ports+=("$KOR_TRAVEL_MAP_RUSTFS_API_PORT" "$KOR_TRAVEL_MAP_RUSTFS_CONSOLE_PORT")
fi
