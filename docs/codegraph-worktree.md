...
`SKILL.md` §"개발 환경 (Linux/WSL)", `CLAUDE.md` §"5. 의존 스택"이 본 문서를
참조한다.
...
NTFS (`F:\dev\`) 하위에 동일 구조로 둘 수 있다. 단, Git/codegraph를 포함한 실행은
WSL에서 `/mnt/f/dev/kor-travel-map-<agent>` 경로로 수행한다. WSL ext4 복사는 대량
I/O 성능·격리 필요 시에만 선택한다.
...
git fetch
git worktree add -b sandbox/claude /mnt/f/dev/kor-travel-map-claude main

# 3) 개발 셋업도 같은 WSL 경로에서 실행
...
```bash
# 0) 자기 worktree에 들어와 있다고 가정 (Claude는 /mnt/f/dev/kor-travel-map-claude/)
cd /mnt/f/dev/kor-travel-map-claude
...
- **Git/codegraph 실행**: WSL에서 `/mnt/f/dev/kor-travel-map-*` 경로를 직접 사용한다.
...
- `docs/dev-environment.md` — Linux/WSL 실행 정책 본문
