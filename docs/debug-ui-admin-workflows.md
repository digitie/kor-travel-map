...
| `/admin/dagster` | Dagster 운영 요약 + Dagster webserver embed. summary 성공 시 POST로 Dagster NUX seen best-effort 처리 | `/ops/dagster/summary`, `/ops/dagster/nux-seen` |
...
성공 시 `ops.offline_uploads.state='validated'`, 실패 시 `validation_failed`로 전이한다.
...
`KRTOUR_MAP_KRADDR_GEO_BASE_URL=http://127.0.0.1:9001`가 설정된 경우 admin API는
...
- JSON/JSONL은 `uploaded`, `validated`, `loaded`, `load_failed` 상태를 load 가능
  상태로 본다.
- CSV/TSV는 `validation_job_id`가 있고 상태가 `validated`, `loaded`, `load_failed`일
  때만 load 가능하다.
...
- load job이 실제 실행되면 `AsyncKrtourMapClient.run_offline_upload_load_job()`이
  `ops.import_jobs` row와 `ops.offline_uploads.state`를 전이한다. CSV/TSV load는
...
16. `/admin/dagster`에서 자체 요약 UI와 Dagster iframe embed 확인. 로컬 첫 실행 Dagster
    커뮤니티 모달은 `POST /ops/dagster/nux-seen` 처리 후 표시되지 않아야 한다.
