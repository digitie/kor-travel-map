...
|------------|----------------------|
| ChatGPT Codex | `krtour-map-codex` |
| Claude Code (본 SDK 포함) | `krtour-map-claude` |
| Google Antigravity 2.0 | `krtour-map-antigravity` |

`krtour-map-*` 접두사는 본 저장소(`python-krtour-map`) 이름에서 따왔다 — 한
머신에서 여러 저장소의 worktree가 `~/dev/` 아래 공존할 때 어느 저장소의
worktree인지 1:1로 식별하기 위함. (이전엔 `geo-*`를 썼으나 형제 저장소
`python-kraddr-geo` 등과 구분이 모호해 2026-05-28 본 저장소 전용
`krtour-map-*`로 변경.)
...
├── krtour-map-codex/                # ChatGPT Codex 전용
├── krtour-map-claude/                # Claude Code 전용
└── krtour-map-antigravity/          # Google Antigravity 2.0 전용
```
...
```bash
# 0) 자기 worktree에 들어와 있다고 가정 (Claude는 ~/dev/krtour-map-claude/)
cd ~/dev/krtour-map-claude
...
- **worktree 본체** (코드/`.venv`/`.codegraph/`): WSL ext4 (`~/dev/krtour-map-*`).
...
(`~/dev/python-krtour-map/`)를 그대로 쓴다. `krtour-map-*` worktree는 **각 AI
