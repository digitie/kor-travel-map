...
`T-RV-09`(offline upload 업로드 크기 상한/무제한 read 차단)를 닫는다.
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 노출 전까지
구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은 `T-RV-06/07/08` 중 계약/라우터
surface 정리다. T-RV HIGH 중 운영 보안/계약 항목을 먼저 닫은 뒤 ADR-045 잔여
...
- **T-RV-27** admin API `0.0.0.0` 노출(ADR-005/.env 모순) —
  **deferred**. production 레벨 외부 노출 전환 전까지 수정하지 않는다. 현재 Docker
