...
merged됐다. 이번 PR은 `T-RV-37` 잔여 hygiene을 닫는다.
provider별 public client live fetcher wiring인 `T-RV-04b`와 `T-212` 계열은 별도
에이전트 진행 범위이므로 Codex 후속 후보에서 제외한다.
...
다음 후보는 **T-209e-c admin backup/restore router + hot-swap UI**다.
...
- ~~**T-DA-13** (MED, 빠진 기능, **DA-D-04 = T-212 묶음**) `/admin/issues`
  ...
  (`ops_repo` 확장 + 통합 테스트). admin UI(승인/거절 화면)는 **T-212b**(codex) 후속.
