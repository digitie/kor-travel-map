...
- **T-RV-04** Dagster provider 서비스키 resource 미구현(D-15, feature-load asset
  ...
  - **T-RV-04b**(진행 중, provider 순차 wiring): provider public client live fetcher를
    ...
    - **나머지 6종은 설계 결정 선행 필요** — 적합성 감사
      ...
      - [ ] **opinet_stations** — **provider 차단**: bulk/지역 엔드포인트 없음(aroundAll
        5km만, ~2만 호출 비현실) → **이슈 `python-opinet-api#7`** 등록, 라이브러리 보강
        대기(또는 POI 주변 타깃 모델 전환 product 결정).
      - [x] **mois_license_records**(Phase B, 2026-06-07) — clean match(provider `PlaceRecord`이
        ...
        - [ ] **mois Phase A(소스 DB sync) — 잔여**: `LocalDataFileClient` +
          `sync_localdata_source_db(PROMOTED_SERVICE_SLUGS)`로 LOCALDATA 다운로드→소스 DB
          적재하는 Dagster op/스케줄(또는 운영자 스크립트). 네트워크+`data_go_kr_service_key`
          필요, 실데이터 검증은 T-212e.
