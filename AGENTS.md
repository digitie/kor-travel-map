...
디버그 UI/REST는 **별도 Python 패키지** `kor-travel-map-admin`로 분리되어 있다
(ADR-020/035/045). 본 monorepo 안의 `packages/kor-travel-map-admin/`에 위치하고,
메인 라이브러리를 import해서 API/Dagster 내부에서 `AsyncKorTravelMapClient`를 호출한다.
...
| 디버그 UI 패키지 | `kor-travel-map-admin` (별도 **Python** 패키지, monorepo 내 `packages/kor-travel-map-admin/`, ADR-020) |
...
| ADR accepted | 001~054 전부 (text on main). 029는 ADR-043으로 supersede. ADR-045는 Docker 독립 프로그램 + OpenAPI 연동으로 ADR-003 운영 모델을 supersede. ADR-046은 ADR-045 이행 시 구 모델 호환 shim 금지. ADR-047은 standalone 고정 포트(API 12301, admin UI 12305, Dagster 12302). ADR-048은 `/v1` 전 표면 clean cut + REST envelope/parameter/error/명명 정합성 표준. ADR-053은 `kor-travel-concierge` provider identity clean cut과 TripMate 직접 연동 제거 경계. ADR-054는 `kor-travel-map` / `kortravelmap` package identity clean cut. |
| ADR proposed | (없음) — 다음 후보 번호는 ADR-055. |
...
디버그 + admin REST는 **별도 패키지** `kor-travel-map-admin`에 둔다. 메인
라이브러리 `kor-travel-map`은 FastAPI/Uvicorn 의존이 없다.

- **위치**: `packages/kor-travel-map-admin/src/kortravelmap/admin/` (본 monorepo
  내, 별도 `pyproject.toml`).
...
- **메인 라이브러리 의존**: `pip install -e packages/kor-travel-map-admin`만
  설치하면 `kor-travel-map`을 자동으로 의존성으로 가져간다.
- **TripMate는 Python 의존하지 않는다** — TripMate는 OpenAPI client만 사용한다.
- `KOR_TRAVEL_MAP_ADMIN_HOST` 기본 `127.0.0.1`. `0.0.0.0` 바인드 시 경고 로그.
...
15. **메인 라이브러리(`kortravelmap`)에 FastAPI/Uvicorn import 금지** — ADR-020.
    HTTP 서버 코드는 `packages/kor-travel-map-admin/`에만.
...
> `kor-travel-map-admin` rename 완료(ADR-020 amendment), 구
