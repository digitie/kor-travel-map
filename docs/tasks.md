...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#209(T-RV-37d
`ops_repo._decode_cursor` broad exception 축소)까지 merged. 코드 리뷰 후속 흐름으로,
이번 PR은 `T-RV-37e`(Docker 이미지 multi-stage/non-root/standalone 위생)를 닫는다.
