...
NTFS (`F:\dev\`) 하위에 동일 구조로 둔다. 메인 코드의 원본이 NTFS에 위치하게 되며, 테스트 구동 시에만 WSL ext4로 카피한다.
...
# 4) <작업 / 코드 작성 / 테스트>
#    테스트 시에는 dev-environment.md의 카피 정책에 따라 WSL ext4로 동기화하여 pytest 실행
#    (로컬 단위 테스트는 윈도우 환경에서도 가능)
...
- **worktree 본체** (코드/`.venv`/`.codegraph/`): NTFS (`F:\dev\python-krtour-map-*`).
- **테스트 실행**: 테스트 수행 시 WSL 내의 ext4 샌드박스로 소스코드를 동기화하여 구동하며, 로컬 윈도우용 SQLite 인덱스는 `.codegraph/` 내에 그대로 남고 WSL 동기화에서 제외됩니다.
