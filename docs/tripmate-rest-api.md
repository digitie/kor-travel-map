...
> 상태: **1차 계약 구현됨 (2026-06-03, T-207e)** — `GET /features/in-bounds`,
> `GET /features/{feature_id}`, `GET /features/search`,
> `POST /tripmate/features/batch`는 OpenAPI와 동기화됐다. D-1~D-16은 전부 확정됨
...
- 신(ADR-045) — **kraddr-geo 동일방식(D-4)**:
  ...
  - **frontend(Next.js/TS)**: krtour-map `openapi.json` → `openapi-typescript`
    codegen + 수동 Zod mirror + CI diff 게이트.
...
- **D-7 분리** — 본 문서의 `/features/*`는 공개(정제 필드). 원문 source_records·
  dedup·consistency는 `/admin/features/*`(별도). 사용자 응답엔 미노출.
