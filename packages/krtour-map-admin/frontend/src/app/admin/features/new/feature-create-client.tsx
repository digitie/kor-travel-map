...
function kraddrCandidateKey(candidate: KraddrGeoCandidate): string {
  const coord = kraddrCandidateToCoord(candidate);
  return [
...
export function FeatureCreateClient() {
  ...
  const [kraddrError, setKraddrError] = useState<string | null>(null);
  const [kraddrCandidates, setKraddrCandidates] = useState<KraddrGeoCandidate[]>([]);
  const [kraddrPending, setKraddrPending] = useState(false);
  ...
  const applyCandidate = (candidate: KraddrGeoCandidate) => {
    const nextCoord = kraddrCandidateToCoord(candidate);
    const address = kraddrCandidateToAddressRecord(candidate);
    const codes = kraddrCodesFromCandidate(candidate);
    setForm((current) => ({
      ...current,
  ...
  const runReverseGeocode = async () => {
    setKraddrError(null);
    setKraddrPending(true);
    try {
      const selectedCoord = parseCoord(form);
      const response = await reverseGeocode(selectedCoord);
      setKraddrCandidates(response.candidates);
      if (response.candidates[0]) {
        applyCandidate(response.candidates[0]);
      }
    } catch (error) {
      setKraddrError(error instanceof Error ? error.message : String(error));
    } finally {
      setKraddrPending(false);
    }
  };
  ...
  return (
    ...
    >
      <form className="flex flex-col gap-4" onSubmit={submitCreate}>
        {(formError || kraddrError || createFeature.isError) && (
          <Alert variant="destructive">
            <AlertTitle>feature 작성 실패</AlertTitle>
            <AlertDescription>
              {formError ?? kraddrError ?? createFeature.error?.message}
            </AlertDescription>
          </Alert>
        )}
          ...
          <div className="flex min-w-0 flex-col gap-4">
            <section className="rounded-lg border bg-background p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                <h2 className="font-medium">kraddr-geo</h2>
                {kraddrPending ? (
                  <Badge variant="outline">조회 중</Badge>
                ) : (
                  <Badge variant="secondary">{kraddrCandidates.length}건</Badge>
                )}
              </div>
              ...
              {kraddrCandidates.length > 0 ? (
                <div className="mt-4 flex flex-col gap-2">
                  {kraddrCandidates.slice(0, 4).map((candidate) => {
                    const candidateCoord = kraddrCandidateToCoord(candidate);
                    const address = candidate.address;
                    return (
                      <button
                        className="rounded-md border px-3 py-2 text-left text-sm hover:bg-muted"
                        key={kraddrCandidateKey(candidate)}
                        type="button"
