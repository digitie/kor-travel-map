...
| `idx_source_records_lineage` | ((COALESCE(lineage_key, source_entity_id)), provider, dataset_key, source_entity_type) | notice 계보 probe (ADR-087). read와 같은 식이어야 쓰인다. 위 인덱스와 선행 열이 겹치지 않도록 계보 식이 앞 |
