...
- ``dedup-merge:{feature_id}`` — manual merge 중복 실행 차단.
...
def dedup_merge_lock_key(feature_id: str) -> str:
    """``krtour-map dedup-merge <feature_id>`` mutex 키."""
    return f"dedup-merge:{feature_id}"
