"""``kortravelmap.providers.krheritage`` — 국가유산청 → place/area/event 변환.
...
async def heritage_items_to_bundles(
    ...
    *,
    ...
    reverse_geocoder
        좌표 → ``Address`` async 역지오코더 (``kortravelmap.geocoding``). 주입하면
    ...
    address_resolver
        ...
        때 소재지 텍스트로 kor-travel-geo ``/v2/geocode``를 호출한다.
