...
### D-2 — Dagster metadata DB 레이아웃/부트스트랩 (BLOCKER, Phase 4/5)
- 질문: `krtour_map_dagster`를 같은 Postgres container 내 별도 DB로(권장, ADR-045 §4)
  둘 때 `DAGSTER_HOME`/storage 설정, 생성·init 순서(app `alembic upgrade` ↔ Dagster
  schema init)는?
- 선택지: (a) 같은 container 별도 DB(권고) (b) 별도 schema (c) 별도 instance.
- 차단: T-208a, T-209b. 반영: `dagster-boundary.md` + docker-compose.

### D-6 — feature-update 큐 실행 모델 + cardinality + lock 정책 (BLOCKER, Phase 2/4)
- 질문: admin API가 `ops.feature_update_requests` + `ops.import_jobs`를 만들고
  Dagster sensor가 RunRequest를 만든다 — request↔job↔run은 항상 1:1:1인가, 1:N
  허용인가? `run_mode=now` + 동일 scope lock 점유 시 **409 vs queued fallback**?
  sensor polling 주기는?
- 권고: request:job 1:1(단일 job이 scope 전체 처리), now는 lock 점유 시 409 +
  `retry_after`. polling 15~30s. 단 대용량 scope는 job 내부 배치.
- 차단: T-206b/d, T-208e, T-207a. 반영: `openapi-admin-contract.md §6` + plan §2.

### D-7 — `/features/{id}` admin↔사용자 응답 분리 (BLOCKER, Phase 3)
- 질문: 사용자(TripMate) 응답과 admin 응답을 같은 엔드포인트로 둘지, admin은
  `/admin/features/*`(source_records 원문·dedup history·consistency 참조 포함)로
  분리할지. 사용자에 노출할 최소 필드 집합은?
- 권고: **분리** — `/features/*`(공개, `tripmate-rest-api.md §2` 형태) + `/admin/
  features/*`(원문/이력 포함). drift gate도 이원(D-3).
- 차단: T-207e, `tripmate-rest-api.md` 확정. 반영: 두 문서 + OpenAPI.

### D-11 — `sigungu_by_radius` 시군구 경계 소스 (BLOCKER, Phase 1/2)
- 질문: 반경↔시군구 교차를 (a) 시군구 경계 polygon 테이블(`feature.sigungu_
  boundaries` 신설)로 정확히 계산할지, (b) 반경 내 feature의 `sigungu_code` distinct
  fallback로 근사할지.
- 권고: MVP는 (b) fallback(추가 테이블 없음) → 정확도 필요 시 (a) 도입(T-205b).
  경계 데이터 출처(행안부/통계청 SHP)도 함께 결정.
- 차단: T-206a(resolve_sigungu_by_radius), T-205b. 반영: plan §1/§2.

### D-14 — offline upload 파일 저장 위치 (BLOCKER, Phase 4 T-208g)
- 질문: admin offline upload 파일을 dev/운영에서 어디에? 보존 기간은?
- 선택지: (a) RustFS temp bucket (b) S3 `uploads/` prefix (c) 로컬 볼륨. 적재 후/N일
  후 삭제?
- 권고: RustFS `krtour-uploads` bucket, 적재 성공 후 30일 보존. 차단: T-208g.
