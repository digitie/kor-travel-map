...
**진행 중**: ADR-045 독립 프로그램화 후속. 현재 기준 T-RV-29/30 user OpenAPI +
frontend generated types까지 반영됐다. TripMate/user OpenAPI에서 admin write 경로는
제거됐고, feature update request 공개 경로는 `/tripmate/feature-update-requests*`가
정본이다.
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 hardening
전까지 구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은
사용자 지시에 따라 **`T-209b-a` Dagster schedule/run/event storage PostgreSQL 전환**이다.
TripMate `docs/krtour-map-requirements.md` 대조 후속은 `T-213a~h`로 별도 등록했다
(`docs/reports/tripmate-requirements-reconcile-2026-06-06.md`). 즉시 순서는 바꾸지 않고,
T-RV HIGH 잔여는 병렬 후속으로 계속 추적한다. Codex 현재 흐름은 T-209b-a를 먼저 닫고
T-201b 정합성 Phase 2 dry-run report와 T-209 Docker/daemon polish를 이어간다.
...
- [ ] **T-DA-13** (MED, 빠진 기능, **DA-D-04 = T-212 묶음**) `/admin/issues`
  ...
  apply_kraddr_geo_address/manual_override) 미구현. ADR-046 주소/좌표 이슈 수동 처리
  (write/action) 경로 부재(읽기 `GET /ops/consistency/issues`만 존재). admin UI 이슈
  승인/거절(T-212b) + write API(T-212c)와 함께 구현. **본 PR: contract §4·§4.1
  "미구현(계획)" 배지만 반영.**
...
- [ ] **T-DA-15** (MED, API 일관성, **DA-D-03 = 전면 통일**) list 응답 셰입 이원화
  (`{data,meta}` vs `{count,items,next_cursor}`) → 전면 envelope 통일. **별도 코드 PR**
  (3 flat list 라우터 + admin frontend hook + `openapi.json` 재생성). 본 PR:
  contract §3.1에 표준+현행예외 명시.
- [ ] **T-DA-16** (MED, API 일관성, **DA-D-03 = 전면 통일**) 단건 응답 envelope 불일치
  (bare object 6종 + import-jobs/{id} `{data}`만) → `{data,meta}` 통일. **별도 코드 PR**.
...
- [ ] **T-RV-41** (LOW 전제, batch_dag) `infra/batch_dag.py:454-460` — `CONCURRENTLY`
  refresh는 MV UNIQUE 인덱스 + 사전 populate 전제. 현재 MV 없어 latent. **`T-101`**
  MV 도입 task 체크리스트로 박을 것(코드 변경 불요).
...
- ~~**T-RV-23** offline upload checksum idempotency + load TOCTOU.~~
  ...
  Failure로 기록한다.
- **T-RV-27, T-RV-31~T-RV-35** — admin API bind 노출(deferred), savepoint,
  router DTO 정확성, sensor 멱등/retry, asset materialize/RetryPolicy.
  (리포트 §2)
...
- [~] T-201 — **`ops.feature_consistency_reports` 도입** (T-201a Phase 1 ✅ 2026-05-29: alembic 0003 + `infra/consistency.py` F1~F3 관측; T-201b Phase 2 = F4~F8 + Dagster 게이트, Sprint 5)
  ...
  - T-201b 진행: F4 dedup backlog WARN은 Sprint 4b에서 추가됨. 2026-06-05 Codex는
    ...
    비교로 추가했다. 남은 Phase 2 범위는 dry-run report다.
...
- [ ] **T-209b-a — Dagster schedule/run/event storage PostgreSQL 강제 전환 (즉시 실행).**
  현재 Dagster가 `DAGSTER_HOME` 아래 `.dagster/schedules/schedules.db-*` SQLite 파일을
  내부 schedule storage로 생성하는 경로가 남아 있다. Docker standalone과 로컬
  admin-stack 모두 `krtour_map_dagster` PostgreSQL DB를 Dagster instance storage의
  단일 source로 사용하게 전환한다.
  - `docker/dagster.yaml` 또는 동등한 committed Dagster instance config에서
    `schedule_storage`, `run_storage`, `event_log_storage`가 모두
    `dagster_postgres`/`KRTOUR_MAP_DAGSTER_PG_URL`을 사용하게 한다. webserver와
    daemon은 같은 config/DB를 공유해야 한다.
  - `DAGSTER_HOME`에는 workspace/log/artifact 등 파일 기반 산출물만 허용하고,
    `schedules/schedules.db`, `schedules/schedules.db-wal`,
    `schedules/schedules.db-shm` 생성은 회귀로 본다.
  - Docker compose, `scripts/run-admin-stack.sh`, runbook/env 예시가 같은
    `krtour_map_dagster` DSN과 init 순서를 쓰도록 맞춘다.
  - 검증 DoD: `dagster instance info` 또는 동등 smoke에서 schedule/run/event storage가
    PostgreSQL임을 확인, webserver/daemon 동시 기동 후 schedule state toggle이
    PostgreSQL에 유지됨을 확인, `$DAGSTER_HOME/.dagster/schedules/schedules.db-*`가
    생성되지 않음을 확인, 관련 compose/runbook 테스트를 추가한다.
...
- [~] T-209e — backup/restore 독립 DB 묶음(ADR-040 amend, D-5).
  ...
  backup 산출물과 검증 절차를 고정한다. 남은 범위는 restore 자동화, admin router,
  hot-swap UI다.
...
- [ ] **T-213b — 일반 좌표 기준 `/features/nearby` 구현.**
  사용자 현재 위치/추천 흐름을 위해 `lon`, `lat`, `radius_m`, `kind[]`, `category[]`,
  `limit`, `cursor`를 받는 endpoint + repo + `AsyncKrtourMapClient.features_nearby`
  메서드를 추가한다. 공간 술어는 입력 좌표를 CTE에서 한 번만 5179로 변환하고
  `feature.features.coord_5179`에 `ST_DWithin`/거리 정렬을 적용한다. EXPLAIN 통합
  테스트에서 GiST index 사용을 확인하고, 기존 `/features/nearby/by-target`과 cursor/
  response shape를 맞춘다.
- [ ] **T-213c — bbox clustering(`cluster_unit`) 설계/구현.**
  `GET /features/in-bounds`의 `zoom`/`cluster_unit` 계약을 실제 DB 집계로 연결할지,
  TripMate client-side 집계로 둘지 최종 결정하고 구현한다. 서버 집계 시
  시도/시군구/읍면동 또는 grid bucket 기준, 최대 행 수, marker/category rollup,
  EXPLAIN/성능 기준을 문서와 테스트에 포함한다.
- [ ] **T-213d — `AsyncKrtourMapClient` read parity 보강.**
  HTTP/repo에 이미 있는 `get_feature_rows_by_ids`와 `search_features`, by-target nearby
  조회를 client 공개 메서드(`get_features`, `search_features`,
  `features_nearby_poi_cache_target`)로 정리한다. TripMate 운영은 계속 OpenAPI만
  사용하지만, API/Dagster 내부 구현과 테스트가 같은 read path를 재사용하도록 한다.
- [ ] **T-213e — weather card/시계열 사용자 API.**
  weather value 적재 테이블/쿼리와 `build_weather_card(feature_id, asof=None)` DB 경로를
  구현하고, `GET /features/{feature_id}/weather` user/admin API 노출 범위를 확정한다.
  nowcast/ultra_short/short/mid/advisory source trace를 포함하고, KMA 변환 함수 재사용과
  freshness 정책을 테스트한다.
- [ ] **T-213f — category catalog HTTP/runtime 표면.**
  `krtour.map.category` 144건 정적 카탈로그를 `GET /categories`로 노출하고, 현재 DB
  feature count/category 활성 여부를 선택적으로 반환한다. `@krtour/map-marker-react`
  marker/icon mapping과 Python catalog drift gate를 추가한다.
- [ ] **T-213g — provider export + sync state/last-sync 표면.**
  `krtour.map.providers.__init__`에 `knps`/`krheritage` 변환 함수와 dataset/provider
  상수를 명시적으로 re-export하고, `AsyncKrtourMapClient`에 sync state read/write helper를
  추가한다. `GET /providers/{provider}/last-sync` user/admin API는
  `provider_sync.provider_sync_state`를 기준으로 구현하고 provider/dataset/scope
  필터와 404 semantics를 고정한다.
- [ ] **T-213h — public health/version/deep health.**
  현재 `/debug/health`/`/debug/version`과 별개로 TripMate liveness/readiness용
  `/health`, `/version` user spec 표면을 추가한다. DB, RustFS, Dagster, OpenAPI version,
  commit metadata 포함 범위를 정하고, 200 liveness와 dependency readiness/deep health를
  분리한다.
