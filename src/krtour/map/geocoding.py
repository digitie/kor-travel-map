...
class KraddrGeoRestClient:
    ...
    호스트는 주입한 ``httpx.AsyncClient``의 ``base_url``로 설정 (예:
    ``httpx.AsyncClient(base_url="http://kraddr-geo:8080")``), 경로 prefix는
...
def kraddr_geo_reverse_geocoder(
    ...
    *,
    ...
    --------
    >>> # import httpx
    >>> # async with httpx.AsyncClient(base_url="http://kraddr-geo:8080") as http:
