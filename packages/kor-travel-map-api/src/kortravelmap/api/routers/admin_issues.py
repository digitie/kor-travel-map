...
resolve/ignore/reopen하거나, kor-travel-geo REST v2(ADR-046)로 정/역지오코딩을
...
kor-travel-geo 호출은 ``_forward_geocode`` / ``_reverse_geocode`` 모듈-레벨 헬퍼 뒤에
...
# ── kor-travel-geo 헬퍼 (테스트 monkeypatch 지점) ─────────────────────────────────


def _kor_travel_geo_base_url() -> str:
    base_url = KorTravelMapSettings().kor_travel_geo_base_url
    if base_url is None:
        raise _KorTravelGeoUnavailable(
...
async def _dispatch_action(
    ...
    *,
    ...
    if action == "apply_kor_travel_geo_address":
        feature_id = _require_feature_id(issue)
        feature = await get_feature_address_snapshot(session, feature_id)
        if feature is None or feature.lon is None or feature.lat is None:
            raise HTTPException(
