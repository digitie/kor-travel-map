...
- [ ] **다음**: CLI mutex(SPRINT-4 §2.8) — `src/krtour/map/cli/` 신설 +
  `cli/mutex.py`(advisory lock 기반 `mutex_lock` async ctx) + import-linter layers에
  cli 추가. `krtour-map import` 등 CLI 명령 골격.
