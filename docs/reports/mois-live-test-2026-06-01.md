...
- **geocoder 보강 라이브**: kraddr-geo REST(`127.0.0.1:8888`)가 미기동이라 좌표 →
  bjd_code 보강 실연동은 미검증. 서비스 기동 후 재검증 대상.
...
Sprint 4a MOIS Step A 파이프라인(변환·적재·5179 generated·snapshot)이 **실데이터로
정상 동작**함을 확인. category/place_kind 매핑·EXCLUDED skip·좌표 변환·PostGIS 적재
모두 검증됨. 법정동코드 부재는 데이터 소스 특성이며 geocoder 보강으로 설계상 대응됨.
