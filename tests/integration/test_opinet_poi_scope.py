...
회귀(#304 리뷰): ``external_system``은 provider명이 아니라 외부 호출자(external-app 등)다.
...
_SEED_SYSTEMS = ("external-app", "kakao")
...
def _poi_target(
    ...
    radius_km: float,
    *,
    external_system: str = "external-app",
) -> PoiCacheTargetRow:
...
async def test_opinet_poi_target_bboxes_active_targets_any_external_system(
    ...
    try:
        async with AsyncSession(migrated_engine) as session, session.begin():
            # 실제 등록 target은 external_system='external-app' 등 외부 호출자다 — 포함돼야 함.
        ...
        # external-app seoul + kakao busan, 2건만.
        assert len(bboxes) == 2
