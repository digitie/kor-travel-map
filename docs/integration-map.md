...
4개 시스템(python-krtour-map · TripMate · kor-travel-concierge · tripmate-manager)의
...
| **kor-travel-concierge** | YouTube 콘텐츠 → 장소 후보 추출/검수 — feature 후보 **provider**. 현 코드/provider 이름은 `krtour-ai-agent` 계열 | API **12401** · web 9042 | kor-travel-concierge `docs/feature-export-api.md` |
...
| (보조) kor-travel-geo | geocoding REST v2 정본. 현 API/env 표기는 kraddr-geo 계열 | **12201** | ADR-046 |
...
[kor-travel-concierge :12401] ──(REST export pull)──▶ [python-krtour-map :12301]
...
- kor-travel-concierge → krtour-map: **pull 모델** — concierge는 export API만 제공, krtour Dagster가
  가져가 `FeatureBundle`로 소유(ADR-053). `operation=upsert` 적재 /
  `reject`·`tombstone` → 대응 feature `status='inactive'` 전환(ADR-050 #4, T-217b).
- TripMate ↛ kor-travel-concierge: 직접 연동 없음 — YouTube 후보는 krtour-map feature를 통해서만
