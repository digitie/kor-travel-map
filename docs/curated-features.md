...
| `tripmate_relation` | PinVi 복사 시 역할. 아래 enum 참고 |
...
`tripmate-copy` 응답은 PinVi import에 필요한 snapshot을 닫힌 형태로 제공한다. PinVi는
...
- `curated_tripmate_copy_snapshots` — PinVi copy endpoint와 같은 snapshot materialize/cache
