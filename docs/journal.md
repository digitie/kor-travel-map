...
(`krtour.map`+`krtour.map_dagster`) / lint-imports green. trails 실적재
...
RustFS S3 API는 `12101`, kraddr-geo는 API `12201`/Web UI `12205`, krtour-map은
...
- 공유 `tripmate-manager` 인프라(`5432`/`12101`/`12105`)를 사용하는
  `docker-compose.external-infra.yml` overlay와 `KRTOUR_MAP_INFRA_EXTERNAL=true`
  경로를 추가했다.
...
- **T-217e(사용자 재정의)**: RustFS를 **tripmate-manager가 일괄 관리** — 실측
  (`docker-compose.yml`: 단일 PostGIS `kraddr-geo-postgres` :5432 + `tripmate-rustfs`
  :12101/12105, Web UI 관리) 후 ADR-052 Amendment. krtour-map·tripmate-agent는 사용자,
...
- **문서 정정**: kraddr-geo REST API 로컬 포트 기준을 `http://127.0.0.1:12201`로
...
- **원인**: `.env`의 로컬 `KRTOUR_MAP_ADMIN_DAGSTER_URL=http://127.0.0.1:12302`이
...
- **수정**: Docker compose는 `KRTOUR_MAP_DOCKER_ADMIN_DAGSTER_URL`을 읽어 API 컨테이너
  내부 `KRTOUR_MAP_ADMIN_DAGSTER_URL`로 주입. 기본값은 `http://dagster:12302`.
...
- **krtour-map 보정**: `KraddrGeoRestClient.regions_within_radius`,
  ...
  추가.
- **실데이터 확인**: 로컬 kraddr-geo REST `http://127.0.0.1:12201` +
...
kraddr-geo REST(`127.0.0.1:12201`, 사용자 기동)로 검증.
...
- kraddr-geo live 12201 geocoding/debug/provider tests: 45 passed.
