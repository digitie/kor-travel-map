# 서비스 완성도·정합성 종합 검토 — kor-travel-map · TripMate · kor-travel-concierge (2026-06-10)

> **상태 (2026-06-10 갱신)**: 검토 보고서 — 의사결정 D-01~13 전 항목 종결, 결정분은
> 정본 반영 완료 (ADR-050~052, tasks T-217a~g, TripMate/kor-travel-concierge 측 문서).
> 결정 이력은 [`decisions-needed-2026-06-10.md`](decisions-needed-2026-06-10.md) 참조.
> **검토 기준 커밋**:
> - kor-travel-map `origin/main` `0e45bd7` (T-216 REST naming + TripMate-agent provider)
> - TripMate `origin/main` `4a10a5b` (#149 공유 trip 읽기전용 뷰)
> - kor-travel-concierge `origin/main` `a443ca0` (#55 PostgreSQL/PostGIS + YouTube source 정규화)
>
...
> kor-travel-concierge 측 반영 항목은 해당 repo `docs/cross-repo-consistency-actions-2026-06-10.md`.
...
[공공 API 9단계 provider]──┐
                           ▼
[kor-travel-concierge] ──(REST export: /api/v1/features/{snapshot|changes})──▶ [kor-travel-map]
...
- **kor-travel-map** = feature **owner**: 정규화·`make_feature_id`·dedup/merge·정합성 게이트·
  공간 조회. 외부 경계는 OpenAPI(:12301)뿐 (ADR-045).
- **kor-travel-concierge** = 후보 **provider**: YouTube → 장소 후보 추출·검수. DB에 FeatureBundle을
  직접 쓰지 않고 export API만 제공 (kor-travel-concierge `docs/youtube-feature-pipeline-plan.md` §2.3, krtour ADR-049).
...
| 시나리오 | TripMate 구현 | kor-travel-map 준비도 | 판정 |
|---|---|---|---|
...
| YouTube 발 장소를 계획에 추가 | ⚠️ 출처 표시 UX 없음 | ✅ provider `kor-travel-concierge-youtube` (marker P-13) | P1 갭 (아래 2.2-b) |

핵심 판정: **TripMate의 사용자 가치 사슬에서 "장소 데이터" 구간 전체가 미배선**이다.
중요한 것은 TripMate 내부 감사 문서(`docs/audit/2026-06-06-doc-impl-audit.md` DEC-01,
`docs/kor-travel-map-integration.md` L7~16)가 그 원인을 "kor-travel-map에 HTTP 표면이 없다
(debug-ui 8087뿐)"로 기술하고 있는데, **이 전제는 2026-06-10 현재 사실이 아니다** —
kor-travel-map은 :12301 `/v1` 61개 엔드포인트 + OpenAPI 기계 정본 + ServiceToken까지 완비했다.
...
- `/features/in-bounds·search·nearby·{id}·{id}/weather` 라우터를
  `apps/api/app/clients/kor_travel_map.py`(신규 httpx client)로 재배선. 구 `etl_bridge` stub
...
- YouTube 발 후보 feature가 공공데이터 feature와 같은 지도에 노출된다. 사용자에게
  ...
  신뢰도 구분이 불가능하다. kor-travel-concierge export에는 `youtube.video_url`,
  `evidence.timestamp_*`, `confidence_score`가 이미 있다
  (`docs/youtube-feature-pipeline-plan.md` §7.1).
- 구현 분담: **kor-travel-map** — feature detail(`detail`/`urls`/`raw_refs`)에 영상 근거가
  소비 가능한 형태로 노출되는지 확인·보강(KR-06). **TripMate** — 상세 카드에 출처 배지 +
  영상 링크 UI(TM-06). **kor-travel-concierge** — 추가 작업 없음(이미 공급).
...
- **`/admin/features` 편집 기능(placeholder)**: feature 쓰기는 kor-travel-map admin의 책임이며
...
## 3. Admin UX 검토 (kor-travel-map 운용 관점, 개발리더 시각)
...
| # | 항목 | 내용 | 근거 |
|---|---|---|---|
| A-1 | **tombstone/reject 라이프사이클 처리** | `kor_travel_concierge_items_to_bundles()`가 `operation != upsert`를 건너뛰기만 함 (`src/kortravelmap/providers/kor_travel_concierge.py:76,87`). kor-travel-concierge에서 검수 철회(reject)·폐기(tombstone)된 후보가 krtour feature로 **영구 잔존**. MOIS Step C(폐업→inactive)와 동형의 비활성 경로 필요 | 완결성 결함, D-03 |
...
| A-3 | **kor-travel-concierge 후보 provenance 가시성** | admin features에서 provider=`kor-travel-concierge-youtube` 필터로 조회는 가능하나, 영상 근거(원본 링크·confidence)를 보여주는 칸과 kor-travel-concierge 검수 UI로의 cross-link 부재. 운영자가 "이 후보 왜 들어왔나"를 추적하려면 두 시스템을 수동 왕복 | 운영 동선 |
...
| C-3 | 🔴 | TripMate 문서가 "krtour HTTP 표면 미존재(debug-ui 8087, `/tripmate/features/batch` 없음)"로 현실 차단 (DEC-01) — **노후 전제**. krtour는 :12301 `/v1` 완비 | krtour `docs/rest-api.md`(2026-06-10), `openapi.json` | TripMate `docs/kor-travel-map-integration.md:7-16`, `docs/audit/2026-06-06-doc-impl-audit.md` §2 | **TripMate 문서 갱신**, DEC-01 폐기 |
| C-4 | 🔴 | kor-travel-concierge export 엔드포인트 `GET /api/v1/features/{snapshot\|changes}` — krtour fetcher는 구현 완료, **kor-travel-concierge backend 미구현(T-066 대기)** | krtour `packages/kor-travel-map-dagster/.../provider_fetchers.py:63-124` (X-API-Key, `{items,has_more,next_cursor}`) | kor-travel-concierge `backend/app/api/routes.py`에 `/krtour` 라우트 없음; 계약 스펙은 `docs/youtube-feature-pipeline-plan.md` §7 (정확히 일치) | **kor-travel-concierge T-066 구현** (계약 스펙·krtour fetcher 기대치 이미 정렬됨 — 순서 의존일 뿐 충돌 아님) |
| C-5 | 🟡 | in-bounds 페이지 cap 파라미터: krtour `max_items`(≤2000) vs TripMate client가 `limit` 전송 → FastAPI가 무시, **cap 지정이 조용히 no-op** | krtour `routers/features.py:419` `max_items` | TripMate `clients/kor_travel_map.py` `features_in_bounds()` `params["limit"]` | **TripMate 수정** (`max_items`) |
| C-6 | 🟡 | TripMate 통합 문서의 경로/필드 표기 노후: `/tripmate/features/batch`(제거된 namespace), batch `items` 등 | krtour `docs/tripmate-rest-api.md` §1 "`/tripmate/*` namespace 제거됨" | TripMate `docs/integrations/kor-travel-map-rest-api.md` | **TripMate 문서 재작성** — krtour `docs/rest-api.md`+`openapi.user.json`을 정본으로 명시 |
| C-7 | 🟡 | client docstring/반환값이 `{"items": ...}`로 wrapper 내부 명명 — API 명명(`found`)과 혼선 | — | TripMate `clients/kor_travel_map.py:187,202` | TripMate 내부 정리 (C-1과 함께) |
...
| C-9 | ⚪ | 인증 방식 이원화: krtour `X-Kor-Travel-Map-Service-Token`(batch) vs kor-travel-concierge `X-API-Key` | 각 repo 정책 문서 | — | 통일 불요(시스템별 정책), 단 cross-repo 문서에 한 표로 명시 (KR-04) |
| C-10 | ⚪ | envelope 3종: krtour `{data,meta}`+RFC7807 / kor-travel-concierge export `{items,has_more,next_cursor}` 무-envelope / TripMate 자체 `Envelope` | 각 코드 | — | 통일 비권장(공용 read vs 내부 export vs 자체 표면) — 결정 D-08로 명문화 |

> **재독 보정 (2026-06-10, 2차)**: TripMate `docs/integrations/kor-travel-map-rest-api.md`
> (2026-06-08~09 갱신)를 재정독한 결과 위 표의 뉘앙스를 보정한다.
> ① **C-3 축소**: "krtour HTTP 미존재" 전제는 `docs/kor-travel-map-integration.md` 경고
...
| ID | 사안 | 현황 | 평가 |
|---|---|---|---|
| R-1 | **RustFS 버킷 소유권** | kor-travel-concierge가 미디어 원본(영상/자막/전사/프레임)을 **kor-travel-map 소유 버킷**(`RUSTFS_BUCKET_*=kor-travel-map`, prefix `features/`)에 직접 저장 (kor-travel-concierge `config.py`) | 경계 침범 소지. krtour rustfs는 "선택" 구성요소이고 offline upload 용도인데, 타 시스템의 무기한 보존 미디어가 같은 버킷에 들어가면 백업/복원·수명주기·용량 관리 책임이 모호해짐 → **D-01** |
...
> (`/admin/feature-requests` 큐) → kor-travel-map admin 최종 반영(`/v1/admin/features*`·
...
| R-3 | **후보 철회 라이프사이클** | reject/tombstone을 krtour가 skip (§3.2 A-1) — "후보 검수 권한은 kor-travel-concierge, feature 생명주기는 krtour" 합의의 마지막 고리 누락 | krtour 측 비활성 경로 구현 필요 → **D-03** |
| R-4 | **export 계약 정본 위치** | 계약 전문이 kor-travel-concierge `docs/youtube-feature-pipeline-plan.md` §7(계획 문서)에만 존재. krtour 측은 ADR-049 + fetcher 코드 | 계획 문서는 계약 정본으로 부적합(완료 후 동결·이동됨). 정본 1곳 + 상대 repo는 링크 → **D-04** |

적절하게 배치된 것들(변경 불요): feature_id 생성·dedup·정합성=krtour / YouTube 후보
검수 UI·MCP=kor-travel-concierge / 일출일몰(KASI)·예산·협업=TripMate / geocoding —
kor-travel-concierge의 VWorld/Kakao/Naver 후보 지오코딩과 krtour 변환부의 kor-travel-geo
...
- **kor-travel-map**: 가장 건강. 2026-06-06 docs-consistency-audit 이후 drift 방지 체계
  (resume/tasks 정본화, CLAUDE.md에 가변값 배제)가 작동 중. 경미 2건 —
  CLAUDE.md에 REST 버전 거버넌스(GA 후 `/v2` N-1) 1줄 부재, architecture.md 포트 미기재.
- **TripMate**: **노후가 P0 갭의 직접 원인**. ① `docs/kor-travel-map-integration.md`와
  audit 문서의 "krtour HTTP 미존재" 전제(C-3) ② `docs/integrations/kor-travel-map-rest-api.md`의
  제거된 경로/필드(C-6) ③ 코드 docstring의 ADR-002 잔재(C-2) ④ OAuth 3종/Kakao Maps
  잔재(자체 T-143~149로 추적 중) — ①②③은 즉시 갱신 대상.
- **kor-travel-concierge**: 일관성 양호(ADR-1~26, T-001~062 정렬). 누락: TripMate 소비 계약
...
4. **kor-travel-concierge 쿼터**: YouTube 일일 10k units, source_scan 주기화(T-063 진행 중)와
...
| 축 | kor-travel-map | TripMate | kor-travel-concierge | cross-repo |
|---|---|---|---|---|
...
**한 줄 결론**: 공급자(kor-travel-map)는 준비가 끝났고, 병목은 ① TripMate의 노후 문서·구모델
잔재 청산과 배선(C-1/2/3/5/6), ② kor-travel-concierge T-066, ③ 경계 4건의 의사결정(D-01~04)이다.
