...
- **결정** (2차 보강 적용 — Vite → Next.js):
  ...
  - 백엔드 API 경유: 환경변수 `NEXT_PUBLIC_KRTOUR_MAP_DEBUG_UI_API` (Next.js
...
- **사용자 보강 (2026-05-25, 2차) — 빌드 도구 정정 Vite → Next.js**:
  3. **디버그 UI frontend = Next.js (App Router)**. 1차 결정의 "React + Vite"
     ...
     - Env 규약: `NEXT_PUBLIC_*` 만 브라우저 노출. `VITE_*` 미사용. 1차 결정의
       `VITE_VWORLD_API_KEY` / `VITE_KRTOUR_MAP_DEBUG_UI_API`는 각각
       `NEXT_PUBLIC_VWORLD_API_KEY` / `NEXT_PUBLIC_KRTOUR_MAP_DEBUG_UI_API`
       로 정정.
...
- **후속**:
  ...
  - `packages/krtour-map-debug-ui/frontend/` skeleton — Vite 가정의
...
1. **운영 단위**
   ...
   - 기존 `krtour-map-debug-ui` 패키지는 이름은 당장 유지하되, 역할은 "debug UI"를
     넘어 **krtour-map admin/API 프로그램**으로 확장한다. 후속 rename이 필요하면
     별도 ADR로 다룬다.
