...
| python-krforest-api | PR#9 merge pin | `ForestSpatialFeature`, `MountainWeather`, `WildfireRiskForecast`, `LandslideForecastIssue` | PR#9 | C05A nested SHP route + C05B typed observed weather + C05C V2 fire index + C05D issue lifecycle. map pin은 provider PR#9 merge SHA로 갱신 |
