...
자세한 Tier 1~4 트리는 `docs/architecture/category.md` §4.
...
같은 payload_hash는 `source_records` 한 번만 저장 (`docs/architecture/data-model.md` §2
UNIQUE constraint).
...
```bash
# 디버그 패키지에서 (별도)
curl -X POST http://127.0.0.1:12701/debug/standard-data \
