"""``krtour.map.infra.db`` — SQLAlchemy 2 async engine + session factory.

라이브러리 자체는 engine instance를 만들지 않는다. 호출자(TripMate / debug-ui /
테스트)가 ``KrtourMapSettings.pg_dsn``으로 engine을 만들고 client/repo에
...
def make_async_engine(
    ...
    *,
    ...
    ----------
    dsn
        DSN. ``str`` 또는 Pydantic ``SecretStr`` (``KrtourMapSettings.pg_dsn``).
