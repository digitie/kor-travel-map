...
DROP PROCEDURE feature.create_manual_curation_item_with_feature_command(jsonb, jsonb, bigint);
...
BEGIN
    ...
    had_create := has_schema_privilege('ktm_curation_command_owner', 'feature', 'CREATE');
    IF NOT had_create THEN
        EXECUTE 'GRANT CREATE ON SCHEMA feature TO ktm_curation_command_owner';
    END IF;
    EXECUTE 'ALTER PROCEDURE feature.create_manual_curation_item_with_feature_command(IN p_feature_payload jsonb, IN p_item_payload jsonb, IN p_domain_command_id bigint, OUT o_outcome text, OUT o_feature_id uuid, OUT o_feature_row_revision bigint, OUT o_curation_item_id uuid, OUT o_item_row_revision bigint, OUT o_collection_row_revision bigint, OUT o_existing_feature_id uuid) OWNER TO ktm_curation_command_owner';
    IF NOT had_create THEN
        EXECUTE 'REVOKE CREATE ON SCHEMA feature FROM ktm_curation_command_owner';
    END IF;
END
$t39_owner$;

-- head-schema.sql:24966-24967을 새 시그니처로 복원한다.
