...
- **`forecast_style` + `timeline_bucket` 두 축 분리** (구 ADR-010에서 결정): provider별
  weather는 원천값 성격(nowcast/observed/index/advisory)과 KMA식 조회 축(ultra_short/
  short/mid)이 직교하므로 한 컬럼에 합치지 않고 두 컬럼으로 분리한다. `timeline_bucket`은
  분류 결과(재계산 가능)이므로 unique key에서 제외한다. 새 provider 추가 시 두 축 매핑을
  §3 표에 정의한다. 정의·표·규칙은 §1~§3 본문 참조.
