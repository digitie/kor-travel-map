...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#211(T-209e-a standalone
cold backup script/runbook)까지 merged. 코드 리뷰 후속 흐름으로,
이번 PR은 `T-RV-20` feature update request router schema 검증을 닫는다.
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 hardening
전까지 구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은 `T-RV-19` admin UI
지도 선행 안정화 또는 `T-RV-22/23/25` offline upload 후속이다.
