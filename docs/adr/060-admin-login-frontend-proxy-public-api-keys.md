...
결정 6의 Map public REST `key` query parameter는 폐기한다.
`RoutePolicy.PUBLIC_KEYED` operation은 `X-Kor-Travel-Map-Api-Key` header 또는
`X-Kor-Travel-Map-Service-Token`만 받으며 URL query 호환 경로는 두지 않는다.
OpenAPI `PublicApiKey` security scheme도 같은 header 이름과 `in: header`를 사용한다.
이 clean-cut은 access log·browser history·Referer에 Map credential이 남는 경로를 제거한다.
