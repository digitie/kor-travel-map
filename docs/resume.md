...
- admin/dagster package는 `kortravelmap.admin`, `kortravelmap.dagster`로 정렬.
...
- `GET /admin/features/change-requests` meta에 `review_mode`를 추가해
  `KOR_TRAVEL_MAP_ADMIN_FEATURE_CHANGE_REVIEW_MODE`를 빈 큐에서도 표시한다.
...
실행은 `KOR_TRAVEL_MAP_ADMIN_BACKUP_COMMAND_ENABLED=true` opt-in과 요청별
...
`KOR_TRAVEL_MAP_ADMIN_API_CALL_LOG_ENABLED`) + error envelope 중앙화(기구현 확인).
...
backend Dagster GraphQL 대상은 `KOR_TRAVEL_MAP_ADMIN_DAGSTER_ALLOWED_HOSTS` allowlist를
...
T-RV-06을 처리한다. `packages/kor-travel-map-admin/src/kortravelmap/admin/app.py`에
...
ADR-045 T-207g로 `packages/kor-travel-map-admin/scripts/export_openapi.py`를 admin/user
profile 기반 export로 확장했다. 기본 `--profile admin`은 기존
`packages/kor-travel-map-admin/openapi.json` 전체 admin spec을 유지하고,
`--profile user`는 TripMate/user-facing API subset을
`packages/kor-travel-map-admin/openapi.user.json`으로 생성한다. `--profile all`은 두
산출물을 함께 생성/검증한다.
...
직접 호출하고, OpenAPI schema는 `packages/kor-travel-map-admin/openapi.json`에 export한다.
...
`KOR_TRAVEL_MAP_ADMIN_*`/`NEXT_PUBLIC_*` 환경변수로 매핑한다.
