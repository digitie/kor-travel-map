...
| ADR accepted | 001~053 전부 (text on main). 029는 ADR-043으로 supersede. ADR-045는 Docker 독립 프로그램 + OpenAPI 연동으로 ADR-003 운영 모델을 supersede. ADR-046은 ADR-045 이행 시 구 모델 호환 shim 금지. ADR-047은 standalone 고정 포트(API 12301, admin UI 12305, Dagster 12302). ADR-048은 `/v1` 전 표면 clean cut + REST envelope/parameter/error/명명 정합성 표준. ADR-053은 `tripmate-agent` → `krtour-ai-agent` provider identity clean cut과 TripMate 직접 연동 제거 경계. |
| ADR proposed | (없음) — 다음 후보 번호는 ADR-054. |
