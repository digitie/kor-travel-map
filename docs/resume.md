...
- [x] krtour-map CLI 골격 + status — `cli/main.py`(argparse + status) +
  ...
  `krtour-map --help` 실동작.)
- [ ] **다음**: dedup queue MOIS sibling — MOIS 자기 안 sibling 중복 탐지(같은
  사업장 2슬러그) + `dedup_review_queue` 적재. 기존 `find_dedup_candidates` 재사용.
  또는 CLI mutate 명령(`import`/`dedup-merge`, mutex 적용) / Step B incremental cursor.
