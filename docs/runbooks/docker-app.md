...
쓴다. `tripmate-manager`가 공유 PostGIS/RustFS를 이미 구동 중이면
`KRTOUR_MAP_INFRA_EXTERNAL=true`로 local infra 서비스를 띄우지 않는다.
...
`http://127.0.0.1:12101/krtour-map`)을 사용한다. offline upload 원본 bucket은
...
Postgres host 포트 기본값은 `KRTOUR_MAP_POSTGRES_HOST_PORT=5432`이며,
`scripts/load-env.sh`는 `KRTOUR_MAP_PG_DSN` 미설정 시
`postgresql+asyncpg://krtour_map:krtour_map@127.0.0.1:5432/krtour_map`을 쓴다.
...
공유 인프라 모드는 `tripmate-manager`가 이미 `kraddr-geo-postgres:5432`와
`tripmate-rustfs:12101`을 띄운 상태에서 사용한다. 이때 krtour-map compose는 API,
frontend, Dagster webserver/daemon만 띄운다.

```bash
KRTOUR_MAP_INFRA_EXTERNAL=true bash scripts/docker-up.sh
```

공유 DB 비밀번호가 기본값과 다르면 `.env`에 `KRTOUR_MAP_POSTGRES_PASSWORD`를 두거나
컨테이너 관점 DSN을 직접 지정한다. 공유 Postgres에는 `krtour_map`과
`krtour_map_dagster` DB가 미리 있어야 한다.

```bash
KRTOUR_MAP_EXTERNAL_DOCKER_PG_DSN=postgresql+asyncpg://krtour_map:...@host.docker.internal:5432/krtour_map
KRTOUR_MAP_EXTERNAL_DOCKER_DAGSTER_PG_URL=postgresql://krtour_map:...@host.docker.internal:5432/krtour_map_dagster
KRTOUR_MAP_EXTERNAL_DOCKER_OBJECT_STORE_ENDPOINT_URL=http://host.docker.internal:12101
```
...
`NEXT_PUBLIC_KRTOUR_MAP_DAGSTER_URL`(기본 `http://127.0.0.1:12302`)이다. 로컬 API가
