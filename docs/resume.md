...
- [x] geocoder 보강 라이브 재검증 — kraddr-geo REST(`127.0.0.1:8888`) 실연동으로
  ...
  §5. 코드 변경 없음.
- [ ] **다음**: CLI mutate 명령(`import`/`dedup-merge`, mutex 적용) — provider
  record source 주입 설계 + argparse. 또는 Step B incremental cursor
  (`ProviderSyncState`) / 실적재 후 dedup false-positive 측정(가중치 조정 ADR-016).
