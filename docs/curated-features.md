...
|------------------|------|----------|------------|--------------------|-----------|
| `mcst_used_bookstores_csv` | 중고서점 | 한국문화정보원 | https://www.data.go.kr/data/15100298/openapi.do?recommendDataYn=Y | 2025-08-13 / 실시간 | 구현됨(T-223b). provider는 OpenAPI(`used_bookstores`)와 keyless CSV(`used_bookstores_csv`)를 모두 제공하고, krtour-map 적재는 CSV slug를 쓴다 |
...
TripMate는 여전히 REST를 호출해 복사하며, 이 테이블은 krtour-map 내부 cache다.
...
2. krtour-map DB migration으로 `feature.curated_*` 테이블과 seed source metadata를 추가한다. **완료(T-223c-1)**.
...
5. Admin UI에 curated 후보 선택/해제, source rule, TripMate copy preview를 붙인다. **T-223c-3**.
