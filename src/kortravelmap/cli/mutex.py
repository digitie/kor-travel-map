"""``kortravelmap.cli.mutex`` — CLI 명령 mutex (ADR-039).
...
lock key 컨벤션
---------------
- ``import:{provider}:{dataset_key}`` — ``ktmctl import`` 중복 실행 차단.
...
def import_lock_key(provider: str, dataset_key: str) -> str:
    """``ktmctl import <provider> <dataset>`` mutex 키."""
    return f"import:{provider}:{dataset_key}"


def dedup_merge_lock_key(target_id: str) -> str:
    """``ktmctl dedup-merge`` mutex 키. ``target_id`` = ``review_id``(병합
