...
## KHOA oceans beach info full scan
...
- 주소: TripMate가 넘긴 `reverse_geocoder` callable로 좌표 기반 주소/법정동코드 보강
...
## data.go.kr standard-data full scans
...
노출한다. Debug UI의 `run_dagster_etl` 액션도 같은 `DagsterEtlRun`/`EtlJobSpec` 계약을 사용한다.
...
## Korea Heritage place/area/event full scans
...
resource wiring, commit/rollback, alert를 맡고, 이 라이브러리는 provider public model을
...
ETL body는 provider client를 직접 호출한다. place/area scan은
`client.heritage.iter_all_details(...)` 또는 `client.search.iter_all_details(...)`를 사용하고,
event scan은 `client.event.iter_months(...)`를 사용한다. GIS 보강은
...
## KRMOIS license feature weekly full update
...
- 주소: TripMate가 넘긴 `reverse_geocoder` callable로 좌표 기반 주소/법정동코드를 보강
...
자세한 타입 기준은 [Notice feature ETL](notice-feature-etl.md)을 따른다.
