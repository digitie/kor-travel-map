...
**open PR 없음** (main PR#149까지 merged, Sprint 4(4a+4b) 완료). 다음 작업은
...
4. **문서 최신성 유지** — 현재 geocoding endpoint 정본은 REST v2
   `POST /v2/reverse`, `POST /v2/geocode`, 로컬 포트는 `http://127.0.0.1:8888`,
   frontend 현재 기준은 Next.js 16 + `maplibre-vworld-js#v0.1.2`.
...
**Phase 5 — Docker / 배포**
- [ ] T-209a — `docker-compose.yml` (서비스 6종, §2, 포트는 `docs/debug-ui-package.md`
  의 8087/8610 기준 + Dagster/Postgres/RustFS 표준 포트).
...
- **001~046 accepted**. **다음 후보 번호 = ADR-047**:
