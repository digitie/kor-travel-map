...
- `geometry json/jsonb null`: GeoJSON-like geometry payload. PostGIS geometry는 host DB에서 별도 materialized view/index로 확장할 수 있다.
- `payload json/jsonb not null default '{}'`
...
Korea Heritage area feature는 `python-krheritage-api` public model을 직접 받아 이 테이블에 적재한다.
...
- `geometry json/jsonb null`: GeoJSON-like LineString/MultiLineString payload
- `payload json/jsonb not null default '{}'`

제약:

- `total_distance_meters >= 0` when not null
- `expected_duration_minutes > 0` when not null
