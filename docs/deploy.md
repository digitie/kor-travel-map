...
| 서비스 | 기본 포트 | 역할 |
|--------|-----------|------|
| `api` | `12301` | `krtour-map-admin` FastAPI, OpenAPI/admin/debug/ops 라우터 |
| `frontend` | `12305` | Next.js admin UI |
| `dagster` | `12302` | krtour-map-owned Dagster UI/code location |
| `postgres` | host `5432`, container `5432` | 독립 `krtour_map` PostGIS DB |
| `rustfs` | API `12101`, console `12105` | S3 호환 객체 저장소(선택, backup 대상) |

`tripmate-manager`가 공유 PostGIS/RustFS를 이미 구동하는 로컬 환경에서는 krtour-map의
local `postgres`/`rustfs` 서비스를 함께 띄우면 `5432`/`12101`이 충돌한다. 이때는
`KRTOUR_MAP_INFRA_EXTERNAL=true bash scripts/docker-up.sh`를 사용해 API, Web UI,
...
로컬 Docker/venv 기본 Postgres host 포트는 `5432`이며, `KRTOUR_MAP_PG_DSN`을
명시하지 않으면 `scripts/load-env.sh`가 `127.0.0.1:5432/krtour_map` DSN을 채운다.
