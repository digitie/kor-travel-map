...
무사고 하루가 **280 = 93%**다(여유 20건). 그래서 여기만 run당 예산
(`opinet_run_call_budget`)이 있다.

**2026-09-19 — 재시도 축을 닫았다.** Dagster의 step 재시도는 asset을 처음부터
다시 실행하고 그 실행이 run 예산을 **전부 다시 쓴다.** 공통 정책
(`max_retries=3`)이면 한 job이 최악 네 번 돌아 하루 560회가 되어 위의 93%가
의미를 잃는다. OpiNet asset 둘에 전용 `RetryPolicy(max_retries=0)`을 두고,
예산 검사가 배수를 그 정책에서 **유도**한다(`test_docker_dagster_runtime.py`의
`_opinet_worst_day_runs`). 재시도를 되살리는 편집이 예산 검사를 함께 빨갛게
만든다 — 이 모드는 시군 윈도를 날짜로 회전시키므로 올바른 재시도 주기는
60초가 아니라 다음 스케줄이다.
