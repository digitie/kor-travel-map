...
`fe12e8dad80d1c6fd0ddcea36309c67949e64c6b`의 user/full OpenAPI SHA-256은 각각
...
`e37eda94d99cc95bd773fa0d386d19723775ea33`는 user vendor를 같은 bytes로, admin-detail vendor를
...
probe를 한 run으로 검증한다. Map `fe12e8da` / PinVi `e37eda94` immutable snapshot의 n150
fresh `0097` PostGIS·actual Dagster runtime ETL·Noble Playwright destructive main/recovery(2/2)·
PinVi public probe는 통과했고, 자동 cleanup 뒤 `BLOCKED.json`과 run 전용 container/volume이 남지
않음을 확인했다. PinVi 공개 `asof`는 Map current weather route에 전달하지 않고 typed snapshot의
`target_at`으로 전환한다.
