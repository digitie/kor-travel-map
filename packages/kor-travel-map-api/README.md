...
### Frontend (Next.js 공개 설정 + server runtime 비밀)
...
| `KOR_TRAVEL_GEO_API_KEY` | (필수, server-only) | `/api/geo/*` BFF가 kor-travel-geo v2에 `X-KTG-API-Key` header로만 보내는 소비자 키. browser query나 `NEXT_PUBLIC_*` 값으로 대체하지 않는다. |
