...
| python-kraddr-geo | (geocoder) | base_address, base_coordinate | on-demand | 주소·좌표 보강. `krtour.map.geocoding.KraddrGeoRestClient`가 REST v2 `POST /v2/reverse`, `POST /v2/geocode` 호출. 로컬 FastAPI 기본 `http://127.0.0.1:9001` |
