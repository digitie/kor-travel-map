...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#218(T-201b-b F5 provider SLA
정합성 검사)까지 merged. 이번 PR은 `T-201b-c` F7 dedup score regression WARN을
닫는다.
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 hardening
전까지 구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은
`T-201b-d` F8 file object orphan 정합성 또는 `T-RV-22/23/25` offline upload 후속이다.
