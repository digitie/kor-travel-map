...
셋뿐이었고, 그래서 OpiNet처럼 수천 건을 쓰는 경로가 0을 냈다 — 하필 **호출량이 가장
크면서 일일 한도는 data.go.kr 포털에서 확인조차 못 한** provider였다
(``docs/etl/upstream-quota.md`` §2). 분자가 유일한 가시성인데 그것이 0이었다.
운영자가 그 0을 "캐시/skip으로 요청이 없었다"로 읽으면(문서가 그렇게 읽으라고
적었다) 정반대 결론에 이른다.
