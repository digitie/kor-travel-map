...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#223(T-RV-24 후속 offline upload
ORM state check 동기화)까지 merged. 이번 PR은 `T-RV-25`로 offline upload store가
`app.state` cached store를 재사용하는 계약을 회귀 테스트와 문서로 고정한다.
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 hardening
전까지 구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은
`T-201b-d` F8 file object orphan 정합성 또는 `T-RV-23` offline upload 후속이다.
...
- **T-RV-23, T-RV-27, T-RV-29~T-RV-35** — offline 멱등/상태 drift,
  노출, user OpenAPI 누출,
