...
ChatGPT Codex → `~/dev/krtour-map-codex/`, Claude Code → `~/dev/krtour-map-claude/`, Google
Antigravity 2.0 → `~/dev/krtour-map-antigravity/`. 작업마다 worktree 안에서 브랜치만
...
24. **CLI 중복 실행이 위험한 명령에 mutex 없이 머지 금지** — ADR-039 proposed.
...
## 9. 코드 작성 단계 (Sprint 1 종료, Sprint 2 진입 준비)

본 저장소는 T-014 승인 (2026-05-25, PR#16) 이후 **코드 작성 단계**다.
Sprint 1 PR#17~#26 머지로 scaffolding 완료:

- `src/krtour/map/` PEP 420 namespace + 6 layer (category/dto/core/infra/
  providers/client placeholders)
- `dto/`: Feature + 5 detail + Coordinate + Source* + FeatureBundle + KST
  aware datetime
- `core/`: 7 exceptions + ID helpers (`make_feature_id` /
  `make_source_record_key` / `make_payload_hash`)
- `infra/`: `crs.py` (pyproj.Transformer ADR-030 narrow cache) + `db.py`
  (async engine + DSN 정규화)
- CI workflows + import-linter 4 계약 활성 + testcontainers PostGIS 통합
  테스트 베이스
...
**다음 단계**: Sprint 2 (ADR-034 1-4단계 visitkorea/kma/opinet/krex) + 진입
prep PR (`infra/models.py` + Alembic + `core/scoring.py`). 자세히는
`docs/sprints/SPRINT-2.md` + `docs/resume.md`.
