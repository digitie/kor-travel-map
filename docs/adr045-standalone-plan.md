...
`infra/consistency`(F1~F4), `infra/status_repo`, `AsyncKorTravelMapClient`(load/
...
- **T-205b** ~~`feature.sigungu_boundaries` 테이블~~ **취소** — D-11 결정: 시군구
  경계는 **kor-travel-geo가 소유**(`tl_scco_sig`), kor-travel-map은 REST 호출만(T-206a /
  T-206a-geo). kor-travel-map에 경계 테이블 신설하지 않는다.
...
- **T-206a** ✅ `infra/scope_repo.py` 신규 — scope→feature_id 해석 (읽기):
  ...
  - `resolve_sigungu_by_radius(session, lon, lat, radius_km)` — **kor-travel-geo
    `POST /v2/regions/within-radius`(D-11) 호출** → 교차 시군구 `code` 목록 →
    `WHERE sigungu_code = ANY(:codes)` feature 조회. `KorTravelGeoRestClient` 재사용.
...
- **T-206c** ✅ `AsyncKorTravelMapClient` 메서드 — `enqueue_feature_update_request` /
  `get_update_request` / `list_update_requests` / `cancel_update_request` (각 자체
  transaction). 기존 `run_*_job` 패턴 일관. Top-level
  `from kortravelmap import AsyncKorTravelMapClient` export도 실제 코드와 맞춘다.
- **T-206d** ✅ request 실행 본체 — scope 해석된 feature/provider를 실제 적재하는
  ...
  client를 import하지 않는다. `AsyncKorTravelMapClient`는
...
- **T-207e** ✅ `/features/*` 사용자/admin 공용 read 라우터 — in-bounds/{id}/search/
  batch. `GET /features/in-bounds`, `GET /features/search`,
  `GET /features/{feature_id}` envelope 상세, `POST /tripmate/features/batch` 완료.
  기존 `GET /features` raw bbox 응답은 admin frontend 호환용으로 유지한다.
...
- **T-208a** `packages/kor-travel-map-dagster/` 패키지 골격 — `definitions.py`(code
  location) + `pyproject.toml`(dagster + kortravelmap + provider libs). import-linter
  계층에 dagster layer 추가(client보다 위).
- **T-208b** `resources.py` — `KorTravelMapDatabaseResource`(독립 DSN) +
  `KorTravelMapClientResource` + provider resource 세트(키는 env, D-15 주입 정책) +
  `KorTravelGeoResource` + 선택 `RustFSResource`.
...
- **T-209b** 기동 순서 스크립트 — postgres ready → `alembic upgrade head`(app DB
  ...
  - **T-209b-a (즉시)**: Dagster가 `.dagster/schedules/schedules.db-*` SQLite 파일을
    ...
    `event_log_storage`는 모두 PostgreSQL `kor_travel_map_dagster`를 사용하고, webserver와
...
4. **T-206a-geo** (형제 repo `kor-travel-geo`) — `/v2/regions/within-radius`
