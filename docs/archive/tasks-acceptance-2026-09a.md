# acceptance 아카이브 — 닫힌 task (~2026-09-13)

> `docs/tasks-acceptance.md`는 **열린 task**의 수용 기준을 갖는다. 그 파일이
> 읽기 한도(220KB)를 넘어(`tests/unit/test_docs_archive_links.py`) 규약 §8대로
> 분리했다 — 여기 있는 항목은 전부 `docs/tasks-done.md`에 완료로 기록된 것이고,
> 읽기 전용 이력이다. 과거 검색은 `rg <패턴> docs/archive/`.

| 항목 | 수 |
|---|---|
| 아카이브된 acceptance 절 | 31개 |
