...
> 작업 태도: 수술적 최소 변경·근거 기반. 모호하면 추측 말고 확인한다.
...
PinVi OpenAPI 연동**이다. PinVi는 kor-travel-map DB에 직접 접근하지 않고,
`kor-travel-map`을 운영 코드에서 직접 import하지 않는다. PinVi ↔ kor-travel-map
...
| PostgreSQL DB 이름 (개발/운영 기본) | `kor_travel_map` (ADR-045 — PinVi 공유 DB 아님) |
...
## PinVi ↔ kor-travel-map 경계 (ADR-045)
...
- **PinVi는 kor-travel-map을 OpenAPI로 호출한다**. 운영 코드에서
  `kor-travel-map`을 직접 import하지 않는다.
- PinVi는 kor-travel-map PostgreSQL/PostGIS DB에 직접 연결하지 않는다.
- kor-travel-map은 Docker 독립 프로그램으로 실행되며 독립 DB(`kor_travel_map`)와 독립
  Dagster metadata DB(`kor_travel_map_dagster`)를 가진다.
- OpenAPI는 우선 admin UI 기준으로 작성하고, PinVi 연동 시 필요한 공개/사용자
  API를 보완·확장한다.
...
- 부족한 endpoint, typed model, pagination, cursor, exception, raw payload
  보존 규칙은 PinVi나 본 저장소에 임시 facade를 만들지 않고 해당
...
4. **의존 방향 역행 금지** — `category → dto → core → infra → geocoding →
   ...
   는 없다 — HTTP 서버 코드는 `packages/kor-travel-map-api/`에만.
