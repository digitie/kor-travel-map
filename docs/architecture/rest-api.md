...
> (ADR-048 #9). `docs/architecture/tripmate-rest-api.md`는 TripMate **소비 매핑 view**로 수렴하고 계약
...
- **전 표면 `/v1`**: `/v1/features/*`·`/v1/categories`·`/v1/providers/*`
  **+ `/v1/admin/*`·`/v1/ops/*`·`/v1/debug/*`**(ADR-048이 #317의 admin 비버저닝을 supersede).
  `/tripmate/*` namespace는 **제거**(kor-travel-map은 TripMate에만 묶이지 않음) — batch는
...
> 본 문서가 전 표면 계약 정본이다. `docs/architecture/tripmate-rest-api.md`는 TripMate 소비 매핑 view이며
> 계약 세부는 이 문서를 따른다.
...
```
- ⚠️ `/tripmate/*` namespace **제거**(kor-travel-map은 TripMate 전용이 아니다). batch는
...
### 2.4.1 `/v1/public/*` — 공개 해수욕장/축제 뷰 (TripMate T-130)
...
- **정본 수렴(T-216g)**: 본 문서를 전 표면 단일 계약 정본으로, `docs/architecture/tripmate-rest-api.md`는
