...
- [ ] T-VN-M05-ACTIVATION — `a3f6a8f3…`·`22563762…`·`c700bd2e…`에 이어 Map `f90b7c28…`·PinVi `fdff06ba…`·pinset `fa28a6e7…`도 최신 CI·전문 적대 리뷰 두 건·trusted Manager `b45f54d5…` release·atomic `pin rotate-pair`·registry/public-copy gate 뒤 n150 isolated M04/M05 launcher를 정확히 한 번 실행했다. launcher는 exit 1이었고 허용된 durable safe result는 만들지 못했지만, 후속 `pin verify`가 exact pinset의 terminal 차단을 확인했다. raw HTTP·컨테이너·환경 출력과 output leaf는 열지 않으며 네 pinset·각 source pair·Manager source·output leaf는 어떤 이유로도 재실행하지 않는다. 다음 실행은 이 terminal 기록을 포함한 **새** Map revision, 새 PinVi provenance revision, 새 Manager source를 atomic `pin rotate-pair`로 함께 결박한 fresh pinset에서만 허용된다. terminal current의 role별 회전은 거부되고 source pair preflight도 ledger claim보다 먼저 실행된다. 최신 CI와 전문 적대 리뷰 두 건 뒤에만 n150 isolated M04/M05 live mutating E2E와 activation attestation을 정확히 한 번 실행한다.
