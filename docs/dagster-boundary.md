...
갖는다. T-200에서 이 컬럼을 사용해 구현할 batch DAG 패턴:
...
- root job은 `ops.import_jobs`에 `load_batch_id=UUID`, `parent_job_id=NULL`로 등록
- child job은 같은 `load_batch_id`와 root `parent_job_id`를 저장
