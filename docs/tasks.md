...
- **T-RV-04** Dagster provider 서비스키 resource 미구현(D-15, feature-load asset
  ...
  - **T-RV-04b**(진행 중, provider 순차 wiring): provider public client live fetcher를
    ...
    - [x] **datagokr_cultural_festivals**(festival, 2026-06-07) — `DataGoKrClient.festival.
      iter_all()`. dagster 단위 테스트(fake client) + 37 dagster suite green.
    - [ ] opinet_stations(area scope + station detail 정책), krex_rest_areas/traffic,
      krheritage items/events, mois_license_records(source DB refresh), knps point/geometry
      (file parser) 잔여 — provider별 fetch 정책과 함께 순차 추가.
