...
## 2026-06-13 Codex 작업 메모 — API/admin 패키지 분리

Prometheus 메트릭 PR 머지 후 사용자 후속 요청으로 REST backend와 admin frontend를
분리했다.

- Python FastAPI/OpenAPI backend는 `kor-travel-map-api`
  (`packages/kor-travel-map-api/`, import `kortravelmap.api`)로 이동했다.
- admin frontend는 `kor-travel-map-admin` (`packages/kor-travel-map-admin/frontend/`)에
  남겼다.
- API 설정 prefix는 `KOR_TRAVEL_MAP_API_*`, frontend API base URL은
  `NEXT_PUBLIC_KOR_TRAVEL_MAP_API`로 clean cut했다. admin web port
  `KOR_TRAVEL_MAP_ADMIN_WEB_PORT=12305`는 유지한다.
- OpenAPI 기계 정본은 `packages/kor-travel-map-api/openapi.json` /
  `openapi.user.json`으로 이동했다.
- ADR-055를 accepted로 추가했다.

**다음 한 작업**: 기존 큰 트랙은 **T-225** — T-212e closure 재검증.

## 2026-06-13 Codex 작업 메모 — T-226 후속 Prometheus 메트릭 추가

T-226 clean cut 이후 Prometheus 성능 측정 표면을 추가했다.

- `kortravelmap.api` FastAPI app이 기본 `GET /metrics`를 노출한다
  (`KOR_TRAVEL_MAP_API_PROMETHEUS_METRICS_ENABLED`, `_PATH`로 제어).
...
**다음 한 작업(당시)**: Prometheus PR/CI/merge 마무리. 2026-06-13 PR #429로 완료.

## 2026-06-13 Codex 작업 메모 — T-226 clean cut 구현 완료

T-226c/d/e package/runtime/document clean cut을 구현했다.

- Python import root는 `kortravelmap`, 권장 import는 `import kortravelmap as ktm`.
- admin/dagster package는 `kortravelmap.api`, `kortravelmap.dagster`로 정렬.
- CLI는 `ktmctl`, env는 `KOR_TRAVEL_MAP_*`, DB는 `kor_travel_map` /
  `kor_travel_map_dagster`, RustFS bucket은 `kor-travel-map` /
  `kor-travel-map-uploads`.
- 형제 프로젝트 표기는 `kor-travel-geo`, `kor-travel-concierge`,
  `kor-travel-docker-manager` 기준.
- E2E는 `kor-travel-docker-manager` 공유 Postgres `5432` 인스턴스를 사용해
  Alembic 적용 후 Windows Playwright 전체 33/33 통과.
...
- ADR-054, `docs/package-identity-rename.md`, T-226b 실행계획의 최종 layout을
  `src/kortravelmap`, `kortravelmap.api`, `kortravelmap.dagster`로 정렬했다.
...
- 현 main 기준 Python/설정/문서 후보 908개 파일 중 `kortravelmap` 참조 파일 368개,
  `KOR_TRAVEL_MAP` 참조 파일 86개가 확인됐다.
- 최종 Python layout은 `src/kortravelmap`, `kortravelmap.api`, `kortravelmap.dagster`로 둔다.
...
- `docs/package-identity-rename.md`를 추가해 현재값(`kor-travel-map`, `kortravelmap`,
  `KOR_TRAVEL_MAP_*`, `kor_travel_map`)과 목표값(`kor-travel-map`, `kortravelmap`,
...
- clean cut/no-shim 원칙을 확정했다. 구 `kortravelmap`/`KOR_TRAVEL_MAP_*` compatibility shim은
  만들지 않는다.
...
- TripMate PR #184(`5966628192a1f7b0c359a6435011f3e2f3f04469`)가 머지되어,
  `POST /admin/notice-plans/imports/krtour-curated-features`가 kor-travel-map
...
- TripMate 쪽 `KOR_TRAVEL_CONCIERGE_API_BASE_URL` / 12401 잔여 설정도 제거됐다.
...
- `curated_repo`에 source metadata refresh, enabled source rule bulk apply,
  ...
  추가하고 `AsyncKorTravelMapClient` 표면으로 노출했다.
...
T-223c 첫 조각으로 kor-travel-map의 curated overlay DB/API 기반을 구현했다.
...
- `openapi.json`/`openapi.user.json`과 `@kor-travel-map/map-user-client` 타입을 재생성했다.
...
T-222b로 TripMate T-130 차단 조건이던 공개 해수욕장/축제 사용자 API를 kor-travel-map
쪽에 구현했다.

- `src/kortravelmap/infra/public_views_repo.py`를 추가했다. 해수욕장은
...
- user OpenAPI allowlist와 `@kor-travel-map/map-user-client` 생성 타입/alias를 갱신했다.
...
`npm -w packages/kor-travel-map-user-client run type-check` 통과.

**다음 한 작업**: **T-223c** — kor-travel-map DB/API/Dagster/Admin UI 구현
...
완료됐고, T-223b provider 보강은 provider PR#10/#11 + kor-travel-map 변환 구현으로
...
- kor-travel-geo 연동: `NEXT_PUBLIC_KOR_TRAVEL_GEO_BASE_URL` 기반 브라우저 helper를 추가해
...
선택, kor-travel-geo geocode/reverse, kind별 detail form, duplicate 후보 확인을
...
## 2026-06-12 Codex 작업 메모 — T-224 kor-travel-concierge provider clean cut

T-224를 완료했다. 기존 `kor-travel-concierge` identity는 `kor-travel-concierge`로 바뀌었고,
TripMate와의 직접 관계는 끊었다. YouTube/AI 후보 provider 관계는
**kor-travel-map ↔ kor-travel-concierge** 사이에만 둔다.

- ADR-053을 추가해 ADR-049/050의 provider 명칭·관계 설명을 supersede했다.
- canonical provider는 `kor-travel-concierge-youtube`, 변환 모듈은
  `kortravelmap.providers.kor_travel_concierge`, raw payload key는
  `detail.payload.kor_travel_concierge`다.
- settings/env/Dagster resource/asset/schedule/test는 `kor_travel_concierge_*` /
  `KOR_TRAVEL_MAP_KOR_TRAVEL_CONCIERGE_*` 기준으로 clean cut했다. 구 `KOR_TRAVEL_CONCIERGE`
  env/provider alias는 두지 않는다.
- TripMate `curated_trip_plans` 생성 flow에는 `kor-travel-concierge`가 관여하지 않는다는
...
## 2026-06-12 Codex 작업 메모 — T-221~T-223 순서 재정렬 + kor-travel-concierge 선행
...
- 사용자 결정 반영: `kor-travel-concierge`는 `kor-travel-concierge`로 이름을 바꾸고 TripMate와의
  직접 관계를 끊는다. TripMate의 `curated_trip_plans` 생성에는 `kor-travel-concierge`가
  관여하지 않으며, AI 후보 provider 관계는 kor-travel-map ↔ kor-travel-concierge 사이에만 둔다.
- 새 선행 작업 T-224를 추가했다. T-221 진입 전 `kor-travel-concierge` provider 경계/명명/
  상세 구현을 마감한다.
...
**다음 한 작업**: **T-224** — `kor-travel-concierge` provider 경계 재정의 + 상세 구현.
...
- kor-travel-concierge: API `12401`
- 반영 범위: `.env.example`, `scripts/load-env.sh`, `docker-compose.yml`, Dockerfile expose,
  `ApiSettings`, main settings, frontend fallback/Playwright 설정, 테스트 기대값, ADR-047과
  운영 문서/runbook.
- 공유 `kor-travel-docker-manager` 인프라를 재사용하는 `docker-compose.external-infra.yml` overlay와
  `KOR_TRAVEL_MAP_INFRA_EXTERNAL=true` 경로를 추가했다.
...
- TripMate는 kor-travel-map REST `GET /v1/curated-features*` 후보 표면을 호출해
...
- TripMate T-130(`/public/*`) 차단 원인은 kor-travel-map 사용자 OpenAPI에 해수욕장/축제
...
- TripMate-agent 후속은 kor-travel-map 쪽 `kor-travel-concierge-youtube` provider로 구현했다.
  `providers.kor_travel_concierge` 순수 변환, Dagster REST fetch/resource/asset/schedule,
...
- `GET /admin/features/change-requests` meta에 `review_mode`를 추가해
  `KOR_TRAVEL_MAP_API_FEATURE_CHANGE_REVIEW_MODE`를 빈 큐에서도 표시한다.
...
`docs/integrations/kor-travel-map-rest-api.md`를 대조해 kor-travel-map REST API 정본 문서를
...
Postgres(`kor-travel-map-codex-postgres-1`)는 `features/source_records/source_links/import_jobs`
...
`npm -w packages/kor-travel-map-admin/frontend run e2e -- e2e/admin-ops.spec.ts --reporter=line`
실행해 9 passed.
...
- krtour 측: `provider_fetchers.fetch_knps_point_records`/`fetch_knps_geometry_records`
  ...
  `AsyncIterable` 지원.
- dataset key는 `KorTravelMapSettings.knps_point_dataset_key`(기본 `knps_visitor_centers`)
...
`.env.restore-swap`을 만들고, `KOR_TRAVEL_MAP_RESTORE_SWAP_APPLY=1`일 때만 compose
서비스를 해당 env로 재기동한다. `docker-compose.yml`은 기본 RustFS volume name은
유지하되 `KOR_TRAVEL_MAP_RUSTFS_VOLUME` override를 지원한다.
...
실행은 `KOR_TRAVEL_MAP_API_BACKUP_COMMAND_ENABLED=true` opt-in과 요청별
...
`NEXT_PUBLIC_*` fail-fast, kor-travel-geo timeout 설정화다. `admin_issues.py` timeout은
...
- **적합성 감사(`docs/reports/t-rv-04b-provider-fetcher-audit-2026-06-07.md`): datagokr
  ...
  - [x] **⑤ mois_license_records**(Phase B, 2026-06-07) — clean match. fetcher가
    `mois_source_db_path`(env `KOR_TRAVEL_MAP_MOIS_SOURCE_DB_PATH`)의 미리 sync된 MOIS 소스
...
(feature.features UPDATE + `ops.feature_overrides` upsert) + kor-travel-geo 정/역지오코딩.
...
`KOR_TRAVEL_MAP_API_API_CALL_LOG_ENABLED`) + error envelope 중앙화(기구현 확인).
...
`KOR_TRAVEL_MAP_DOCKER_BIND_HOST=127.0.0.1`로 제한하고, 컨테이너 내부 `0.0.0.0` listen은
유지한다. host 모든 interface 노출은 `KOR_TRAVEL_MAP_DOCKER_BIND_HOST=0.0.0.0` 명시
...
검증은 `TMPDIR=/tmp .venv/bin/python -m pytest tests/unit/test_infra_consistency.py tests/integration/test_consistency_reports.py tests/unit/test_cli_consistency_report.py packages/kor-travel-map-dagster/tests/test_maintenance.py -q`,
...
산출물을 staging app DB(`kor_travel_map_restore`), staging Dagster DB
(`kor_travel_map_dagster_restore`), staging RustFS Docker volume
(`kor-travel-map-rustfs-restore`)으로 복원한다. script는 `meta/SHA256SUMS`를 먼저 검증하고,
운영 DB 이름으로 restore하려는 경우 즉시 실패한다. 기존 staging 대상 재사용/삭제도
기본 금지이며 `KOR_TRAVEL_MAP_RESTORE_RECREATE=1`을 명시해야 한다.
...
`ktmctl consistency-report` CLI는 `run_consistency_checks()` F1~F8을 기본
...
helper 4종 + `kortravelmap.providers` knps/krheritage re-export.
...
독립 PR + 매번 origin/main rebase + 격리 WSL sandbox(`~/dev/kor-travel-map-claude`)**
로 진행한다.
...
**T-213d 완료**: `AsyncKorTravelMapClient.get_features` / `search_features` /
...
`storage.postgres` 설정을 `KOR_TRAVEL_MAP_DAGSTER_PG_URL` 기준으로 사용한다. Dagster
instance config에서 이 key는 run/event/schedule-sensor tick metadata를 함께
PostgreSQL에 저장하므로, local/Docker 모두 `kor_travel_map_dagster`가 단일 metadata DB다.
...
HTTP 모델, `kor-travel-map-admin`, user OpenAPI, feature update request 큐 구현 이전
...
`/features/nearby`(`T-213b`), bbox clustering(`T-213c`), `AsyncKorTravelMapClient`
...
`packages/kor-travel-map-admin/frontend/src/api/types.ts`를 커밋하고,
`src/api/*` DTO를 `paths`/`components` 파생 타입으로 전환했다. `gen:types`는
`../../kor-travel-map-api/openapi.json`을 읽고, `gen:types:check`가 frontend CI에서 drift를 차단한다.
...
운영 모델은 독립 PostgreSQL `kor_travel_map_dagster`를 Dagster metadata DB로 둔다고
...
`KOR_TRAVEL_MAP_DAGSTER_PG_URL`/`kor_travel_map_dagster`를 쓰게 하는 것이다. DoD는 schedule
...
우선 재사용하고, 없을 때만 `KorTravelMapSettings()`/S3 client를 lazy 1회 생성해
...
`kortravelmap.core.offline_upload_states`로 분리해 admin router,
`kortravelmap.offline_upload`, `infra.offline_upload_repo`가 같은 계약을 공유하게 한다.
...
hook 4개를 등록한다. `kor-travel-map-journal-required`는 staged `src/` 또는 `tests/` 계열
...
찾아 staged Python 파일 대상 `ruff format --check`, `mypy --strict -p kortravelmap`,
`mypy --strict -p kortravelmap.dagster`, `lint-imports`를 실행한다.
...
`scripts/docker-backup.sh`를 호출해 standalone Docker compose의 `kor_travel_map` app DB,
`kor_travel_map_dagster` Dagster metadata DB, RustFS volume을
`data/backups/<backup_id>/` 아래에 저장한다.

스크립트는 API/frontend/Dagster/RustFS writer service가 실행 중이면 기본 중단하고,
운영자가 명시적으로 `KOR_TRAVEL_MAP_BACKUP_ALLOW_RUNNING=1`을 준 경우에만 best-effort
...
T-RV-37 cleanup 묶음 중 `src/kortravelmap/infra/ops_repo.py`의 `_decode_cursor` broad
...
T-RV-37 cleanup 묶음 중 `@kor-travel-map/map-marker-react` dependency metadata를 정리했다.
...
backend Dagster GraphQL 대상은 `KOR_TRAVEL_MAP_API_DAGSTER_ALLOWED_HOSTS` allowlist를
...
정리한다. 실제 `packages/kor-travel-map-dagster` 구현은 provider 적재 schedule 9개와
...
T-RV-36을 처리한다. `packages/kor-travel-map-dagster/pyproject.toml`은 더 이상
`kor-travel-map`을 무핀으로 두지 않고 같은 릴리스인
`kor-travel-map==0.2.0-dev`에 맞춘다.
...
root(`:12305`), Dagster는 webserver root(`KOR_TRAVEL_MAP_DAGSTER_PORT`, 기본 `12302`)를
확인한다.
...
kor-travel-geo resolver 설정 누락은 `SigunguResolverUnavailable` 타입으로 표현하고
...
dataset, `KOR_TRAVEL_MAP_*` credential env, source env를 설명하는 guard resource를
등록한다. guard는 resource materialize 시 명확한 `RuntimeError`를 내며, secret 값은
메시지에 포함하지 않는다.

`KorTravelMapSettings`에는 `data_go_kr_service_key`, `opinet_api_key`,
...
T-RV-03을 처리한다. `kor_travel_map_client_resource`를 일반 return resource에서
...
`packages/kor-travel-map-dagster/tests/test_resources.py`가 fake engine/fake client로
...
`dagster-db-init` 서비스는 같은 Postgres container 안에 `kor_travel_map_dagster` DB가
없으면 생성한다. `docker/dagster.yaml`은 `KOR_TRAVEL_MAP_DAGSTER_PG_URL` 기반
`storage.postgres`를 설정하고, `kor-travel-map-dagster` 패키지에 `dagster-postgres`
의존성을 추가했다.
...
T-RV-07을 처리한다. `ApiSettings`에 `admin_routes_enabled`와
...
T-RV-06을 처리한다. `packages/kor-travel-map-api/src/kortravelmap/api/app.py`에
...
`KorTravelMapSettings.offline_upload_max_bytes`/`KOR_TRAVEL_MAP_OFFLINE_UPLOAD_MAX_BYTES`를
...
`src/kortravelmap/infra/batch_dag.py`는 기존 provider/offline 적재가 만든 실제
...
`AsyncKorTravelMapClient.run_batch_dag_consistency_gate(...)`로 DB transaction을 소유한다.
...
- `kortravelmap.offline_upload`: CSV/TSV preview, validation result/issue DTO, validation
...
- `kortravelmap.geocoding`: `AddressResolver`, geocode response → `Address` 변환,
  kor-travel-geo REST v2 address resolver, cached resolver. `bjd_code`가 없으면 주소
...
- provider 변환기: datagokr 표준데이터, MOIS, OpiNet, KREX, krheritage 변환 경로에
  ...
  실패해도 주소가 있으면 kor-travel-geo geocode 결과로 feature_id 계산 전 보강한다.
...
- Dagster: `offline_upload_load` op가 `KOR_TRAVEL_MAP_KOR_TRAVEL_GEO_BASE_URL`이 있으면
  kor-travel-geo REST v2 resolver/reverse geocoder를 열고 CSV/TSV load에도 주입한다.
...
- `kortravelmap.infra.file_store.S3ObjectStore`: boto3 호환 S3 client wrapper.
  `read_bytes(storage_key)`가 `OfflineUploadObjectStore` protocol을 만족하고,
  `write_bytes()`는 다음 admin multipart upload API에서 재사용할 수 있다.
- `KorTravelMapSettings`: `.env.example`과 실제 field명을 맞췄다.
  `KOR_TRAVEL_MAP_OBJECT_STORE_*`는 feature file bucket(`kor-travel-map`)에 쓰고,
  `KOR_TRAVEL_MAP_OFFLINE_UPLOAD_BUCKET` 기본값은 ADR-045 D-14 기준 `krtour-uploads`다.
- `kortravelmap.dagster.resources`: `offline_upload_store_resource`가
  `KorTravelMapSettings` → boto3 client → `S3ObjectStore(bucket=krtour-uploads)`를 만든다.
- `docker-compose.yml`: RustFS service를 추가했다. host port는 API `12101`, console
  `12105`이며, `rustfs-init`가 `kor-travel-map`/`krtour-uploads` bucket을 생성한다.
...
확인했다. 추가로 Docker RustFS를 실제 기동해 `rustfs-init`가 `kor-travel-map`과
...
T-208g는 admin API/UI가 올린 오프라인 파일을 kor-travel-map 독립 Dagster가 적재할 수
...
- `kortravelmap.infra.offline_upload_repo`: 업로드 메타 생성/조회, load 시작/종료 상태
  전이 raw SQL repository.
- `kortravelmap.offline_upload`: JSON/JSONL `FeatureBundle` parser, checksum/size 검증,
  provider/dataset/scope advisory lock, `import_jobs` running/done/failed 전이,
  `load_bundles` 적재 orchestration.
- `AsyncKorTravelMapClient.run_offline_upload_load_job`: Dagster/admin 내부에서 쓰는
  client 진입점.
- `packages/kor-travel-map-dagster`: `offline_upload_load` job + `offline_upload_store`
  resource key 등록.
...
T-208f를 진행했다. 새 `kortravelmap.infra.dedup_refresh_repo`는 DB에 적재된 활성
feature를 provider/dataset scope 기준으로 읽어 `core.dedup.DedupInput` 형태로
제공한다. `AsyncKorTravelMapClient`에는 다음 운영 메서드를 추가했다.
...
ADR-045 T-208d로 `packages/kor-travel-map-dagster`에 당시 구현된 9개 Feature 적재 asset의
provider별 KST schedule과 asset job을 등록했다. 새 모듈
`kortravelmap.dagster.schedules`는 `FEATURE_LOAD_SCHEDULE_SPECS`,
...
ADR-045 T-207g로 `packages/kor-travel-map-api/scripts/export_openapi.py`를 admin/user
profile 기반 export로 확장했다. 기본 `--profile admin`은 기존
`packages/kor-travel-map-api/openapi.json` 전체 admin spec을 유지하고,
`--profile user`는 TripMate/user-facing API subset을
`packages/kor-travel-map-api/openapi.user.json`으로 생성한다. `--profile all`은 두
산출물을 함께 생성/검증한다.
...
read API를 `kor-travel-map-admin`에 연결했다. 새 endpoint는
...
ADR-045 T-207d로 `kor-travel-map-admin`에 운영 조회용 `/ops/*` 라우터를 추가했다.
...
ADR-045 T-207c로 `kor-travel-map-admin`에 `/admin/features` 운영 목록과
...
ADR-045 T-208e로 `packages/kor-travel-map-dagster`에 feature update request 큐 실행
sensor/job을 추가한다. `feature_update_request_queue_sensor`는 15초 간격으로
`AsyncKorTravelMapClient.peek_next_update_request()`를 호출해 queued/now request 후보를
...
`AsyncKorTravelMapClient.execute_feature_update_request()`를 호출한다. 실제 provider 호출은
...
`AsyncKorTravelMapClient.fail_update_request()`로 request/import job 실패 전이를 보강하고,
...
ADR-045 T-207f로 `kor-travel-map-admin`에 `/admin/poi-cache-targets`와
...
ADR-045 T-207a로 `kor-travel-map-admin`에 `/admin/feature-update-requests` 라우터를
...
직접 호출하고, OpenAPI schema는 `packages/kor-travel-map-api/openapi.json`에 export한다.

목록 필터는 `state`, `scope_type`, `provider`, `dataset_key`, `created_from`,
`created_to`, `page_size`, `cursor`다. 이를 위해 `infra.feature_update_repo`와
`AsyncKorTravelMapClient.list_update_requests`도 optional filter를 받도록 확장했다.
...
형제 repo `kor-travel-geo` main 기준으로 T-206a-geo를 재검증했다.
...
test_optional_real_postgres_regions.py`는 `KOR_TRAVEL_GEO_TEST_PG_DSN`이 있을 때 실제
...
`15 passed, 1 skipped`였다. skip은 현재 shell에 `KOR_TRAVEL_GEO_TEST_PG_DSN`이 없어서
...
ADR-045 T-206c로 `AsyncKorTravelMapClient`에 feature update request 메서드 4종을
...
동시에 `from kortravelmap import AsyncKorTravelMapClient` top-level import를 실제 public
...
다음 한 작업은 T-206c다. `AsyncKorTravelMapClient`에
...
`sigungu_by_radius`는 kor-travel-geo REST v2 `/v2/regions/within-radius`를 직접 import하지
...
ADR-047로 kor-travel-map standalone 로컬 포트를 API `12301`, admin UI `12305`, Dagster
`12302`으로 고정했다. `ApiSettings`, frontend scripts, Playwright 기본 baseURL,
...
`KOR_TRAVEL_MAP_API_*`/`NEXT_PUBLIC_*` 환경변수로 매핑한다.

## 2026-06-02 Codex 작업 메모 — kor-travel-map Dagster Feature ETL 1차 구현

TripMate 구현을 참고하지 않고 kor-travel-map 자체 Dagster code location
`packages/kor-travel-map-dagster/`를 추가했다. 메인 라이브러리 `kortravelmap`은 계속
Dagster를 import하지 않으며, Dagster 패키지가 provider record resource를 받아 기존
provider 변환 함수 9종을 호출하고 `FeatureBundle` 주소/좌표 검증 후
`AsyncKorTravelMapClient.load_feature_bundles`로 PostGIS에 적재한다.
...
## 2026-06-02 Codex 작업 메모 — kor-travel-geo 반경 endpoint 재정합

kor-travel-geo `origin/main` 기준 `POST /v2/regions/within-radius`가 구현되어 있음을
다시 확인하고, kor-travel-map `KorTravelGeoRestClient`/helper/parser를 최신 REST v2 계약에
...
실데이터 확인은 로컬 kor-travel-geo REST `http://127.0.0.1:12201` + T-027 최종 적재
...
geocoding 전용 디버그 화면/라우터는 kor-travel-geo 프로젝트에서만 본다는 사용자 결정에
따라 kor-travel-map-admin에서 제거했다. kor-travel-map에는 provider 주소 보강에 필요한
`kortravelmap.geocoding` client만 남긴다. 검증은 frontend lint/type/build,
...
(MOIS Step A bulk + Step B incremental cursor + `ktmctl dedup-merge` +
...
명세, 실행 계획, 모든 D-1~D-16 의사결정, `kor-travel-map-admin` rename을 완료했다.
...
(`F:\dev\kor-travel-map*`)를 Git 원본으로 명시한다. WSL ext4는
테스트/실행 전용 샌드박스이며, 필요 시 NTFS 소스를 `rsync`해서 사용한다.
`kor-travel-geo` 최신 로컬 포트 정책도 재확인하여 geocoding REST live
...
provider 라이브러리. ADR-045 = kor-travel-map Docker 독립 프로그램 + 독립 DB/Dagster +
...
TripMate 연계 (**ADR-045**, ADR-003 supersede): kor-travel-map은 Docker 독립 프로그램
이고 TripMate는 **OpenAPI(HTTP)** 로 feature 조회/상세/업데이트를 호출한다(직접
import·공유 DB 없음). admin/API는 `kor-travel-map-admin` 패키지(ADR-020, 내부망·
인증 없음 ADR-005)가 제공하며, 메인 라이브러리(`kortravelmap`)는 그 api/dagster가
내부에서 쓰는 핵심 엔진이다.
...
- **Provider ⑤ KNPS** 14 dataset (`providers/knps.py`) — Point/place 5건 +
  ...
  **knps-api 책임**(ADR-028 Amendment I / ADR-044). ⚠️ CSV 컬럼명 best-guess —
  ...
  (통계 3건은 feature 본문 X.)
- **geocoding (kor-travel-geo REST v2 연동)** (`kortravelmap.geocoding`) ✅ — 좌표↔주소
  ...
  `geocode_response_to_coordinate` + `KorTravelGeoRestClient`(httpx **주입**,
  TYPE_CHECKING-only — 메인 패키지 런타임 httpx 의존 X) + 팩토리
  `kor_travel_geo_{reverse,address}_geocoder` + `cached_reverse_geocoder`(좌표 메모이즈).
  설정: `KOR_TRAVEL_MAP_KOR_TRAVEL_GEO_BASE_URL` (로컬 기본 예:
...
- **provider 변환기 전면 async + geocoder 자동 보강** ✅ — festival/opinet/krex/
  ...
  async. **남은 것**: kor-travel-geo client 수명·실 DB 적재 오케스트레이션은 호출자.
...
- ~~`AsyncKorTravelMapClient` 적재/dedup 오케스트레이션~~ ✅ 완료 (PR#89/#122 —
...
- [x] `kortravelmap.mois.load_mois_license_features_bulk` loader — `license_records_to_bundles`
  → `infra.load_bundles` 얇은 오케스트레이션 + `AsyncKorTravelMapClient` 메서드 +
...
- [x] Step A snapshot soft-delete — `infra.soft_delete_features_not_in_snapshot` +
  `kortravelmap.mois.{delete_mois_license_features_not_in,sync_mois_license_features_bulk}`
  + `AsyncKorTravelMapClient.sync_mois_license_features_bulk` + PostGIS 통합 3건.
...
- [x] MOIS Step A 작업 통합 — `jobs_repo.start_import_job` + `kortravelmap.mois.run_mois_license_bulk_job`
  (advisory lock 단일 워커 직렬화 + import_jobs 추적 + sync) + `AsyncKorTravelMapClient`
...
- [x] CLI mutex(SPRINT-4 §2.8) — `src/kortravelmap/cli/` layer 신설 + `cli/mutex.py`
  (`mutex_lock`/`try_mutex_lock` + lock key 헬퍼) + import-linter layers cli 최상위
  추가. unit 4 + integration 3. (mypy 55 / 737 passed.)
- [x] kor-travel-map CLI 골격 + status — `cli/main.py`(argparse + status) +
  `infra/status_repo.gather_status_counts` + `AsyncKorTravelMapClient.status_counts` +
  `[project.scripts] kor-travel-map`. unit 5 + integration 2. (mypy 57 / 744 passed,
  `ktmctl --help` 실동작.)
- [x] dedup MOIS self-sibling — `core/dedup.find_sibling_candidates`(within-set
  pairwise) + `AsyncKorTravelMapClient.sync_sibling_candidates`. unit 6 + integration 1.
  (mypy 57 / 751 passed.)
- [x] geocoder 보강 라이브 재검증 — kor-travel-geo REST(`127.0.0.1:12201`) 실연동으로
  MOIS 좌표 → bjd_code 보강 200/200(100%) 확인. `docs/reports/mois-live-test-2026-06-01.md`
  §5. 코드 변경 없음.
- [x] CLI mutate 명령 ①  `ktmctl import mois <records-file>` — NDJSON snapshot
  ...
  776 passed.)
- [x] CLI mutate 명령 ② `ktmctl dedup-merge <review_id>` — 수동 병합(ADR-016).
...
- [x] (Sprint 4b) dedup 운영 FP 측정 도구 — `infra.status_repo.dedup_fp_stats`
  (confirmed=merged+accepted / FP=rejected / precision / fp_rate) + `ktmctl status`
...
- [x] (Sprint 4b) Place 전화번호 보강 백그라운드 시작 — `kortravelmap.enrichment`
...
- PR#110 `AGENTS.md` + `docs/dev-environment.md` + `docs/codegraph-worktree.md` 정책 문서 수정 (NTFS를 메인레포로 잡고 테스트 시 WSL 내 ext4로 카피하는 정책 정립, 에이전트별 worktree 프리픽스를 `kor-travel-map-`으로 개정하고 NTFS F:\dev\ 상에 신설 및 .env 로컬 키 복사, MCP 설정의 `codegraph.cwd` 를 각 에이전트 워크트리 경로로 동기화)
...
- PR#17 `src/kortravelmap/` PEP 420 scaffolding + settings + smoke tests
- PR#18 `category/` 144건 (kraddr-base → kortravelmap.category, ADR-023/027)
...
- PR#48 agent worktree `geo-*` → `kor-travel-map-*` rename + tasks.md 최신화
...
- [x] `src/kortravelmap/category/` — 144건 PlaceCategory
- [x] `src/kortravelmap/dto/` — Feature + 5 detail + Coordinate + Address +
      WeatherValue + PriceValue + SourceRecord/Link/FeatureBundle
- [x] `src/kortravelmap/core/` — exceptions 7종 + `make_feature_id` +
...
- [x] `src/kortravelmap/infra/` — models.py (ORM, +FeatureConsistencyReportRow) +
...
- [x] `src/kortravelmap/providers/` — standard_data / kma / opinet / krex /
...
- [ ] `src/kortravelmap/providers/` — knps SHP bytes→WKT 디코딩(park_boundaries) /
      krheritage (Sprint 3) / mois (Sprint 4)
- [x] `src/kortravelmap/infra/feature_repo.py` — raw SQL load 경로 (Sprint 3 —
      FeatureBundle upsert features/source_records/source_links + get_feature_row,
      ADR-004; bulk COPY + /features 라우터는 후속)
- [ ] `src/kortravelmap/client/` — `AsyncKorTravelMapClient` (Sprint 3~4)
- [x] `packages/kor-travel-map-admin/` — create_app + routers (health/version/etl)
      + settings (8 provider key) + etl_fixtures + etl_live + openapi.json
- [x] `packages/kor-travel-map-admin/frontend/` — Next.js 16 + TanStack + Zustand
      + ETL preview page
...
- [x] Place phone enrichment (Sprint 4b — `kortravelmap.enrichment`)
...
029→043 supersede, 044 (로컬 우선 조회 + 정합성 책임), 045 (kor-travel-map Docker 독립
...
ls src/kortravelmap/                       # 기존 모듈 구조
cat docs/event-feature-etl.md            # provider 문서 예시
git checkout main                        # 복귀
```

또는 GitHub UI:
- https://github.com/digitie/kor-travel-map/tree/v1

저장소 루트의 `kor-travel-map-spec.docx` (약 80쪽)는 v1 산출물 + SPEC V8
정합 + kor-travel-geo 디시플린을 종합한 reference.
