...
**해제 시점**: 사용자 T-014 (Sprint 1 진입) 승인 → Sprint 1 PR로 모든 proposed
ADR (027~034) 일괄 accepted 전환 + `src/krtour/map/` scaffolding. 자세히는
`docs/sprints/SPRINT-1.md` + `AGENTS.md §"코드 작성 금지"`.

**현재 허용된 예외**:
- `pyproject.toml` enforcement (import-linter 차단 계약 + coverage 주석)
- `packages/krtour-map-debug-ui/scripts/export_openapi.py` skeleton
- `packages/krtour-map-debug-ui/frontend/` Next.js skeleton (ADR-025)
- `packages/map-marker-react/` npm 패키지 skeleton (ADR-029)
