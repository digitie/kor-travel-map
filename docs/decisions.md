...
## ADR-053: `tripmate-agent` provider identity를 `krtour-ai-agent`로 clean cut하고 TripMate 직접 연동을 제거한다
...
`kor-travel-concierge`가 됐다. 본 ADR의 `krtour-ai-agent` 코드/provider/env 이름은
...
다시 바뀌었다. provider canonical name(`krtour-ai-agent-youtube`)과 env/module 이름
...
- canonical provider name은 `krtour-ai-agent-youtube`다. 구
  `tripmate-agent-youtube` alias나 호환 shim은 만들지 않는다.
- Python 변환 모듈은 `krtour.map.providers.krtour_ai_agent`, 주 변환 함수는
  `krtour_ai_agent_items_to_bundles`, inactive helper는
  `krtour_ai_agent_inactive_entity_ids`다.
- Dagster resource/asset/schedule 이름은 `krtour_ai_agent_youtube_features`,
  `feature_place_krtour_ai_agent_youtube`,
  `feature_place_krtour_ai_agent_youtube_daily_schedule`로 둔다.
- krtour-map 설정/env는 `KRTOUR_MAP_KRTOUR_AI_AGENT_*`를 사용한다. 구
  `KRTOUR_MAP_TRIPMATE_AGENT_*`는 읽지 않는다.
- export item의 provider 기본값은 `krtour-ai-agent-youtube`, dataset_key는 기존
  `youtube_place_candidates`, source_entity_type은 기존
  `extracted_place_candidate`를 유지한다.
- Feature detail 원본 payload key는 `detail.payload.krtour_ai_agent`로 저장한다.
  출처 UX가 읽는 평면 key는 계속 `detail.facility_info`를 우선한다.
- TripMate는 krtour-ai-agent를 호출하지 않는다. YouTube 후보는 krtour-map feature로
  정규화된 뒤 krtour-map OpenAPI를 통해서만 TripMate에 도달한다.
- `curated_features` → TripMate `curated_trip_plans` 복사 계약에는
  krtour-ai-agent가 참여하지 않는다.
...
- 기존 개발 DB에 `tripmate-agent-youtube` feature가 있으면 삭제 후
  `krtour-ai-agent-youtube` snapshot을 다시 적재한다. provider name이
  `feature_id`/source 자연키에 들어가므로 자동 rename migration은 만들지 않는다.
- `docs/integration-map.md`의 직접 관계는
  `krtour-ai-agent → krtour-map → TripMate`로만 표현한다.
...
같은 날 추가 결정으로 CLI 목표명은 `ktmctl`, PostgreSQL 기본 DB와 RustFS에서 쓰는
사용자 가시 이름은 `kortravelmap*` 계열로 맞춘다.
관련 형제 프로젝트의 GitHub repo/project 표시명도 함께 정렬한다:
`python-kraddr-geo`는 `kor-travel-geo`, 기존 `tripmate-agent`/`krtour-ai-agent` 계열
...
| Python import root(메인) | `krtour.map` | `kortravelmap` |
| 권장 import | `from krtour.map import ...` | `import kortravelmap as ktm` |
| Admin distribution | `krtour-map-admin` | `kor-travel-map-admin` |
| Admin import | `krtour.map_admin` | `kortravelmap.admin` |
| Dagster distribution | `krtour-map-dagster` | `kor-travel-map-dagster` |
| Dagster import | `krtour.map_dagster` | `kortravelmap.dagster` |
| CLI | `krtour-map` | `ktmctl` |
| env prefix | `KRTOUR_MAP_*` | `KOR_TRAVEL_MAP_*` |
| admin env prefix | `KRTOUR_MAP_ADMIN_*` | `KOR_TRAVEL_MAP_ADMIN_*` |
| 개발/운영 DB 이름 | `krtour_map` | `kortravelmap` |
| Dagster metadata DB 이름 | `krtour_map_dagster` | `kortravelmap_dagster` |
| RustFS bucket/volume 표시명 | `krtour-map*`, `krtour-uploads` | `kortravelmap*`, `kortravelmap-uploads` |
| Docker/image/service 표시명 | `krtour-map*` | `kor-travel-map*` |
| 주소 서비스 프로젝트/레포명 | `python-kraddr-geo` | `kor-travel-geo` |
| AI 후보/concierge 프로젝트/레포명 | `tripmate-agent`, `krtour-ai-agent` | `kor-travel-concierge` |
...
- runtime/deployment identity는 T-226d에서 수행한다: env prefix, 기본 DB 이름,
  Dagster metadata DB 이름, RustFS bucket/volume 이름, Docker/image/service name,
...
- T-226b는 `docs/reports/t-226b-package-clean-cut-plan-2026-06-12.md`로 분할 단위와
  검증 게이트를 확정한다.
- T-226c/d/e가 끝나기 전까지 `python-krtour-map` / `krtour.map` 표기는 코드와 현재 운영값을
  가리키는 사실로 남는다.
