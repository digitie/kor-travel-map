...
저장 정본은 항상 `kraddr-geo` REST v2 결과로 만든 `krtour.map.dto.Address`다.

- 좌표가 있으면 `POST /v2/reverse`로 주소와 행정코드를 얻는다.
- 주소 문자열만 있고 좌표가 없으면 `POST /v2/geocode`로 좌표 후보를 얻은 뒤,
  그 좌표를 다시 `POST /v2/reverse`로 정규화한다.
- 좌표와 주소가 함께 있으면 **좌표 기준 reverse 결과를 정본**으로 삼고, provider
  주소 문자열과 kraddr-geo 주소를 비교해 `AddressMatchReport`를 만든다.
- provider 주소가 일부만 있거나 결측이면 남은 주소 문자열, 좌표, 시군구명 같은
  단서를 kraddr-geo 요청 입력으로 사용한다.
...
- kraddr-geo 호출 실패, 결과 없음, confidence 미달, provider 주소와 좌표 기준 주소의
...
의존하므로(ADR-009) 변환기는 feature_id 계산 **전에** kraddr-geo를 호출해
`Address`를 채운다. 좌표가 있으면 `reverse_geocoder(coord)`, 좌표가 없고 주소
문자열이 있으면 `address_geocoder(address)` 후 `reverse_geocoder(coord)`를 호출한다.
같은 batch의 중복 좌표는 `geocoding.cached_reverse_geocoder`로 1회만 호출한다
(변환기가 내부에서 자동 래핑).
...
| VisitKorea `areaCode`/`sigunguCode` | provider 코드 — `raw_data`/`payload`에만 보존. 법정동코드로 저장 X. kraddr-geo reverse/geocode로 확정 |
| OpiNet `sigun_code` | 동일 — `raw_data`/`payload`에만. kraddr-geo reverse/geocode로 확정 |
| MOIS 관할기관 코드 | `payload`에만 |

**철칙**: provider 주소 문자열이나 provider 자체 코드만으로 `legal_dong_code`를
추정하지 않는다. kraddr-geo 결과가 없으면 `null` + 검토 상태로 둔다.
...
| `source_legal_dong` | 원천이 표준 법정동코드를 직접 제공했고 kraddr-geo 결과와 충돌 없음 | 정상 |
...
| `provider_address_mismatch` | 좌표 기준 kraddr-geo 주소와 provider 주소가 다른 장소로 보임 | **검토 대상** |
...
| `not_geocoded` | 좌표/주소 단서가 있지만 kraddr-geo 결과 없음 또는 resource 없음 | **검토 대상** (운영 설정/데이터 문제) |
...
async def festival_to_bundles(items, *, fetched_at, reverse_geocoder=None):
    for item in items:
        # 1. provider 응답은 raw/provenance. 저장 정본은 kraddr-geo 결과.
...
[httpx.AsyncClient → kraddr-geo REST]   POST /v2/reverse · POST /v2/geocode
...
- [ ] `kraddr-geo` REST 서비스 v2(`POST /v2/reverse`, `POST /v2/geocode`)가 운영 환경에서 reachable
