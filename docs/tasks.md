      ...
      - [~] **opinet_stations** — **provider 라이브러리 보강 완료, krtour wiring 후속**.
        ...
        **물리적으로 없음**(station 반환은 aroundAll 반경≤5km/lowTop10 top20/detailById 단건뿐,
        ...
        - **krtour wiring 후속** — 사용자 결정(2026-06-08): **bbox + POI-타깃 둘 다 지원**. 3 PR:
          ...
          - [ ] **opinet-2 bbox fetcher**: settings `opinet_scope_*`(bbox) + `fetch_opinet_stations`
            (`iter_stations_in_bbox`, uni_id dedup) + bespoke asset + resource guard→live.
          - [ ] **opinet-3 POI-타깃**: opinet POI cache target → bbox 변환·enumerate(DB 조회) 모드.
