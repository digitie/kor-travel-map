...
- [ ] **다음**: `ops.import_jobs` 마이그레이션(alembic 0006) + `infra/jobs_repo.py`
  (enqueue/claim SKIP LOCKED/heartbeat/finish + lifespan 복구, ADR-011) — advisory
  lock helper 위에. 이후 mois source DB cursor iterator(`collect_and_load_*`) +
  CLI mutex(SPRINT-4 §2.8).
