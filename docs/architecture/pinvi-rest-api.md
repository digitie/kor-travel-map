# PinVi 소비용 kor-travel-map REST API 매핑

> **상태**: 2026-06-12, ADR-048 / T-216a~g + curated_features 문서 계약 기준.
> **역할**: 이 문서는 PinVi가 어떤 kor-travel-map API를 소비하는지 정리하는 view다.
> 전 표면 REST 계약의 단일 정본은 [`docs/architecture/rest-api.md`](rest-api.md)이고, 기계 정본은
> `packages/kor-travel-map-api/openapi.user.json` / `openapi.json`이다. 충돌 시 OpenAPI를 우선한다.

## 1. 연결 원칙

- PinVi는 kor-travel-map DB에 직접 접근하지 않고, `kor-travel-map`을 운영 코드에서 직접
...
- `KOR_TRAVEL_MAP_API_SERVICE_TOKEN`이 설정된 환경에서는 service read `POST /v1/features/batch`
  호출에 `X-Kor-Travel-Map-Service-Token`을 붙인다. (`/tripmate/*` namespace는 제거됐다 — kor-travel-map은
  PinVi 전용이 아니다.)

## 2. PinVi 소비 표면

| PinVi 기능 | kor-travel-map API | 비고 |
|---------------|----------------|------|
...
| 운영 feature 추가/수정/삭제 | `/v1/admin/features*` | PinVi public client 직접 호출 금지 |
| 운영 refresh 실행 | `/v1/admin/feature-update-requests*` | admin/operator flow만 |
| 비로그인 공개 해수욕장/축제 뷰 | `GET /v1/public/*` | T-222b부터 `openapi.user.json` 포함 |
| 테마형 curated trip plan import | `GET /v1/curated-features*` | T-223c-1부터 `openapi.user.json` 포함. `curated_features` → PinVi `curated_trip_plans` 1:1 복사 |

`/tripmate/feature-update-requests*`는 제거됐다. 사용자가 새 장소 추가, 정보 수정, 폐업 삭제를
제안하는 큐는 PinVi app DB가 소유하고, 운영자 승인 뒤 `/v1/admin/features*` 또는
...
```json
{
  "type": "https://kor-travel-map/errors/feature-not-found",
...
표준 코드는 `docs/architecture/rest-api.md` §4를 따른다.

## 5. 필드 불변식

- `feature_id`는 UUID가 아니라 opaque string이며, PinVi는 파싱하지 않고 그대로 저장한다.
- `feature_id` 값은 provider 재적재, 사용자 수정, version 승급, soft delete에도 바뀌지 않는다.
  정체성이 바뀌는 사건은 새 feature + link로 모델링한다.
- 좌표 필드명은 `lon`/`lat`가 cross-repo 정본이다. PinVi DEC-07도 이쪽으로 정렬한다.
...
- **T-182**: PinVi DEC-07 좌표명을 `lon`/`lat`로 하향 정렬.
- **T-210e** ✅(2026-06-11): kor-travel-map에 `packages/kor-travel-map-user-client/`
  (`@kor-travel-map/map-user-client`) — `openapi.user.json` 생성 TS 타입 + named alias +
  ADR-048 표면 단언 + CI drift gate. PinVi frontend는 vendoring 또는 같은
  openapi-typescript 버전 자체 codegen으로 pin(패키지 README 참조).

## 7. 사용자 제안 연동 합의 (ADR-051, 확정 2026-06-11 — T-217c)

PinVi 사용자 feature 추가/수정/삭제 제안의 전송 구간은 **기존
`/v1/admin/features*` change API**다(신규 suggestions API 없음, ADR-051). PinVi
admin 1차 승인 → 본 API 호출 → kor-travel-map `change-requests` 큐 최종 반영의 2단 검토.
PinVi 측 질의 5건(해당 repo `docs/integrations/kor-travel-map-rest-api.md` §7)의 확정:
...
## 8. curated_features → curated_trip_plans import 계약 (T-223c-1 read 구현, 2026-06-12)

테마형 source(세계음식점, 독립서점, 카페가 있는 서점, 도서관, 무장애 관광지 등)는
kor-travel-map `feature.curated_features` overlay가 소유한다. PinVi의 저장 정본명은
`app.curated_trip_plans` / `app.curated_plan_pois`다. PinVi API의 `/notice-plans`
또는 `notice_plan_id`는 기존 호환 alias일 뿐이며, 신규 DB/ORM/문서에서는
`curated_trip_plans`를 쓴다.

엔드포인트와 상세 payload는 [`docs/curated-features.md`](../curated-features.md)와
`packages/kor-travel-map-api/openapi.user.json`을 따른다.
...
- kor-travel-map `curated_features` 1건 = PinVi `curated_trip_plans` 1건.
- 하위 장소·정류점·추천 POI는 PinVi `curated_plan_pois`로 복사한다.
- PinVi는 `source_system='kor-travel-map'`,
  `source_curated_feature_id`, `source_curated_feature_version`, `source_etag`,
  `source_imported_at`을 저장한다.
- PinVi는 kor-travel-map DB를 직접 읽지 않고 REST payload snapshot만 저장한다.
- `feature_id`는 opaque string으로 저장하고 파싱하지 않는다.

## 9. YouTube 후보 feature detail 소비 계약 (T-217f — PinVi TM-08 선행)

provider `kor-travel-concierge-youtube`(marker `P-13`, kind `place`,
...
원본 export item 전체는 `detail.payload.kor_travel_concierge.{youtube,evidence,...}`에
보존된다(디버그/확장용 — UX는 facility_info 우선). export는 검수 통과 후보만
내려오므로(D-05) 별도 검수 상태 분기는 불필요하다.

## 10. T-130 공개 해수욕장/축제 뷰

PinVi T-130(`/public/*`)은 비로그인 공개 API이며, 현재 차단 조건은 kor-travel-map
사용자 OpenAPI에 해수욕장/축제 전용 뷰와 닫힌 detail 스키마가 없다는 점이었다.
kor-travel-map 쪽 계약은 [`docs/architecture/public-views-api.md`](public-views-api.md)에 둔다.
T-222b(2026-06-12)부터 이 표면은 `openapi.user.json`과
