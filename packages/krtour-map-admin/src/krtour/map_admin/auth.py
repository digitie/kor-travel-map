"""``krtour.map_admin.auth`` — 앱 레벨 service-token / 파괴적 작업 게이트.
...
- ``require_service_token`` — ``settings.service_token`` 설정 시 외부 surface에서
  ``X-Krtour-Service-Token`` 헤더를 **상수시간** 비교로 검증. 미설정이면 통과
  (intranet/dev 하위호환).
...
SERVICE_TOKEN_HEADER = "X-Krtour-Service-Token"
...
async def require_service_token(
    ...
    """``service_token`` 설정 시 ``X-Krtour-Service-Token``을 상수시간 검증한다.
