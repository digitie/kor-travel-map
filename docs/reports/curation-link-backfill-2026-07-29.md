...
### 오링크는 계통적이다 — 관광지 이름의 서울 사무소

같은 질의에서 §2의 남이섬과 **동일한 패턴**이 반복해 나왔다.

| 관광지 | DB feature 주소 |
| --- | --- |
| 남이섬 | 서울특별시 중구 남대문로9길 52 |
| 호미곶 | 서울특별시 중구 퇴계로28길 15 |
| 오륙도 | 서울특별시 중구 을지로3길 20 |

MOIS 인허가 데이터에 **관광지 이름을 단 서울 사무소·본사**가 다수 있고, 이름 기반 매칭이
정확히 여기에 붙는다. 즉 §2의 3건은 일회성 실수가 아니라 **재현되는 오탐 유형**이며,
`T-VN-H33`은 이 유형 전수를 대상으로 삼아야 한다.
