# Standard data feature ETL

`krtour_map.standard_data`는 공공데이터포털 표준데이터 5건만 다루는 bounded asyncio client와
feature ETL입니다. 범용 data.go.kr gateway로 확장하지 않습니다.
...
- `catalog.py`: dataset id, endpoint URL, feature kind, refresh cadence
...
로컬 확인은 다음 명령으로 실행합니다.

```bash
python -m krtour_map.debug_ui
```

UI는 표준데이터 ETL catalog, raw item preview, feature DB load, Dagster 계약 기반 수동 실행,
Kakao map feature marker, RustFS 설정/파일 목록, table browser를 제공합니다.
