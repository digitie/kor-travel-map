# krmois-license-feature-etl.md — KRMOIS 인허가 → place ETL

본 문서는 KRMOIS(localdata) 인허가 데이터의 영업중 row만 `place` feature로
승격하는 ETL이다. 폐업/취소/제외 업종은 본 라이브러리 feature DB에 보존하지
않는다.
...
| dataset_key | `krmois_license_features` |
| Feature.kind | `place` |
| source_entity_type | `license_place` |
| 상세 테이블 | `feature_place_details` |
| 코드 entrypoint | `krtour.map.providers.krmois` (변환), `krtour.map.krmois` (loader) |
| 갱신 주기 | 주 1회 (full update) |

## 2. 범위 / 책임

- `python-krmois-api`: localdata zip 다운로드 + MOIS source DB 갱신, raw/localdata
...
- `python-krtour-map`: 영업중 `PlaceRecord` → `Feature(kind=place)` + `PlaceDetail`,
  service slug 필터링, 오래된 KRMOIS feature 삭제, job metadata.
- TripMate: Dagster, MOIS source DB session, feature DB session, reverse
  geocoder, transaction, 알림.

wrapper/adapter/gateway 금지. 누락은 `python-krmois-api`에서 먼저.
...
| 제외 업종 | `KRMOIS_EXCLUDED_SERVICE_SLUGS` (아래 §4) |
| 주간 full update | snapshot에 없는 feature는 `delete_krmois_license_features_not_in(snapshot)` |
| 폐업/취소 처리 | `iter_closed_place_records()` + `delete_krmois_license_features_for_records(...)` |
| KRMOIS 전용 물리 컬럼 | **추가 금지**. 모든 정보는 `Feature.detail` JSONB |
...
위 업종은 여행 도메인과 거리가 멀거나 다른 source가 더 정확 → KRMOIS에서는
가져오지 않는다.

## 5. 승격 매핑 (`KRMOIS_LICENSE_PROMOTION_MAPPINGS`)
...
| `museums` (KRMOIS 분류) | `museum` | `CULTURE_MUSEUM` |
| ... (전체 매핑은 코드 상수) |
...
- 원본 좌표: EPSG:5174 → WGS84 변환 (`python-krmois-api`가 처리).
...
각 업종별 facility fields는 KRMOIS localdata의 service-specific 컬럼에서 추출.
스키마는 코드 상수 (`KRMOIS_FACILITY_FIELD_MAP`).

## 9. DB 적재 흐름

```python
from krtour.map.krmois import (
...
async def run_krmois_weekly_update(
    mois_session,                # python-krmois-api source DB
    ...
    async for batch in _batched(open_records, size=KRMOIS_LICENSE_DEFAULT_BATCH_SIZE):
        result = await collect_and_load_krmois_license_features(
            feature_session, batch, reverse_geocoder=reverse_geocoder,
        )
        snapshot_keys.update(result.upserted_source_entity_ids)
    
    # 3. snapshot에 없는 KRMOIS feature 삭제
    deleted = await delete_krmois_license_features_not_in(feature_session, snapshot_keys)
    
    # 4. 폐업/취소 별도 처리
    closed_records = mois_client.aiter_closed_place_records(mois_session)
    async for batch in _batched(closed_records, size=KRMOIS_LICENSE_DEFAULT_BATCH_SIZE):
        await delete_krmois_license_features_for_records(feature_session, batch)
...
| 항목 | 값 |
|------|----|
| asset 이름 | `feature_place_krmois_licenses` |
| JOB_SPEC | `krtour.map.providers.krmois.JOB_SPEC` |
| suggested cron | `0 2 * * 1` (매주 월 02:00 KST) — 또는 `0 2 5 * *` (월 5일) |
| group | `features_place` |
| ConcurrencyConfig | `krmois_api: max_concurrent=1` |
...
- snapshot에서 빠진 feature 삭제 (`delete_krmois_license_features_not_in`)
- 폐업 row 적재 시도 → 생성 안 됨
- 같은 입력 2회 → idempotent

## 12. 후속

- 관리번호 공백 row는 fingerprint 정책을 `python-krmois-api`에서 먼저 안정화.
- 새 업종 추가는 ADR + `KRMOIS_LICENSE_PROMOTION_MAPPINGS` 업데이트.
- KRMOIS sigun_code → 법정동코드 변환 표 (`python-krmois-api` 책임).
