...
| #11 | merged | debug UI frontend를 Next.js로 정정 | stack 통일 결정이 명확해졌다. `packages/krtour-map-admin/frontend`는 skeleton이므로 실제 라우터/API 타입 동기화는 Sprint 2 첫 라우터 PR에서 재검증이 필요하다. |
...
보완:
- "별도 패키지 `krtour-map-admin`의 debug REST/UI 계약 제공"처럼 표현을 바꾼다.
