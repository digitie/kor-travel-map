...
- **🟢 기타 코멘트 (구현 시 확인)**:
  - **코드 체계 검증 필수**: kraddr-geo `tl_scco_sig.sig_cd`(통계청 SIG 5자리)와
    krtour-map `feature.sigungu_code`(현재 bjd 기반 5자리)가 **동일 체계인지 1건
    실측 검증**. 다르면 (a) kraddr-geo가 행안부 시군구코드로 매핑해 반환하거나
    (b) bjd_code(10자리) prefix(앞 5자리)로 양쪽 통일.
  - **레벨 범위**: 1차는 `sigungu`만 구현하되 `levels` 파라미터로 시도·읍면동 확장
    여지 남김.
