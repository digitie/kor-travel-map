...
- 통합 테스트(``tests/integration/``)는 별도 ``tests/integration/conftest.py``의
  ``pg_container`` / ``pg_engine`` / ``pg_session`` fixture 사용 — source digest가
  고정된 testcontainers PostGIS (``postgis/postgis:16-3.5-alpine``, ADR-007).
  Docker 또는 testcontainers 미설치 시 자동 skip.
"""
