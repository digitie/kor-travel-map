...
**Sprint 3 ✅ 완료 → Sprint 4 🟡 진입 준비 완료** (2026-05-30 기준).
main 최신: `PR#95` + 본 prep PR (Sprint 4 4a/4b 분할 채택, coverage bar 75
상향). PR#86~#95 본 세션 10건 머지: krheritage 후속(area_square_meters /
file_sources) + dedup 큐(`core/dedup` + `ops.dedup_review_queue` + `dedup_repo`)
+ `AsyncKrtourMapClient` 오케스트레이터 + geocoding REST API v2 전환 +
Playwright e2e + workspace 루트 + frontend WSL 기동 + frontend CI 게이트 +
docs 정합 + `/features` 지도 페이지.
테스트: unit 599 / integration 11+ / mypy --strict 49 src files / import-linter
4 contracts kept / coverage 92.66% (`fail_under=75`) / Windows Playwright e2e
9/9(WSL frontend↔backend 실연동).

2026-05-31 기준 개발 환경 문서는 Windows Git(`git.exe`) + NTFS worktree
(`F:\dev\python-krtour-map*`)를 Git 원본으로 명시한다. WSL ext4는
테스트/실행 전용 샌드박스이며, 필요 시 NTFS 소스를 `rsync`해서 사용한다.
동일 날짜 기준 `python-kraddr-geo` 최신 로컬 포트 정책도 재확인하여 지오코딩
REST live 기본값은 FastAPI backend `http://127.0.0.1:8888`로 맞췄다.
...
- **Provider ⑤ KNPS** 14 dataset (`providers/knps.py`) — Point/place 5건 +
  ...
  **knps-api 책임**(ADR-028 Amendment I / ADR-044). ⚠️ CSV 컬럼명 best-guess —
  ...
  (통계 3건은 feature 본문 X.)
- **geocoding (kraddr-geo REST API v2 연동)** (`krtour.map.geocoding`) ✅ — 좌표↔주소
  보강. **PR#90/#123**: in-process python client(`reverse_v2`/`geocode_v2` —
  현 kraddr-geo에 미존재)에서 **REST 서비스 `/v1/address/*`**(ver 2.0)로 전환.
...
- ~~Debug UI WSL 기동 + Windows Playwright e2e~~ ✅ 완료 (PR#91/#92/#93, #117 —
  workspace 루트 + e2e 7/7 + 검출한 잠복 빌드 버그 fix + frontend CI 게이트).
...
## 다음 한 작업 — **Sprint 4 진입 PR (4a)**

`docs/sprints/SPRINT-4.md §2.1` Step A(bulk):
1. `python-mois-api` provider 모듈 추가 (`providers/mois.py` — structural Protocol).
2. MOIS Step A bulk 변환 함수(`*_to_bundles`) + PROMOTED 42 슬러그 category 매핑.
3. 첫 적재(advisory lock, ADR-011) + dedup queue 본격 운영.
4. 첫 적재 후 false-positive 측정 → 필요 시 가중치 조정 PR(ADR-016 supersede).
...
- 본 PR maplibre-vworld v0.1.2 + Next.js 16 최신화 (git URL+tag, ESLint CLI flat config)
