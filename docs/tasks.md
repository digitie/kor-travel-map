...
T-213a~h(TripMate 요구사항 후속 전체), T-209e-c admin backup/restore surface까지
...
`T-209e` 최종 잔여(advisory lock, staging restore smoke/count, restore hot-swap env
전환)를 닫는다.
...
- **T-RV-04** Dagster provider 서비스키 resource 미구현(D-15, feature-load asset
  ...
  - **T-RV-04b**(진행 중, provider 순차 wiring): provider public client live fetcher를
    ...
    - **나머지 6종은 설계 결정 선행 필요** — 적합성 감사
      ...
      - [~] **opinet_stations** — **provider 라이브러리 보강 완료, krtour wiring 후속**.
        ...
        **물리적으로 없음**(station 반환은 aroundAll 반경≤5km/lowTop10 top20/detailById 단건뿐,
        ...
        - [ ] **krtour wiring 후속(scope 결정 필요)**: 전국 nightly는 쿼터 비현실 →
          opinet은 **bounded bbox(operator 설정) 또는 POI-타깃 모델**로 wiring. krtour
          `OpinetStationItem` Protocol(station_name/brand_code/longitude·latitude Decimal/tel/lpg_yn)을
          provider `Station`(name/brand enum/lon·lat float, tel·lpg_yn 없음)에 ADR-044 재정렬 +
          settings-gated bbox fetcher 필요. (opinet 1건 외 T-RV-04b provider wiring 전부 완료.)
