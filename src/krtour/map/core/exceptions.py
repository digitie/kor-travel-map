"""``krtour.map.core.exceptions`` — 라이브러리 공개 예외 계층.

``docs/backend-package.md §5`` + ``docs/debug-ui-package.md §6.4 HTTP 매핑``의
원천. 모든 예외는 ``KrtourMapError``를 상속한다.
...
class KrtourMapError(Exception):
    """``krtour.map`` 라이브러리 모든 공개 예외의 베이스.

    호출자는 ``except KrtourMapError:``로 라이브러리 발 예외 전체를 catch할 수
...
class ValidationError(KrtourMapError):
...
class ImportJobConflictError(KrtourMapError):
...
class ProviderError(KrtourMapError):
