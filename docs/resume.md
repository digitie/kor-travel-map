...
**Sprint 3 진행 중** (2026-05-30 기준).
main 최신: `f33bd8f...PR#93` (PR#86~#93 본 세션 8건 머지 — krheritage 후속 +
실 DB 적재 오케스트레이션 + dedup 큐 + geocoding REST 전환 + Playwright e2e +
frontend CI 게이트).
테스트: unit 599 / integration 11+ / mypy --strict 49 src files / import-linter 4
contracts kept. Windows Playwright e2e 7/7 통과(WSL frontend↔backend 실연동).
...
- **`/features/*` 라우터 + frontend 지도 wiring (maplibre-vworld) — 다음 작업** (c).
