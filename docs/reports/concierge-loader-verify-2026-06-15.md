...
후속 PR에서 해소했고, P-01만 concierge측 잔여다.
...
- **concierge (잔여 — 그쪽 repo)**: P-01(`limit`에 `Query(ge=1,le=500)`로 명시적 422
  계약). 또한 `source_record.source_entity_id`가 한 후보의 upsert·reject/tombstone export
  에서 byte 동일하다는 계약 테스트 권장(map inactivate 조인 전제).
