...
> 에이전트별 worktree)을 본 라이브러리에 맞춰 옮긴 것이다. TripMate ↔ 본 lib는
> **함수 직접 호출**(HTTP 아님, CLAUDE.md §1) 관계라 운영 절차는 공유하되 식별자·
> 포트는 본 lib 기준으로 적는다.
...
| 4 게이트 | `ruff check` + `mypy --strict` + `lint-imports` + `pytest -q` (debug-ui 변경 시 openapi-drift 추가) | ADR-014/032 |
