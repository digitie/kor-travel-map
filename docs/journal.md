...
- **install script 정책**: npm 12에서 실행이 필요한 `esbuild@0.28.1`과
  `unrs-resolver@1.12.2`만 `allowScripts`에 version-pin했다.
  `strict-allow-scripts=true`라 새 dependency script나 version drift는 검토 없이 실행되지 않고
  clean install이 실패한다. 현재 `npm install-scripts ls` 결과는 unreviewed package 0개다.
- **Node 계약**: npm 12.0.1의 published engine과 동일한
  `^22.22.2 || ^24.15.0 || >=26.0.0`을 root/workspace/lockfile에 고정한다. frontend/CI의
  Node 22.23.1과 C7의 Node 24.15.0은 이 지원 집합 안에 있다.
...
- `packages/kor-travel-map-admin/settings.py`: `KOR_TRAVEL_MAP_API_KOR_TRAVEL_GEO_BASE_URL`
...
- `pytest packages/kor-travel-map-api/tests -m "not live"`: 113 passed / 45 deselected.
...
- **테스트 경로**: test-strategy §5 e2e → `packages/kor-travel-map-api/tests/`.
...
- apihub는 `authKey`(apihub.kma.go.kr)로 인증 — data.go.kr `serviceKey`와 **별개
  키** → `settings.kma_apihub_key` (`KOR_TRAVEL_MAP_API_KMA_APIHUB_KEY`) +
...
`ApiSettings`에 추가. KMA 3 dataset (short / nowcast / ultra_short_forecast)
...
- 각 provider repo (`python-kma-api`/`python-opinet-api`/…)의 `.env`에 박힌
  키 이름을 그대로 가져오고, prefix `KOR_TRAVEL_MAP_API_`만 붙여 디버그 UI
  `.env`로 옮긴다.
- 예: `python-kma-api/.env`의 `KMA_SERVICE_KEY=...` → 디버그 UI의
  `.env`에 `KOR_TRAVEL_MAP_API_KMA_SERVICE_KEY=...`로 저장.
...
**Verification**:
- `pytest tests/ packages/kor-travel-map-api/tests/ --ignore=tests/integration
...
**신규 파일 — backend** (2):
- `packages/kor-travel-map-api/src/kortravelmap/api/etl_fixtures.py`
  (~340 line):
  ...
  - `list_providers()` / `list_datasets(provider)` / `run_fixture_preview(
    provider, dataset)`
- `packages/kor-travel-map-api/src/kortravelmap/api/routers/etl.py`
  (~150 line):
...
- `packages/kor-travel-map-api/src/kortravelmap/api/app.py` —
  `etl_router` include
- `packages/kor-travel-map-api/src/kortravelmap/api/routers/__init__.py`
  — re-export
- `packages/kor-travel-map-admin/frontend/src/app/page.tsx` — `/etl` 링크
- `packages/kor-travel-map-api/openapi.json` — drift gate baseline 재생성

**테스트**:
- `packages/kor-travel-map-api/tests/test_etl_routers.py` (13 case):
...
- `python packages/kor-travel-map-api/scripts/export_openapi.py --check`
  → exit 0

**디버그 서버 사용 흐름** (사용자가 지금 바로):
1. `pip install -e packages/kor-travel-map-admin` (PR#35 시점에 1회만 필요)
2. `uvicorn kortravelmap.api.app:app --host 127.0.0.1 --port 8087`
...
**Verification (local)**:
- `pytest tests/ packages/kor-travel-map-api/tests/ --ignore=tests/
...
- `packages/kor-travel-map-admin/frontend/.env.example` — default backend port
  `8600` → `8087` (PR#35 ApiSettings.port 기본과 정합)
...
**신규 파일** (7):
- `packages/kor-travel-map-api/src/kortravelmap/api/__init__.py` —
  namespace package + `__version__ = "0.2.0-dev"`.
- `py.typed` — PEP 561 marker.
- `settings.py` — `ApiSettings` (pydantic-settings, host/port/log_level/
  debug_routes_enabled, `KOR_TRAVEL_MAP_API_*` env prefix).
...
- `tests/test_routers.py` — pytest + httpx ASGI 테스트 6건 (200 OK / schema
  ...
  매칭).
- `packages/kor-travel-map-api/openapi.json` — drift gate baseline (FastAPI
...
- `packages/kor-travel-map-api/scripts/export_openapi.py` 신설 — ADR-031
...
- `docs/debug-ui-package.md` §2 디렉토리 + §14 신설 (~120 lines):
  ...
  - §14.1 기술 스택 (maplibre-vworld v1.0.0, ISC license, React 19, Vite,
    포트 8610)
  - §14.2 환경변수 (`VITE_VWORLD_API_KEY`, `VITE_KOR_TRAVEL_MAP_API_API`)
...
- `pyproject.toml` — `packages.find` (`kortravelmap*` + `namespaces=true`),
  ...
  coverage source.
- `packages/kor-travel-map-api/pyproject.toml` + `README.md` — namespace 정합.
...
- **ADR-022** `krtour` PEP 420 implicit namespace 채택 — `kor-travel-map`은
  `kortravelmap`으로 import, `kor-travel-map-admin`는 `kortravelmap.api`로
...
- `.env.example` — `KOR_TRAVEL_MAP_DEBUG_API_*` → `KOR_TRAVEL_MAP_API_*` 갱신 +
  주석.
- `docs/test-strategy.md` — e2e 코드 예시의 `from kortravelmap.api.app import ...`
  → `from kortravelmap.api.app import ...`.
- `packages/kor-travel-map-api/pyproject.toml` (신규) — 별도 패키지 pyproject.
