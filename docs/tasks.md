...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#213(T-202 pre-commit hook
정착)까지 merged. 이번 PR은 Sprint 5 운영 진입 gate 중 `T-204` GitHub branch
protection 설정 가이드를 닫는다.
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 hardening
전까지 구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은 `T-201b` Phase 2 정합성
게이트, `T-RV-19` admin UI 지도 선행 안정화, 또는 `T-RV-22/23/25` offline upload 후속이다.
...
- **T-RV-19, T-RV-22~T-RV-25, T-RV-27, T-RV-29~T-RV-35** — poi-cache list
  cursor/무한정 JSON, offline orphan/멱등/상태 drift, 노출, user OpenAPI 누출,
...
- [~] T-201 — **`ops.feature_consistency_reports` 도입** (T-201a Phase 1 ✅ 2026-05-29: alembic 0003 + `infra/consistency.py` F1~F3 관측; T-201b Phase 2 = F4~F8 + Dagster 게이트, Sprint 5)
  ...
  - T-201b 진행: F4 dedup backlog WARN은 Sprint 4b에서 추가됨. 2026-06-05 Codex는
    F6 opening_hours 모순 ERROR를 `run_consistency_checks()`에 추가했다. 남은 Phase 2
    케이스는 F5/F7/F8 + dry-run report.
