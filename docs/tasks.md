...
merged했고, T-RV-38/39 consistency count 의미 정리까지 완료됐다. 이번 PR은 `T-212a`
inventory/gap matrix를 닫는다.
...
- [ ] **T-DA-15** (MED, API 일관성, **DA-D-03 = 전면 통일**) list 응답 셰입 이원화
  ...
  - [x] `/admin/feature-update-requests` → `data.{items,next_cursor}` + `meta.{count,duration_ms}`
    (router/frontend hook+page/openapi/types 재생성, 2026-06-06).
  - [ ] `/admin/offline-uploads`, `/admin/poi-cache-targets` 잔여.
- [ ] **T-DA-16** (MED, API 일관성, **DA-D-03 = 전면 통일**) 단건 응답 envelope 불일치
  ...
  - [x] `/admin/feature-update-requests/{id}`·`/tripmate/feature-update-requests/{id}`
    → `{data, meta}` (2026-06-06).
  - [ ] `/admin/offline-uploads/{id}`, `/admin/poi-cache-targets/{id}`, `/ops/dagster/summary`,
    `/ops/metrics`, `/debug/mois-license/{id}`, `/ops/import-jobs/{job_id}`(meta 추가) 잔여.
...
- [~] T-209e — backup/restore 독립 DB 묶음(ADR-040 amend, D-5).
  ...
  복원하는 비파괴 cold restore 자동화를 고정한다. 남은 범위는 admin router,
  hot-swap UI다.
