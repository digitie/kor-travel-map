# TripMate 소비용 kor-travel-map REST API 매핑

> **상태**: 2026-06-12, ADR-048 / T-216a~g + curated_features 문서 계약 기준.
> **역할**: 이 문서는 TripMate가 어떤 kor-travel-map API를 소비하는지 정리하는 view다.
> 전 표면 REST 계약의 단일 정본은 [`docs/rest-api.md`](rest-api.md)이고, 기계 정본은
> `packages/kor-travel-map-admin/openapi.user.json` / `openapi.json`이다. 충돌 시 OpenAPI를 우선한다.
...
- `KOR_TRAVEL_MAP_ADMIN_SERVICE_TOKEN`이 설정된 환경에서는 service read `POST /v1/features/batch`
  호출에 `X-Kor-Travel-Map-Service-Token`을 붙인다. (`/tripmate/*` namespace는 제거됐다 — kor-travel-map은
  TripMate 전용이 아니다.)

## 2. TripMate 소비 표면

| TripMate 기능 | kor-travel-map API | 비고 |
|---------------|----------------|------|
...
| 테마형 curated trip plan import | `GET /v1/curated-features*` | T-223c-1부터 `openapi.user.json` 포함. `curated_features` → TripMate `curated_trip_plans` 1:1 복사 |
...
TripMate 측 질의 5건(해당 repo `docs/integrations/kor-travel-map-rest-api.md` §7)의 확정:

| # | 항목 | 확정 |
|---|---|---|
| 1 | review_mode | 기본 **`require_review`** 유지(kor-travel-map 최종 반영 권한 보존). 운영 합의 시 `KOR_TRAVEL_MAP_ADMIN_FEATURE_CHANGE_REVIEW_MODE=immediate`(전역) |
...
kor-travel-map `feature.curated_features` overlay가 소유한다. TripMate의 저장 정본명은
...
`packages/kor-travel-map-admin/openapi.user.json`을 따른다.
...
- kor-travel-map `curated_features` 1건 = TripMate `curated_trip_plans` 1건.
- 하위 장소·정류점·추천 POI는 TripMate `curated_plan_pois`로 복사한다.
- TripMate는 `source_system='kor-travel-map'`,
  `source_curated_feature_id`, `source_curated_feature_version`, `source_etag`,
  `source_imported_at`을 저장한다.
- TripMate는 kor-travel-map DB를 직접 읽지 않고 REST payload snapshot만 저장한다.
...
TripMate T-130(`/public/*`)은 비로그인 공개 API이며, 현재 차단 조건은 kor-travel-map
사용자 OpenAPI에 해수욕장/축제 전용 뷰와 닫힌 detail 스키마가 없다는 점이었다.
kor-travel-map 쪽 계약은 [`docs/public-views-api.md`](public-views-api.md)에 둔다.
