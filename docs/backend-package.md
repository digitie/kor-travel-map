...
디버그 REST API/UI는 **별도 Python 패키지** `kor-travel-map-admin` (ADR-020)에
...
class KorTravelMapSettings(BaseSettings):
    ...
    # admin/API 프로그램 기본 바인드 (패키지 쪽 KOR_TRAVEL_MAP_ADMIN_*가 우선)
...
요약:
- 별도 Python 패키지 `kor-travel-map-admin` (`packages/kor-travel-map-admin/`).
