...
>
> 파괴적 판정이 근거 부족이었다는 결론은 유지한다. 다만 이 자료가 증명하는 것은
> **기존 규칙으로 좌표 오류가 성립한 건이 0건**이라는 점이지, 일반적인 의미의
> “좌표 오류가 0건”이라는 점이 아니다. 코드 축은 위치 검증이 아니라 **producer 캐시
> staleness 검출**로만 쓴다.
...
| 분류 | 건수 |
| --- | --- |
| `code_cache_same` — payload 시군구코드 == geo 시군구코드 | **380** |
| `code_cache_diff` — payload 시군구코드 != geo 시군구코드 | **0** |
...
- 후보 집합 전체(1,477건)로 넓혀도 payload 코드와 geo 코드가 **불일치하는 건은 0건**이다
  (일치 1,424 / 코드 없음 53).
- reverse 최근접 거리: `<10m` 210건, `<100m` 136건, `<1km` 34건. 이는 같은 reverse
  시스템 내부의 관측값이므로 좌표 정확성을 독립적으로 증명하지 않는다.

즉 코드 축에서는 기존 error 380건 중 위치 오류를 입증한 건이 없으며, 이 축으로 영구
drop을 정당화할 수 없다.

## 2-bis. provider 텍스트 ↔ 좌표 reverse 이름 축

provider가 쓴 주소 문자열은 geo에서 유래하지 않아 payload 행정코드 캐시와 독립이다.
그러나 비교 대상인 `geo_sigungu_name`은 좌표 reverse 결과이므로 **좌표와 독립인 축은
아니다**. 이 축으로 380건을 다음처럼 분류했다.

| 건수 | 판정 | 근거 |
| --- | --- | --- |
| **375** | 판정 불능 | 텍스트에 시/군/구 토큰이 **아예 없다**(`부산 기장 조방국밥`). 좌표가 맞거나 틀렸는지와 무관하게 기존 부분문자열 검사는 통과할 수 없다 |
| **4** | 동일 어간 | provider 텍스트와 reverse 이름의 축약·단계 표기가 다르다(`양구읍` vs `양구군`) |
| **1** | 확인됨 — 경계 | `서울 서대문구 통일로 251`. 텍스트를 **정지오코딩**하니 서대문구, 후보 좌표에서 **143 m**. 좌표가 맞고 종로구 경계에 걸린 것 |

375건은 텍스트만으로 기존 규칙이 판정 불능임을 알 수 있고, 4건은 reverse 이름과 같은
어간이다. 남은 1건은 제3의 수단인 정지오코딩으로 경계 사례임을 확인했다. 따라서
**기존 규칙의 불일치 근거가 성립한 후보는 0건**이다. 이는 380건 전부의 일반적 좌표
정확성을 증명한다는 뜻이 아니며, 해당 규칙으로 파괴적 drop을 할 근거가 없다는 뜻이다.
...
이 분류만으로 어느 쪽이 맞는지는 결정할 수 없다. `서울 서대문구 통일로 251` 한 건은
정지오코딩으로 경계 사례임을 별도 확인했지만, payload 코드와 reverse 코드가 같다는
사실만으로 주소 문자열이 틀렸다고 결론내리지 않는다.
...
`h28a_final.py`는 export 전량을 페이징 수집하고, 운영과 같은 변환을 돌린다. 현재 validator의
severity로 후보를 고르지 않고, `provider_address_mismatch@8073d0d4`로 버전 고정한 기존
선별식을 직접 실행해 380건 baseline을 재생성한다. 출력은 baseline 결과와 현재 validator
결과를 분리하며, `upserts == bundles + quarantine` 보존 불변식과 quarantine 식별자·사유도
기록한다.
...
- **380건 전량 회복, 손실 0.**
- upsert 보존 불변식 **1,477 = bundle 1,477 + quarantine 0**.
...
- 행정코드 staleness warning **0건**. 이는 좌표 정확성 판정이 아니다.
...
1. 기존 규칙의 severity를 `error`(영구 drop)로 둘 근거가 없다. baseline 380건 중 해당
   규칙으로 위치 오류를 입증한 건은 0건이다.
2. payload 행정코드와 좌표 reverse 코드는 **위치 검증축이 아니라 staleness 관측축**으로
   비교한다. 두 값을 병합 전에 보존해야 캐시 변화를 관측할 수 있다(§4).
3. provider 텍스트 ↔ reverse 이름 축은 토큰이 있을 때 warning으로만 관측하고, 후보집합과
   판정 불능 상태를 함께 집계한다.
