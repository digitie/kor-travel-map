...
- [ ] T-VN-H33 — **curation_items 오링크 3건 정리 (H25B 파생)**
  ...
  **오탐 유형이 계통적임이 확인됐다** — MOIS 인허가에 관광지 이름을 단 **서울 사무소·본사**가
  다수 있고 이름 매칭이 여기 붙는다: 남이섬·호미곶·오륙도가 모두 `서울특별시 중구` 주소의
  feature를 갖는다(H25B 리포트 §5). 3건만이 아니라 **이 유형 전수**를 대상으로 삼아야 한다.
  탐지 착안점: curation item의 `metadata_json.region`과 링크된 feature의 `sido_code` 불일치.
  전수 확인 후 unlink하고 공개 projection 노출 여부를 실증한다.
