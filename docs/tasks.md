...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#231(T-201b-d F8 file object
orphan 정합성 검사)까지 merged. 이번 PR은 `T-RV-29/30`으로 TripMate/user
OpenAPI에서 admin write 경로를 제거하고 frontend generated OpenAPI type drift를
막는다.
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 hardening
전까지 구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은
`T-RV-31/32/33` router/executor 정확성 묶음 또는 `T-RV-34/35` Dagster 실행 품질
묶음이다. `T-209b-a`(Dagster schedule storage PostgreSQL 전환)는 별도 백로그로
등록되어 있으나, 현재 Codex 진행은 사용자 지시에 따라 남은 `T-RV-*`를 우선 닫는다.
