...
| **krex** | 고속도로 휴게소 | — | — | ❌ | — | upstream 라이브러리 파싱 에러(§4) |
...
## 4. 발견 — provider 라이브러리 측 이슈 (본 lib 대상 아님, ADR-044)

**krex(`python-krex-api`) 휴게소 list 파싱 실패.** `client.restarea.list_all` 호출 시
`KrexParseError: restAreaNm/serviceAreaName is required`. raw 응답 직접 확인 결과
data.go.kr `tn_pubr_public_rest_area_api`의 휴게소명 필드가 **`entrpsNm`**("강릉(강릉)")
인데, krex 라이브러리는 `restAreaNm`/`serviceAreaName`을 요구한다 — **upstream API의
필드명 변경에 라이브러리가 미추종**.

- 책임: data 정합성 1차 책임은 provider 라이브러리(ADR-044) → `python-krex-api`에서
  `entrpsNm` alias 추가가 맞음(필요 시 upstream PR). 본 lib `rest_areas_to_bundles`
  변환 자체는 정상(데이터만 받으면 동작) — 어댑터로 WGS84 lon/lat 매핑 확인됨.
- 본 lib `KrexRestAreaItem` Protocol엔 `uni_id`가 필요하나 `RestArea` 모델엔 없어
  자연키 합성 필요(휴게소명+노선+방향) — `::` 구분자(ADR-009, `|` 금지).
...
- **krex**: `python-krex-api`의 `entrpsNm` 필드 미추종 해소 후 재검증.
...
opinet/krheritage/datagokr/kma 4종 provider의 변환·적재 경로가 **실데이터로 정상
동작**함을 확인(좌표 변환·5179 generated·place/event/notice 분기 모두 검증). 실데이터
검증으로 notice alias map 갭 1건을 발견·수정했다. krex는 upstream 라이브러리의 필드명
미추종(ADR-044 provider 책임), krforest는 본 lib 미구현으로 제외.
