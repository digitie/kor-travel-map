...
- **geocoding (kraddr-geo v2 연동)** (`krtour.map.geocoding`) ✅ — 좌표↔주소
  ...
  비동기 콜러블 팩토리 `kraddr_geo_{reverse,address}_geocoder`. **남은 wiring**:
  적재 함수가 geocoder resource로 Feature 자동 보강(`docs/address-geocoding.md
  §7`) — knps/datagokr place의 bjd_code 미상(feature_id 'global' bucket) 해소용.
