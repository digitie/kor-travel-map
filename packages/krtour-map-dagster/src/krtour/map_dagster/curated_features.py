...
_RESOURCE_KEYS: Final[set[str]] = {"krtour_map_client"}
_GROUP_NAME: Final[str] = "curated_features"

CURATED_FEATURE_JOB_TAGS: Final[dict[str, str]] = {
    "krtour_map.job_scope": _GROUP_NAME,
    "krtour_map.job_kind": "curated_features_refresh",
...
def _client(context: AssetExecutionContext) -> "AsyncKrtourMapClient":
    resources = cast(Any, context.resources)
    if not hasattr(resources, "krtour_map_client"):
        raise AttributeError("Dagster resource 없음: krtour_map_client")
    return cast("AsyncKrtourMapClient", resources.krtour_map_client)
