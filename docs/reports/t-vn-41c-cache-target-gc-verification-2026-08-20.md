...
- 재실행: `scripts/verify-tvn41c-cache-target-gc.sh` (n150에서
  ...
  실행 시각 2026-08-20 08:07~08:11 UTC, 소스 `f9f72b3d`.
...
| ③ | 수동 GC | 적격 **56/2,800 전부 삭제**, 대조군 24 **불변**, `remaining_*` 0, 4 batch, 27,026 items/s |
| ④ | 처리량 > 유입률 | 유입 **14,439 items/s** vs GC **54,795 items/s** (약 3.8배), 18 batch, remaining 0 |
| ⑤ | schedule ON → tick | cron override 반영 확인 후 STOPPED→start, **t+36초에 run 생성 · t+41초 SUCCESS**, backlog 42/2,100 → **0/0**, tick 1건 |
...
[증가율만  ] alert=true  reasons=[item_growth, header_growth]   headers_growth_per_hour=8,404
```
