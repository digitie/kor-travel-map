...
--   시그니처는 안 바뀐다 — IN/OUT 어디에도 feature 식별자가 없다.
--   본문에서 옮긴 것은 두 자리뿐이다.
--
--   1) reconcile scope 비교의 `link.feature_id` → `link.feature_id::text` (3곳).
--      `ops.curation_rule_reconcile_scope_members.member_key`는 source_entity 키와
--      feature 키를 함께 담는 이종 컬럼이라 309가 재타입하지 않는다
--      (alembic/head-schema.sql:16713, text). 캐스트가 없으면 안쪽 UNION이
--      `42804: UNION types text and uuid cannot be matched`로 죽고, 그 공통타입이
--      그대로 나가는 바깥 EXCEPT도 함께 깨진다. 생산자
--      `_309_create_curation_rule_reconcile_receipt.sql`이 member_key를 같은
--      `link.feature_id::text`로 적재하므로 두 쪽 표기가 갈라지지 않는다.
--   2) `v_feature_id` text → uuid. 이 루프가 도는 UNION의 두 갈래
--      (`theme_feature_candidates.feature_id` · `source_links.feature_id`)가 모두
--      uuid다. 락 키 `'feature-write:' || v_feature_id`는 `text || anynonarray`로
--      성립하고 결과가 uuid의 canonical 소문자 표기라 문자열이 예전과 같다.
--
--   `feature.current_theme_candidate_snapshot(uuid, text, uuid)` 7개 호출은
--   3번째 인자로 `link.feature_id`(uuid)를 넘기므로 새 시그니처와 맞는다.
-- ─────────────────────────────────────────────────────────────────────────
SET ROLE ktm_curation_command_owner;

DROP PROCEDURE feature.materialize_theme_candidate_generation(uuid, text, uuid, uuid, bigint, text, jsonb);
