...
| `admin-features` | `/admin/features` | feature 검색/수동 추가/비활성화/삭제 |
...
## 8. TripMate 연동 시 확장 후보

초기 admin OpenAPI가 안정된 뒤 TripMate에는 다음 API를 우선 제공한다.

| API | 목적 |
|-----|------|
| `GET /features` 또는 `/tripmate/features` | bbox 기반 사용자 지도 feature |
| `GET /features/{feature_id}` | feature 상세 |
| `POST /tripmate/features/batch` | 여러 feature_id 상세 batch 조회 |
...
admin audit log를 노출하지 않는다. 이 제한이 필요해지는 시점에 `/tripmate/*` prefix를
분리한다.

모든 feature 목록/상세 응답에는 KST aware `last_updated_at`을 포함한다. 목록 API는
JSONB detail/raw payload를 반환하지 않고, 특정 feature 상세 API에서만 추가 JSON
데이터를 반환한다.
