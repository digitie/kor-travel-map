...
그리고 prod `.env`에는 **아무 데서도 참조되지 않는**
`NEXT_PUBLIC_KOR_TRAVEL_GEO_API_KEY`가 남아 있다. 지금 geo에 걸면 `401 E0401`이다.
PR #979의 정적 가드가 "이 축은 못 본다"고 적어 둔 바로 그 자리이고, 실제로 오염돼
있었다. 가드는 저장소가 잘못된 값을 권하지 않게 할 뿐이다.

확인하다 한 번 헛짚었다 — geo `/v2/reverse`는 키를 **query 파라미터**로 받는데 본문에
넣어서 인증 이전에 `400`으로 떨어졌고, 하마터면 유효한 키를 무효로 판정할 뻔했다.
