"""``kortravelmap.cli.main`` — ``kor-travel-map`` CLI entry-point (ADR-022/039).
...
주입받는다. ``--geocoder-url``을 주면 좌표 → bjd_code 보강(kor-travel-geo REST)을 켠다.

engine은 ``KorTravelMapSettings.pg_dsn``에서 만들고 호출 종료 시 dispose한다
(ADR-004 — 호출자 소유). DSN은 ``--dsn`` 또는 ``KOR_TRAVEL_MAP_PG_DSN`` 환경변수.
...
from kortravelmap.infra.merge_repo import MergeError
...
def build_parser() -> argparse.ArgumentParser:
    """``ktmctl`` argparse 파서 구성."""
    parser = argparse.ArgumentParser(
        prog="ktmctl",
        description="kor-travel-map 운영 CLI (지도 feature 적재/조회).",
    )
    parser.add_argument(
        "--dsn",
        default=None,
        help="PostgreSQL async DSN (미지정 시 KOR_TRAVEL_MAP_PG_DSN 환경변수/기본값).",
    )
    ...
    mois_p.add_argument(
        ...
        default=None,
        help=(
            "kor-travel-geo REST base URL(예: http://127.0.0.1:12501). 주면 좌표 → "
...
async def _geocoders_for(
    ...
    """``--geocoder-url`` → kor-travel-geo geocoder 콜러블들 (없으면 ``None``).

    httpx client 수명은 본 컨텍스트가 소유한다(ADR-002). ``base_url``엔 ``/v2``를
    붙이지 않는다 — ``KorTravelGeoRestClient``가 ``base_path``로 붙인다.
    """
    if base_url is None:
        yield None, None
        return
...
async def _cmd_import_mois(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        async with (
            ...
            AsyncKorTravelMapClient(engine) as client,
        ):
