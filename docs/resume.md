...
**다음 한 작업: `T-VN-QUEUE-QUOTA` — 남은 **셋**의 분모를 포털에서 본다**
(`krheritage`·`krex`·`mois`). OpiNet은 2026-09-14에 봤고 **저장소가 5배 틀려
있었다**(무료키 300/일인데 1,500으로 알고 예산 600을 세웠다 — 켜면 첫날에 막혔다). `krheritage`·`opinet`·`krex`·`mois`는 data.go.kr이 아니라 각자 포털에 있고
