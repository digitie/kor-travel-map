...
- **TM-08 — 장소 출처/신뢰도 표시 UX** 🟡
  ...
  - 노출 정책(검수 전 후보 표시 여부)은 의사결정 D-05 결과를 따른다.
...
- **TM-12 — admin 표면 축소** (의사결정 D-06 승인 시)
  ...
  - `/admin/etl`·`/admin/seed`·`/admin/reset` placeholder 제거 (`apps/etl` 레거시는
    krtour T-210c와 함께 정리).
  - `/admin/category-mapping` 제거 또는 `GET /v1/categories` read-only 뷰로 대체
    (카테고리 정본 이원화 방지).
- **TM-13 — FeatureSuggestion 릴레이 경로 확정** (의사결정 D-02 결과 반영)
  - 현재 사용자 제보가 TripMate DB에서 종착. 결정된 릴레이 방식(권장: krtour 서비스용
    수신 API)으로 배선하거나, 결정 전까지 UI에 "운영 검토 후 반영" 안내 명시.
