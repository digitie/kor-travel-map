...
TripMate OpenAPI 연동**이다. TripMate는 kor-travel-map DB에 직접 접근하지 않고,
`kor-travel-map`을 운영 코드에서 직접 import하지 않는다. TripMate ↔ kor-travel-map
사이는 OpenAPI 기반 HTTP 계약으로 연결한다.

REST/OpenAPI backend는 **별도 Python 패키지** `kor-travel-map-api`로 분리되어 있다
(ADR-055). 본 monorepo 안의 `packages/kor-travel-map-api/`에 위치하고, admin UI는
`packages/kor-travel-map-admin/frontend/`가 소유한다. API/Dagster 내부에서는 메인
라이브러리를 import해서 `AsyncKorTravelMapClient`를 호출한다. 별도 인증 키를 요구하지
않는다(내부망 전용, ADR-005). 단 service-read `POST /v1/features/batch`에는 opt-in
`ServiceToken`(`X-Kor-Travel-Map-Service-Token`) 검증 예외가 있다(ADR-005 D-1(2026-06-08) —
`service_token` 미설정 시 통과). 정본 표는 `docs/integration-map.md` §3.
...
> **T-226 package identity(ADR-054)**: public 배포명은 `kor-travel-map`, Python import root는
> `kortravelmap`, 권장 예시는 `import kortravelmap as ktm`로 clean cut한다. CLI 목표명은
> `ktmctl`, PostgreSQL 기본 DB는 `kor_travel_map`, RustFS bucket/prefix는
> `kor-travel-map` 계열로 둔다.
> 형제 프로젝트 표시는 `kor-travel-geo`, `kor-travel-concierge`,
> `kor-travel-docker-manager` 기준으로 맞춘다. 전환 정본은
> `docs/package-identity-rename.md`.
...
| Python import (REST API) | `from kortravelmap.api import ...` |
| CLI 명령 | `ktmctl ...` |
| 환경변수 prefix | `KOR_TRAVEL_MAP_*` (env는 underscore 표준 유지) |
| PostgreSQL DB 이름 (개발/운영 기본) | `kor_travel_map` (ADR-045 — TripMate 공유 DB 아님) |
...
| ADR accepted | 001~059 전부 (text on main). 029는 ADR-043으로 supersede. ADR-045는 Docker 독립 프로그램 + OpenAPI 연동으로 ADR-003 운영 모델을 supersede. ADR-046은 ADR-045 이행 시 구 모델 호환 shim 금지. ADR-047은 docker-manager 로컬 포트 기준(API 12701, admin UI 12705, Dagster 12702, kor-travel-geo API 12501). ADR-048은 `/v1` 전 표면 clean cut + REST envelope/parameter/error/명명 정합성 표준. ADR-053은 `kor-travel-concierge` provider identity clean cut과 TripMate 직접 연동 제거 경계. ADR-054는 `kor-travel-map` / `kortravelmap` package identity clean cut. ADR-055는 `kor-travel-map-api` Python backend와 `kor-travel-map-admin` frontend 분리. ADR-056은 `pinvi` T-108에서 streaming replication을 제외하고 N150/Odroid multi-platform Docker build를 운영 자동화로 고정. ADR-057은 kor-travel-concierge feature_id를 안정 `candidate.id`에 고정(bjd/category를 식별자에서 분리, 재export 중복 방지). ADR-058은 geocoder를 필수화해 geocoder-의존 provider feature_id 결정성을 보장(F-01, re-key 없음). ADR-059는 벤더링 agent/skill 원문의 언어·context-discovery 예외 정책(#452 #450). |
| ADR proposed | (없음) — 다음 후보 번호는 ADR-060. |
| Sprint plan | `docs/sprints/SPRINT-1.md` ~ `SPRINT-5.md` |
| Provider 구현 순서 (ADR-034) | 축제→날씨→유가→휴게소→국립공원/트래킹→국가유산→**MOIS**→휴양림/수목원→박물관/미술관 |

## 개발 환경 정책 (PC, WSL)

PC 개발 source of truth는 **NTFS (`F:\dev\kor-travel-map*`)** worktree다. 단, 2026-06-12 이후 agent 작업 실행 위치는 다음처럼 강제한다: **순수 `git` 명령만 Windows/NTFS host에서 실행**하고, 파일 탐색·문서/코드 수정·테스트·lint·build·Docker·Python/Node/npm·`gh`/GitHub CLI 등 나머지 모든 작업은 **WSL에서 `/mnt/f/dev/kor-travel-map-<agent>` 경로로 실행**한다. 디버그 UI Playwright e2e는 예외로 **Windows 호스트에서만 실행**한다(서버는 WSL). 형제 라이브러리 (`kor-travel-geo` / `python-kraddr-base` / `python-knps-api` 등)도 동일 정책을 따른다.

- **코드/가상환경**: NTFS (`F:\dev\kor-travel-map/`)에 보관하되, agent의 shell 작업은 WSL `/mnt/f/dev/kor-travel-map-<agent>`에서 실행한다. Windows host shell에서는 순수 `git` 명령만 허용한다.
- **데이터(`data/`)**: NTFS의 프로젝트 디렉토리 (`F:\dev\kor-travel-map\data\`).
- **테스트/실행/Docker**: Python/Node 테스트, lint, type-check, Docker compose/build/smoke는 모두 WSL에서 실행한다. **단, 디버그 UI Playwright e2e는 서버(backend `:12701` + frontend `:12705`)만 WSL에 띄우고 Playwright(chromium)는 Windows 호스트에서 실행한다** — WSL에 chromium system lib(`libasound.so.2` 등)가 없고 sudo가 비밀번호를 요구해 자동 설치가 불가하기 때문. 자세히는 `docs/dev-environment.md` §8.1 + `packages/kor-travel-map-admin/frontend/README.md` §"e2e (Playwright)".
- **카피 정책**: 필요 시 테스트 실행 및 배포 전 NTFS -> WSL ext4로 rsync 하여 동기화한다. 현재 agent 기본은 NTFS worktree를 WSL `/mnt/f` 경로로 직접 다루는 방식이다. Git의 source of truth는 NTFS다.
...
- 메인 repo 디렉토리(`F:\dev\kor-travel-map\`)의 **형제로** worktree 생성:
  `git worktree add -b sandbox/claude ../kor-travel-map-claude main`.
...
```bash
git.exe -C F:/dev/kor-travel-map-claude fetch
git.exe -C F:/dev/kor-travel-map-claude switch main
git.exe -C F:/dev/kor-travel-map-claude pull --ff-only
git.exe -C F:/dev/kor-travel-map-claude switch -c feat/<topic> main
codegraph sync              # 인덱스 증분 동기 (init 아님)
# ... 작업 / 필요 시 NTFS -> WSL rsync 후 pytest / ruff / mypy / lint-imports ...
git.exe -C F:/dev/kor-travel-map-claude push -u origin feat/<topic>
...
본 라이브러리는 kor-travel-map API/Dagster 내부에서 직접 호출되는 **Python API**를
...
## TripMate ↔ kor-travel-map 경계 (ADR-045)
...
- **TripMate는 kor-travel-map을 OpenAPI로 호출한다**. 운영 코드에서
  `kor-travel-map`을 직접 import하지 않는다.
- TripMate는 kor-travel-map PostgreSQL/PostGIS DB에 직접 연결하지 않는다.
...
라이브러리 `kor-travel-map`은 FastAPI/Uvicorn 의존이 없다.

- **위치**: `packages/kor-travel-map-api/src/kortravelmap/api/` (본 monorepo
  내, 별도 `pyproject.toml`).
...
- **인증**: 별도 키 없음 (ADR-005 그대로). 네트워크 계층(Cloudflare Tunnel /
  SSO 게이트웨이 / IP allowlist)에서 보호. 코드에 인증 로직 침투 금지. 유일한
  sanctioned 예외는 service-read `POST /v1/features/batch`의 opt-in `ServiceToken`
  이다(ADR-005 D-1(2026-06-08), 미설정 시 통과 — 정본 표 `docs/integration-map.md` §3).
- **메인 라이브러리 의존**: `pip install -e packages/kor-travel-map-api`만
  설치하면 `kor-travel-map`을 자동으로 의존성으로 가져간다.
- **TripMate는 Python 의존하지 않는다** — TripMate는 OpenAPI client만 사용한다.
- `KOR_TRAVEL_MAP_API_HOST` 기본 `127.0.0.1`. `0.0.0.0` 바인드 시 경고 로그.
...
- **지도**: `maplibre-vworld-js` 별도 라이브러리(현재 v0.1.3, ADR-036). 공통
  기능은 상류, TripMate 전용 확장만 본 저장소(`packages/kor-travel-map-admin/
...
1. **의존 방향 역행 금지** — 메인 패키지: `category → dto → core → infra →
   geocoding → providers → client → cli` 한 방향. `import-linter`가 CI에서 강제.
   `kortravelmap.api`는 존재하지 않는다 (ADR-020 — 디버그 REST는 별도 패키지).
2. **동기 인터페이스 추가 금지** — `AsyncKorTravelMapClient`만. 동기는 호출자가
...
14. **디버그 API/UI 패키지에 신규 인증(JWT/OAuth 등) 추가 금지** — 내부망 전제.
    외부 노출이 필요해지면 네트워크 계층(SSO 게이트웨이/IP allowlist/Cloudflare
    Tunnel)에서 보호. 코드/응답에 인증 로직 침투 X. 유일한 sanctioned 예외는
    기존 service-read `POST /v1/features/batch`의 opt-in `ServiceToken`이다
    (ADR-005 D-1(2026-06-08)).
15. **메인 라이브러리(`kortravelmap`)에 FastAPI/Uvicorn import 금지** — ADR-020.
    HTTP 서버 코드는 `packages/kor-travel-map-api/`에만.
...
18. **`from kor_travel_map import ...` (flat) 사용 금지** — 항상 `import kortravelmap
    as ktm` 또는 `from kortravelmap import ...` (ADR-054). `src/kor_travel_map/`
    디렉토리 만들지 말 것.
19. **`src/krtour/` namespace 부활 금지** — T-226 이후 `kortravelmap`이 유일한
...
20. **CLI 중복 실행이 위험한 명령에 mutex 없이 머지 금지** (ADR-039 accepted) —
    ...
    은 예외. lock key: `hash(f"kor-travel-map:{command}:{scope}")`.
21. **`@kor-travel-map/map-marker-react` npm registry 게시 금지** (ADR-043) —
...
> ↔ kor-travel-map 경계" / "디버그·관리 REST API 정책" / "Frontend stack" 절,
> `CLAUDE.md §2`, `docs/decisions.md`를 정본으로 본다. 패키지는
> `kor-travel-map-api` Python backend와 `kor-travel-map-admin` frontend로 분리 완료
> (ADR-055), 구 이름/env/import 호환 shim은 만들지 않는다(ADR-046).

**계속 유효한 코드 작성 가이드**:
- 모든 신규 코드는 `import-linter` 의존 방향 (`category → dto → core →
  infra → geocoding → providers → client → cli`) 준수.
- `src/krtour/` namespace는 **절대 되살리지 말 것**. T-226 이후 `kortravelmap`이
  유일한 import root이며, `tests/lint/test_package_layout.py`가 차단한다.
