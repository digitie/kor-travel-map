...
(ADR-066~075). **2026-07-26 사용자 지시(서비스 전 단계)**: prod 환경 보전·호환성·기존 문서
계약·최소 수정은 **비제약** — 필요 시 DB 스키마·문서 계약 수정 가능. lane 규율·상세는
`docs/tasks.md` §공통 규율(2026-07-26 개정)이 정본.
...
| ADR 현황 | 상태·색인·**다음 후보 번호** 모두 `docs/adr/README.md` 상단이 정본 (여기 박지 않는다 — 반복 drift 회피, DA-D-01) |
| Sprint plan | `docs/sprints/SPRINT-1.md` ~ `SPRINT-5.md` |
| Provider 구현 순서 (ADR-034) | 축제→날씨→유가→휴게소→국립공원/트래킹→국가유산→**MOIS**→휴양림/수목원→박물관/미술관 |

## 개발 환경 정책 (Linux/WSL)

핵심 규칙: 모든 개발 명령은 Linux 환경에서 실행한다. Windows PC에서는 WSL
`/mnt/f/dev/kor-travel-map-<agent>`를 기본 실행 위치로 쓰며, 과거에 예외였던
`git`/`gh`/`codegraph`도 Linux `git`/`gh`/`codegraph`로 실행한다. 파일 탐색·코드/문서
수정·테스트·lint·build·Docker·Python/Node/npm도 모두 Linux/WSL에서 수행한다.
디버그 UI Playwright e2e는 n150 Linux 환경에서 우선 실행하고, n150에서 실행할 수 없을 때만
Windows 호스트 브라우저를 fallback으로 사용한다(서버는 Linux/WSL). NTFS worktree는 보관
위치일 뿐 실행 경계가 아니다. 절차 정본은 `docs/dev-environment.md`, 복구는
