...
### 0.1 일회성 DB 컨테이너 이름 규약

검증용으로 PostgreSQL 컨테이너를 손으로 띄울 때가 있다. **`ktm-` 접두어를 쓰지 마라.**

2026-08-17에 `ktm-tvn36-db`(:18736)를 정본 prod DB로 착각한 채 권한 감사를 돌려
...
- 며칠 남겨야 하면 이름에 **종료 예정일**을 넣는다(예: `tmp-tvn36-db-until-0820`).
- `restart` 정책은 `no`다. 다만 재부팅이 드물면 실질적인 수명 제한이 못 된다 —
  실제로 `restart=no`인 컨테이너가 **9일**을 버텼다.

prod 컨테이너 이름은 `AGENTS.md` §식별자가 정본이다. 그 목록에 없는 이름이
`docker ps`에 보이면 일회성이라는 뜻이어야 한다.
...
#### 8.1.1 n150에서 live e2e 표준 실행 경로 — **고정 Playwright executor**
...
[`C7 production live E2E 실행·복구`](runbooks/c7-prod-live-e2e.md)가 정본이다.
