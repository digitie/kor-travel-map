...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#236(`T-209b-a` Dagster
schedule/run/event storage PostgreSQL 전환)까지 merged했고, 현재 Codex 작업은
T-RV 실행 품질 묶음(`T-RV-34/35`)을 닫는다. `T-RV-27`(admin API bind 노출 정정)은
사용자 결정에 따라 production 레벨 hardening 전까지 구현하지 않고 문서 추적만 유지한다.
...
**T-RV-04b(provider live fetcher wiring)** 또는 새 리뷰 백로그 **T-RV-38~41**다.
...
- ~~**T-RV-23** offline upload checksum idempotency + load TOCTOU.~~
  ...
  Failure로 기록한다.
- **T-RV-27** — admin API bind 노출(deferred). production hardening gate 전까지
  문서 추적만 유지한다. (리포트 §2)
