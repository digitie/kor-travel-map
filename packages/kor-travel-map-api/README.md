...
| `NEXT_PUBLIC_KOR_TRAVEL_GEO_API_KEY` | `KOR_TRAVEL_MAP_KOR_TRAVEL_GEO_API_KEY`와 동일 값 | kor-travel-geo v2 `key` query 값. geo가 소비자별로 발급하는 **별도** key다 — VWorld 키로는 인증되지 않는다(401 `E0401`). production에서는 명시 필수 |
