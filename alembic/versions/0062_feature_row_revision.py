...
별도 autocommit_block에서 ``VALIDATE``해 ACCESS EXCLUSIVE 스캔을 피한다.
...
def upgrade() -> None:
    ...
    # 2) CHECK는 NOT VALID로 먼저 붙이고(짧은 ACCESS EXCLUSIVE, 정의만 기록) 아래
    #    autocommit_block에서 VALIDATE한다(SHARE UPDATE EXCLUSIVE, read/write 비차단).
    op.execute(
        """
    ...
    # 4) VALIDATE는 autocommit_block에서. block 진입이 위 DDL을 먼저 commit해
    #    ACCESS EXCLUSIVE를 풀고, VALIDATE는 자기 statement에서 비차단 스캔만 한다.
