"""``kortravelmap.admin.settings`` — 디버그/관리 UI runtime 설정.

``Pydantic Settings`` 기반. ``KOR_TRAVEL_MAP_ADMIN_*`` 환경변수 prefix.
...
본 lib도 ``KOR_TRAVEL_MAP_ADMIN_KMA_SERVICE_KEY``로 박는다 (Pydantic Settings
...
class AdminSettings(BaseSettings):
    """디버그/관리 UI 백엔드 설정 (`KOR_TRAVEL_MAP_ADMIN_*` env prefix).
    ...
    model_config = SettingsConfigDict(
        env_prefix="KOR_TRAVEL_MAP_ADMIN_",
        env_file=".env",
    ...
    service_token: SecretStr | None = Field(
        ...
        description=(
            ...
            "operator는 proxy SSO). env ``KOR_TRAVEL_MAP_ADMIN_SERVICE_TOKEN``."
        ),
    )
    admin_destructive_enabled: bool = Field(
        ...
        description=(
            ...
            "``KOR_TRAVEL_MAP_ADMIN_DESTRUCTIVE_ENABLED``."
        ),
    )
    ...
    #
    # 각 provider repo의 `.env`에 박힌 이름과 동일 (prefix `KOR_TRAVEL_MAP_ADMIN_`
    # 만 추가). 예: `python-kma-api`의 `.env`가 ``KMA_SERVICE_KEY=...``라면 본
    # lib는 ``KOR_TRAVEL_MAP_ADMIN_KMA_SERVICE_KEY=...``로 박는다. 미설정 시
