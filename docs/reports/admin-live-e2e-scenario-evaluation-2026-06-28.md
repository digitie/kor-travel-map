...
Admin UI의 주요 기능을 실제 n150 live 환경에서 끝까지 검증하기 위해, route/UI 조작/API 반영/교차 화면 반영을 함께 다루는 시나리오 catalog와 write live spec을 보강했다. 데이터 유실 위험보다 실제 반영 확인을 우선하는 요청에 맞춰, `read-only` smoke에 머물지 않고 feature change request와 Settings API key/audit 흐름에는 실제 write를 포함했다.

## 시나리오 범위

- 논리 시나리오 catalog: 13,651건
...
- 실제 destructive backup/restore/swap 실행은 기존 opt-in guard가 꺼져 있어 full run에서는 skip했다. plan/invalid id/UI command surface는 검증하고, execute 계열은 `E2E_BACKUP_RESTORE_EXECUTE` 계열 opt-in이 필요하다.
