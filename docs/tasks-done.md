...
- [x] **T-215a — place/event feature 추가·수정·삭제 admin API + versioning.**
  ...
  `KOR_TRAVEL_MAP_ADMIN_FEATURE_CHANGE_REVIEW_MODE=require_review|immediate` 설정에 따라
...
- [x] **T-215b — admin UI feature change queue 화면.** (2026-06-09)
  ...
  `review_mode`를 추가해 `KOR_TRAVEL_MAP_ADMIN_FEATURE_CHANGE_REVIEW_MODE` 현재값을 빈 큐에서도
...
- **T-207g** (2026-06-03): OpenAPI export를 admin 전체
  `packages/kor-travel-map-admin/openapi.json`과 TripMate/user subset
  `packages/kor-travel-map-admin/openapi.user.json`으로 이원화했다. CI drift gate는
...
- **PR#158** (merged 2026-06-02): Docker API 컨테이너의 Dagster URL을
  `KOR_TRAVEL_MAP_DOCKER_ADMIN_DAGSTER_URL` 기본값(`http://dagster:12302`)로 분리.
...
- [x] T-012b — ADR-030/031/032/033 enforcement 코드 (완료: 2026-05-25, PR#10
  ...
  - `pyproject.toml`: import-linter 차단 계약 (cachetools/async_lru/
    ...
    Sprint별 schedule 주석
  - `packages/kor-travel-map-admin/scripts/export_openapi.py` skeleton
