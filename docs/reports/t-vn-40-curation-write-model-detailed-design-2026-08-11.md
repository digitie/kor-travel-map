...
5. PR #978 squash 뒤 active graph는
   `0200_schema_baseline→0104_tvn36_final_fence(bridge)→0202…0220`만 존재한다. bridge의 revision id는
   기존 n150 head와 같으므로 application DB를 stamp하거나 baseline으로 재생성하지 않는다. writer fence와
   최종 backup receipt를 확인한 뒤 기존 DB는 `0202…0220`만 forward upgrade하고, fresh DB만
   `0200→bridge→0202…0220` 전체를 실행한다. 두 경로 모두 runtime ACL reconciliation과 provider 증거
   검증 뒤 writer를 재개한다.
