...
merged했고, T-RV-38/39 consistency count 의미 정리, DA-D-03 envelope 전면 통일까지
완료됐다. `T-RV-27`, `T-RV-40`, `T-RV-41` 운영 hardening/performance 묶음도
merged됐고, `T-RV-37` 잔여 hygiene도 PR#263에서 merged됐다. 이번 PR은
`T-209e-c` admin backup/restore router + hot-swap UI를 닫는다.
provider별 public client live fetcher wiring인 `T-RV-04b`와 `T-212` 계열은 Claude Code
진행 범위이므로 Codex 후속 후보에서 제외한다.
TripMate `docs/krtour-map-requirements.md` 대조 후속 `T-213a~h`는 별도 에이전트가
완료했다(`docs/reports/tripmate-requirements-reconcile-2026-06-06.md`). T-207b는
사용자 결정에 따라 구현하지 않는다.
...
- **T-RV-04** Dagster provider 서비스키 resource 미구현(D-15, feature-load asset
  ...
  - **T-RV-04b**(진행 중, provider 순차 wiring): provider public client live fetcher를
    ...
    - **나머지 6종은 설계 결정 선행 필요** — 적합성 감사
      ...
      - [ ] **krheritage_events** — 가장 깨끗한 다음 후보(`select_event_list` 페이지).
        착수 전 `python-krheritage-api` EventService model이 `KrHeritageEvent` Protocol
        만족하는지 실검증.
      - [ ] **krex_rest_areas / krex_traffic_notices** — **Protocol 재조정(ADR-044)**:
        `RestArea`/`Incident` model이 Protocol 필드(uni_id/address/좌표 등) 미충족.
        upstream provider PR 또는 krtour Protocol+transform 재정렬(+uni_id 대체 자연키 결정).
      - [ ] **opinet_stations** — **fetch 정책**: bulk 엔드포인트 없음 → grid
        `search_stations_around` + dedup + `get_station_detail` N+1 커버리지 정책 결정.
      - [ ] **mois_license_records** — **정책**: 원천이 SpatiaLite DB 파일(LOCALDATA ZIP
        다운로드+파싱 vs DB 스냅샷 동기).
      - [ ] **knps_point/geometry** — **정책+어댑터**: keyless 파일셋 download→SHP/CSV
        파싱→record 어댑터.
...
- [~] T-209e — backup/restore 독립 DB 묶음(ADR-040 amend, D-5).
  ...
  artifact 목록, backup/restore command plan, manual-required hot-swap 경계를 노출한다.
  남은 범위는 ADR-039 advisory lock critical section, staging restore 후 smoke/count
  check 자동화, 운영 DSN/volume hot-swap 자동 실행이다.
