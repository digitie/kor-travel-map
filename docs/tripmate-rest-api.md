...
`packages/krtour-map-admin/openapi.user.json`을 따른다.
...
provider `krtour-ai-agent-youtube`(marker `P-13`, kind `place`,
...
원본 export item 전체는 `detail.payload.krtour_ai_agent.{youtube,evidence,...}`에
...
krtour-map 쪽 계약은 [`docs/public-views-api.md`](public-views-api.md)에 둔다.
