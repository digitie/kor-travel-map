...
(2026-06-01 기준). main 최신: `PR#142` 이후(admin OpenAPI cache 문서). Sprint 4a
...
runbook)를 PR#133~#142로 완료했다. 그 전 PR#96~#114에서 Sprint 4 prep,
...
ADR **001~045 모두 accepted**. 029→043, 003·035 일부→045로 supersede.
ADR-044 = 관련 라이브러리 `F:\dev\` 로컬 우선 조회 + 데이터 정합성 책임은 각
provider 라이브러리. ADR-045 = krtour-map Docker 독립 프로그램 + 독립 DB/Dagster +
TripMate OpenAPI 연동(ADR-003 함수 직접 호출 모델 supersede). 다음 후보 번호 = ADR-046.
...
import·공유 DB 없음). admin/API는 `krtour-map-debug-ui` 패키지(ADR-020, 내부망·
...
Sprint 4(4a+4b)는 아래 체크리스트대로 **전부 완료**(PR#133~#142). 다음 한 작업은
**ADR-045 독립 프로그램화 1단계** — Docker Compose 골격(`api`/`frontend`/`dagster`/
`postgres`) + admin-first OpenAPI 계약 확장(`docs/openapi-admin-contract.md` 기준) +
독립 Dagster queue(`ops.import_jobs` claim) wiring. 그 다음 Sprint 5
(MOIS-sibling provider 휴양림/수목원/박물관 + ADR-033 Phase 2 정합성 게이트).
세부는 `docs/sprints/SPRINT-5.md` + ADR-045 §후속.
...
- [x] dedup MOIS self-sibling — `core/dedup.find_sibling_candidates`(within-set
  ...
  (mypy 57 / 751 passed.)
- [x] geocoder 보강 라이브 재검증 — kraddr-geo REST(`127.0.0.1:8888`) 실연동으로
...
**accepted (text on main)**: ADR-001 ~ ADR-045 전부.
029→043 supersede, 044 (로컬 우선 조회 + 정합성 책임), 045 (krtour-map Docker 독립
+ OpenAPI, ADR-003 supersede). 다음 후보 번호 = ADR-046.
