...
- [ ] **다음**: advisory lock(ADR-011) + `ops.import_jobs` 마이그레이션(alembic 0006)
  + Step A snapshot `delete_mois_license_features_not_in` + mois source DB
  iterator(`collect_and_load_*`). CLI mutex(SPRINT-4 §2.8)와 함께 진행 검토.
