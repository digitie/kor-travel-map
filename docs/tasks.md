...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#233(TripMate 요구사항 대조
task 등록)까지 merged했고, PR#232에서 T-RV-29/30 user OpenAPI + frontend generated
type drift도 반영됐다. 이번 PR은 `T-RV-31/32/33` router/executor 정확성 묶음으로
runner savepoint, admin issue DTO closed schema, nearby 좌표 non-null 계약을 고정한다.
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 hardening
전까지 구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은
`T-RV-34/35` Dagster 실행 품질 묶음이다. `T-209b-a`(Dagster schedule storage
PostgreSQL 전환)는 별도 백로그로 등록되어 있으나, 현재 Codex 진행은 사용자 지시에
따라 남은 `T-RV-*`를 우선 닫는다.
TripMate `docs/krtour-map-requirements.md` 대조 후속은 `T-213a~h`로 별도 등록되어 있다
(`docs/reports/tripmate-requirements-reconcile-2026-06-06.md`).
...
- ~~**T-RV-23** offline upload checksum idempotency + load TOCTOU.~~
  ...
  Failure로 기록한다.
- **T-RV-27, T-RV-34~T-RV-35** — admin API bind 노출(deferred),
  sensor 멱등/retry, asset materialize/RetryPolicy. (리포트 §2)
