# debug-ui-package.md — `kor-travel-map-admin` 사양

> 파일명 `debug-ui-package.md`는 legacy(구 `kor-travel-map-admin`, PR#148에서
> `kor-travel-map-admin`으로 rename). 파일 경로를 참조하는 문서가 있어 파일명은 유지하며,
> 내용은 현재 `kor-travel-map-admin` 패키지 정본이다.

본 문서는 `kor-travel-map` 저장소 내 별도 Python 패키지
`kor-travel-map-admin`의 사양 reference다. 결정 근거는 `docs/decisions.md`의
...
| 항목 | 값 |
|------|----|
| 패키지명 (PyPI 형식) | `kor-travel-map-admin` |
| Python import | `from kortravelmap.admin import ...` |
| 위치 | `packages/kor-travel-map-admin/` (monorepo) |
| 별도 `pyproject.toml` | 예 |
...
`kor-travel-map-admin`는 단순 debug 도구가 아니라 ADR-035에 따라 admin/유지보수/
프로덕션 운영 UI를 겸한다. ADR-045 이후에는 Docker에서 실행되는 독립 kor-travel-map
...
├── pyproject.toml
├── README.md
├── src/kortravelmap/admin/
│   ├── __init__.py
│   ├── py.typed
│   ├── app.py           — FastAPI app factory + uvicorn entrypoint
│   ├── settings.py      — KOR_TRAVEL_MAP_ADMIN_* + 메인 settings 상속
│   ├── deps.py          — AsyncKorTravelMapClient + Engine 주입 (FastAPI Depends)
...
│   ├── next.config.js
│   ├── tsconfig.json
│   ├── .env.example           — NEXT_PUBLIC_VWORLD_API_KEY, NEXT_PUBLIC_KOR_TRAVEL_MAP_ADMIN_API
│   └── src/
...
│           # (categoryMaki / markerColor는 @kor-travel-map/map-marker-react에서 import — ADR-029)
└── tests/
...
```
kortravelmap.admin.app
   ↓
kortravelmap.admin.routers.*
   ↓
kortravelmap.admin.deps   ──→   kortravelmap.client (AsyncKorTravelMapClient)
                                       ↓
                              메인 패키지 (dto/core/infra/providers)
```

- 본 패키지는 `kortravelmap.client`만 import한다.
- `kortravelmap.infra`, `kortravelmap.providers` 직접 import 금지 — 본 패키지의
  존재 이유는 디버깅 UI이지 비즈니스 로직 우회가 아니다.
- 본 패키지는 자체 settings 외에 메인 라이브러리의 `KorTravelMapSettings`를
  상속/병합한다.

## 4. settings

실제 구현은 `packages/kor-travel-map-admin/src/kortravelmap/admin/settings.py`:
...
class AdminSettings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="KOR_TRAVEL_MAP_ADMIN_", env_file=".env")
...
# 환경변수로 override
KOR_TRAVEL_MAP_ADMIN_HOST=127.0.0.1 \
KOR_TRAVEL_MAP_ADMIN_PORT=12301 \
KOR_TRAVEL_MAP_PG_DSN=postgresql+asyncpg://... \
uvicorn kortravelmap.admin.app:app

# 또는 CLI (옵션)
kor-travel-map-admin run --host 127.0.0.1 --port 12301
```
...
```bash
# packages/kor-travel-map-admin/scripts/export_openapi.py
...
| `NEXT_PUBLIC_KOR_TRAVEL_MAP_ADMIN_API` | 백엔드 API base URL (개발: `http://127.0.0.1:12301`) |
...
| `KOR_TRAVEL_MAP_ADMIN_DAGSTER_URL` | backend가 Dagster GraphQL을 조회할 때 쓰는 Dagster webserver base URL. 로컬 기본 `http://127.0.0.1:12302`, Docker API 컨테이너 기본 `http://dagster:12302` |
...
| `KOR_TRAVEL_MAP_ADMIN_BACKUP_COMMAND_ENABLED` | backup/restore host command 실행 허용 여부. 기본 `false`라 `/admin/backups` UI는 command plan만 생성한다 |
| `KOR_TRAVEL_MAP_ADMIN_BACKUP_COMMAND_TIMEOUT_SECONDS` | opt-in host command 실행 timeout. 기본 `1800` |
| `KOR_TRAVEL_MAP_ADMIN_RESTORE_APP_DB` / `KOR_TRAVEL_MAP_ADMIN_RESTORE_DAGSTER_DB` / `KOR_TRAVEL_MAP_ADMIN_RESTORE_RUSTFS_VOLUME` | staging restore target 기본값 |
| `KOR_TRAVEL_MAP_DOCKER_ADMIN_DAGSTER_URL` | Docker compose가 API 컨테이너의 `KOR_TRAVEL_MAP_ADMIN_DAGSTER_URL`로 주입하는 내부 Dagster URL. `.env`의 로컬 `127.0.0.1` 값과 분리 |
| `KOR_TRAVEL_MAP_DOCKER_ADMIN_DAGSTER_ALLOWED_HOSTS` | Docker compose가 API 컨테이너에 주입하는 Dagster host allowlist. 기본은 `["dagster","127.0.0.1","localhost","::1"]` |
...
| `KOR_TRAVEL_MAP_ADMIN_FRONTEND_DIST` | (FastAPI 측) Next.js build 산출물 경로 — static export 모드 시에만 사용 (`.next/` 또는 `out/`) |

**VWorld API key 공유 정책 (확정, ADR-025 보강 2026-05-25)**:
`kor-travel-geo` ADR-019의 `KOR_TRAVEL_GEO_VWORLD_API_KEY`를 **공유 사용**한다.
...
| `<MakiMarker>` | feature 1건 마커 | `kortravelmap.category` Tier 4 → maki icon 55종 |
...
`packages/kor-travel-map-admin/frontend/src/lib/categoryMaki.ts`:

```typescript
// kortravelmap.category와 동기. openapi-typescript로 자동 생성 후 수동 보강.
...
CI에서 drift 검증 (kor-travel-geo ADR-015 패턴 미러). 자세한 절차는 §8 OpenAPI export.
