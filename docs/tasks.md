...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#190(T-RV-03 Dagster
resource lifecycle)까지 merged. 코드 리뷰 후속 HIGH 선반영 흐름으로, 이번 PR은
`T-RV-04a`(Dagster provider record guard resource + env mapping)를 닫는다.
`T-RV-27`(admin API bind 노출 정정)은 사용자 결정에 따라 production 레벨 hardening
전까지 구현하지 않고 문서 추적만 유지한다. 다음 구현 작업은 `T-RV-04b` provider
public client live fetch wiring 또는 `T-RV-05/11` run-now/claim 락 보강이다.
