...
- [ ] T-212c — API endpoint/error/log contract 정리.
      ...
      - [ ] **system/provider/API call log 조회 표면**(T-212c-API-04) — 현재 백킹 테이블
        없음(`ops.*` log 스키마 미존재). 신규 마이그레이션(예: `ops.api_call_log`/
        `ops.system_log`) 설계 후 조회 API. **schema 설계 선행 필요 → 별도 task**.
      - [ ] error envelope 일관성 전수(라우터별 `error.code` 표준화) 점검.
