...
| `_SNAPSHOT_MATERIAL_BYTE_LIMIT` | `512 MiB` | **`56 MiB`** (아래 §byte 축) |
...
> **측정 조건 정정.** 위 세 수치를 잰 시점의 soak은 측정 동안
> `_SNAPSHOT_BUILD_TIMEOUT_SECONDS`를 3,600초로 덮고 session `statement_timeout`도
> 3,600초로 올린 상태였다(적대 리뷰가 잡았다). 소요 시간 자체는 유효하지만
> **배포 deadline 아래에서 완주가 증명된 것은 아니다** — `_snapshot_build_deadline`이
> 한 번도 돌지 않았다. 그 우회를 제거하고 상한에서 다시 쟀다.

**확정 측정 — 배포 예산·배포 상한, 우회 없음** (`SOAK: PASS`, exit 0):

| 축 | 값 |
|---|---|
| N | **500,000** (= `_SNAPSHOT_ITEM_LIMIT`) |
| build | **123.0초** = 예산의 41%, 게이트(150초)에 **18% 여유** |
| 처리량 | **4,065 item/s** · 재료 **422,764 B/s** |
| 상한 + 1 | typed `snapshot_item_limit_exceeded`, partial row 0 |
| compaction | 500,000행 / 7.2초 |
| 증거 보존 | material 1 · receipt 1 (item 0) |
| seed | 19.4초 |

CI 불변식이 쓰는 상수는 **이 확정 측정**이다(4,065 item/s, 422,764 B/s). 탐색 측정의
4,225는 예산을 덮은 실행이라 쓰지 않는다.
...
500,000은 배포 예산 아래 123.0초로 예산의 41%, 게이트(예산/2 = 150초)에 18% 여유를 남긴다.
게이트를 만족하는 최대치는 150 × 4,065 ≈ **609,750**이며 500,000은 그 아래 라운드 값이다.
...
예전 512 MiB는 실측 재료 처리량 **422,764 B/s**(500,000 × 104 B / 123.0초) 기준
**1,270초 = 예산의 4.2배**라 시간 방어선이 될 수 없었다. 게다가 item 상한을 500,000으로
낮춘 뒤로는 계약 최대 폭에서도 500,000 × 683 B = 325.7 MiB < 512 MiB라 **ASCII stream에서
아예 발화하지 않는 죽은 코드**가 됐다.

**56 MiB**로 조인다. 422,764 B/s에서 138.9초로 예산 절반(150초) 안이다. 60 MiB는 148.8초로
게이트에 1.2초만 남아 다음 측정에 뒤집히므로 쓰지 않는다. 정상 폭 stream은 영향이 없다 —
103 B/item이면 570,099 item에 해당해 item 상한(500,000)이 먼저 걸린다.
...
오늘의 235.7초 = **2.32배**다. 상한에 적용하면 123.0 × 2.32 = 285.4초로 예산의 95.1%다.
...
- 하드웨어를 바꾼다 / `target_key` 폭이 크게 달라진다 / PG major를 올린다 → 재측정한다.
  500,000 기준 build 123초 + seed 19초라 1,000,000(235초 + 39초) 대비 정기 실행이 현실적이다.
