...
## 머지 직전 live 실측 — 도는 서비스가 422를 낸다 (2026-09-11)

적대 리뷰가 집은 major #1은 "uuid 필터에 아무거나 넣으면 500"이었다. 그 수정은
테스트로 고정했지만, **테스트가 422라고 말하는 것과 도는 서비스가 422를 내는 것은
다른 사실이다.** 격리 스택(`~/ktm-live-301`)을 현재 HEAD로 올리고 직접 물었다.

```
GET /v1/ops/consistency/issues?feature_id=...

no filter                                  -> 200   (전체 목록)
feature_id=<존재하는 uuid>                 -> 200
feature_id=<canonical uuid, 없는 Feature>  -> 200   (빈 목록 — 기존 계약 보존)
feature_id=f_1156010100_p_0000000000000000 -> 422
    "feature 참조 'f_...'는 이 시스템의 Feature를 가리키지 않습니다"
```

세 갈래가 설계대로다. 특히 넷째 줄이 **500이 아니라 422**이고, 조용히 `None`으로
낮춰 전체 목록을 내보내지도 않는다 — 그 둘이 이 helper가 세 갈래를 가진 이유다.

같은 스택에서 provider 핀 8종을 올린 뒤에도 기동(`/health` 200)·bbox·uuid 상세가
그대로다. 즉 khoa async 전환을 포함한 bump가 API 기동이나 조회 경로를 건드리지
않는다.

### 곁가지로 확인된 fail-close 둘

이 실측을 세우면서 설정 층의 fail-close 두 개가 실제로 동작하는 것을 봤다.
`KOR_TRAVEL_MAP_API_OPS_{READ,CANCEL,FIXTURE}_TOKEN`은 **셋이 함께** 설정돼야 하고
(하나만 주면 기동 자체가 거부된다), ops scope는 `ops:read`/`ops:cancel`만 받는다.
둘 다 메시지가 무엇을 요구하는지 정확히 말한다 — 이 저장소가 반복해서 지키려는
성질이다.
