...
파일 다운로드 라이브러리)를 활용한 본 라이브러리(`python-krtour-map`)의 feature
적재 전체 흐름이다. 4 step lifecycle + 195 업종 슬러그 카탈로그를 다룬다.

> **license `place` 승격의 1차 흐름만 다루는** 좁은 가이드는
> [`docs/mois-license-feature-etl.md`](mois-license-feature-etl.md)가 별도로 있다.
...
| 본 라이브러리 변환 모듈 | `krtour.map.providers.mois` (ADR-022) |
| 본 라이브러리 loader 모듈 | `krtour.map.mois` |
...
[Step B] iter_open_place_records → krtour_map.features (영업중만)
...
async def feature_place_mois_licenses(
    ctx, mois_async_session, krtour_map_client: AsyncKrtourMapClient,
):
    promoted_slugs = krtour_map_client.providers.mois.PROMOTED_SERVICE_SLUGS
    ...
    async for batch in _batched(
        ...
        size=krtour_map_client.providers.mois.DEFAULT_BATCH_SIZE,
    ):
        bundles = list(
            krtour_map_client.providers.mois.license_records_to_bundles(
    ...
    deleted = await krtour_map_client.delete_mois_license_features_not_in(snapshot_keys)
...
async def feature_place_mois_licenses_incremental(
    ctx, mois_client, mois_async_session, krtour_map_client,
):
    sync_state = await krtour_map_client.get_sync_state(
    ...
    async for batch in _batched(
        ...
        bundles = list(krtour_map_client.providers.mois.license_records_to_bundles(batch))
        await krtour_map_client.load_feature_bundles(bundles)

    # 폐업 처리
    closed = [r for r in source_db_rows if r.status_category == "CLOSED"]
    if closed:
        await krtour_map_client.delete_mois_license_features_for_records(closed)
...
### 4.1 승격 슬러그 (`KRTOUR_MAP_MOIS_PROMOTED_SLUGS`)
...
총 **42종**. 코드는 `krtour.map.providers.mois.PROMOTED_SERVICE_SLUGS:
frozenset[str]`로 상수화.

### 4.2 제외 슬러그 (`KRTOUR_MAP_MOIS_EXCLUDED_SLUGS`)
...
제외 slug 정확한 set은 코드 상수 (`krtour.map.providers.mois.EXCLUDED_SERVICE_SLUGS`).
...
| `KRTOUR_MAP_PG_DSN` | 본 라이브러리 feature DB |
...
| `krtour_map.krmois` | `krtour.map.mois` |
| `krtour_map.providers.krmois` | `krtour.map.providers.mois` |
