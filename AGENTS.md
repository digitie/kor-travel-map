...
> **T-226 예정 변경(ADR-054)**: public 배포명은 `kor-travel-map`, Python import root는
> `kortravelmap`, 권장 예시는 `import kortravelmap as ktm`로 clean cut한다. CLI 목표명은
> `ktmctl`, PostgreSQL 기본 DB와 RustFS 사용자 가시 이름은 `kortravelmap*` 계열로 둔다.
> 형제 프로젝트 표시는 `python-kraddr-geo` → `kor-travel-geo`,
> `tripmate-agent`/`krtour-ai-agent` → `kor-travel-concierge` 기준으로 맞춘다.
> 아래 표는 T-226c/d/e 적용 전 현재 코드/운영값이다. 전환 정본은
...
PC 개발 source of truth는 **NTFS (`F:\dev\python-krtour-map*`)** worktree다. 단, 2026-06-12 이후 agent 작업 실행 위치는 다음처럼 강제한다: **순수 `git` 명령만 Windows/NTFS host에서 실행**하고, 파일 탐색·문서/코드 수정·테스트·lint·build·Docker·Python/Node/npm·`gh`/GitHub CLI 등 나머지 모든 작업은 **WSL에서 `/mnt/f/dev/python-krtour-map-<agent>` 경로로 실행**한다. 디버그 UI Playwright e2e는 예외로 **Windows 호스트에서만 실행**한다(서버는 WSL). 형제 라이브러리 (`kor-travel-geo`(구 `python-kraddr-geo`) / `python-kraddr-base` / `python-knps-api` 등)도 동일 정책을 따른다.

- **코드/가상환경**: NTFS (`F:\dev\python-krtour-map/`)에 보관하되, agent의 shell 작업은 WSL `/mnt/f/dev/python-krtour-map-<agent>`에서 실행한다. Windows host shell에서는 순수 `git` 명령만 허용한다.
