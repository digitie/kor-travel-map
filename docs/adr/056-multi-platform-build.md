# ADR-056: N150/Odroid 병행 multi-platform Docker build

### 상태

Accepted (2026-06-13) — kor-travel-map 독립 프로그램의 운영 배포 자동화로
multi-platform Docker build를 정본화하되, 사용자 재지시에 따라 streaming
replication은 하지 않는다.

### 배경

kor-travel-map은 ADR-045 이후 독립 프로그램이고, API/frontend/Dagster/Postgres/RustFS
운영 묶음을 amd64(N150 16GB)와 arm64(Odroid M1S) 두 노드에서 자체적으로 검증해야 한다.
2026-06-13 사용자 추가 지시로 이 저장소에서는 streaming replication을 제외하고,
...
- `docs/deploy.md`와 `docs/runbooks/docker-app.md`가 양 노드 image build 절차의
  정본이다.
