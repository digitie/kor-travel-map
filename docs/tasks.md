...
- [ ] **T-RV-50 maplibre-vworld-js 최신화**(point 6): 현재 frontend 핀 `#v0.1.2`가 최신 **태그**지만
  upstream `main`에 untagged 후속 커밋 존재. (a) `v0.1.2..main` diff 확인 → 기능 변경 있으면
  maplibre repo에 신규 태그(v0.1.3) 릴리스(PR+머지), docs-only면 그대로. (b) frontend `package.json`
  핀을 최신 태그로, 관련 `maplibre-gl ^5` peer 정합 + map wrapper 코드 최신 API 적응. (c) Windows
