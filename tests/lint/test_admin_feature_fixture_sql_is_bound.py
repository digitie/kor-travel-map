...
AS <alias>`로 **alias→테이블을 유도**하고, 같은 SQL 안의 `<alias>.<column>` 참조를
...
membership 권한을 자동으로 갖지 않으므로, `SET ROLE` **전에** 관계를 읽으면
권한 오류로 죽는다. 2026-09-05에 `public.alembic_version`이 정확히 그랬다.
"""
...
_TABLE = re.compile(
    r"CREATE TABLE (?P<name>[a-z_]+\.[a-z_]+) \((?P<body>.*?)
\);", re.DOTALL
)
_VIEW = re.compile(r"CREATE (?:MATERIALIZED )?VIEW (?P<name>[a-z_]+\.[a-z_]+)")

#: `FROM/JOIN/USING/UPDATE/INTO <schema>.<table> [AS] <alias>`
_ALIASED = re.compile(
    r"(?:FROM|JOIN|USING|UPDATE|INTO)\s+(?P<table>[a-z_]+\.[a-z_]+)"
    r"(?:\s+AS)?\s+(?P<alias>[a-z_][a-z0-9_]*)",
    re.IGNORECASE,
)
_REFERENCE = re.compile(r"(?P<alias>[a-z_][a-z0-9_]*)\.(?P<column>[a-z_][a-z0-9_]*)")

#: alias 자리에 올 수 없는 SQL 예약어. 이것들을 alias로 잡으면 대조가 공허해진다.
_KEYWORDS = frozenset(
    {
        "and", "as", "conflict", "cross", "do", "from", "full", "group", "inner",
        "into", "join", "left", "limit", "not", "nothing", "on", "or", "order",
        "returning", "right", "select", "set", "using", "values", "where", "with",
    }
)


def _schema_columns() -> dict[str, frozenset[str]]:
    """baseline DDL에서 관계 → 컬럼 집합을 유도한다(뷰는 빈 집합 = 미검사)."""
...
def _helper_sql() -> list[tuple[int, str]]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Constant) and isinstance(node.value, str):
            value = node.value
            if re.search(r"(SELECT|INSERT|UPDATE|DELETE)", value) and "." in value:
    ...
    for lineno, sql in _helper_sql():
        ...
        for match in _REFERENCE.finditer(sql):
            ...
            if column not in known:
                violations.append(f"line {lineno}: {alias}.{column} not in {table}")
    assert violations == [], (
...
def test_preflight_reads_no_relation_before_role_escalation() -> None:
    ...
    end = source.index("async def _run(", start)
    ...
    referenced = re.findall(r"FROM\s+([a-z_]+\.[a-z_]+)", before, re.IGNORECASE)
    assert referenced == [], (
        "role escalation **전에** 관계를 읽고 있다: "
        f"{sorted(set(referenced))}. "
