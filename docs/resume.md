...
- [ ] **다음**: Step B incremental + dataset_key cursor. `ProviderSyncState` 테이블은
  **이미 존재**(provider/dataset_key/sync_scope PK + cursor JSONB + last_success/failure
  + next_run_after) → 마이그레이션 불필요. cursor 저장/조회 repo + incremental sync
  경로 + client/CLI 통합.
- [ ] 실적재 후 dedup false-positive 측정(가중치 조정 ADR-016).
