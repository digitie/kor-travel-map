...
- `weather_values`: +30일 (참조 trip 0건은 즉시 삭제 — TripMate trip_pois join
  으로 별도 검증).
- purge SQL: `docs/data-model.md` §7 또는 `docs/postgres-schema.md` §7.
...
- import: `krtour_map.models.WeatherValue` → `krtour.map.dto.weather.WeatherValue`.
- `krtour_map.db.feature_weather_values` → `krtour.map.infra.models.feature_weather_values`.
