...
> `docs/integrations/krtour-map-rest-api.md`(2026-06-08~09 갱신)가 이미 대부분의 항목을
...
> 화이트리스트)·TM-10(공유 뷰 권한)·TM-11(지도 성능 정책)·admin base 9011 정정 정도다.
...
- **TM-12 — admin 표면 축소** ✅ D-06 **수정 승인됨 (2026-06-10)**
  ...
  - **`/admin/etl`은 유지** — TripMate 자체 ETL 관리 로직의 관제 화면으로 존치
    ...
    TripMate 고유 데이터 잡만** 관리, krtour-map 적재와 중복 없음 → T-210c(구
...
- **TM-13 — FeatureSuggestion 릴레이 경로** ✅ D-02 확정 + **2차 보정 (2026-06-10 재독)**
  ...
  - krtour 측 T-217c가 잔여 합의 5건(review_mode/idempotency/출처 태깅/admin 인증/
    closure — TripMate `docs/integrations/krtour-map-rest-api.md` §7 질의 목록)을
    확정해 회신 예정.
  - **정정 필요(TripMate 측 오류)**: admin client base를 9012로 가정하나 **9012는
    krtour admin UI(Next.js)**이고 admin **API는 9011 `/v1/admin/*`** — T-180 설계와
    `TRIPMATE_KRTOUR_MAP_ADMIN_BASE_URL` 의미(기본 9012)를 9011 기준으로 재정의.
  - 출처 태깅의 사용자 식별 정보: ✅ D-11 확정 — **익명**, TripMate 측 불투명 참조
    ID(suggestion_id)만 (krtour-map 개인정보 비저장, 역추적은 TripMate admin에서).
