"""``krtour.map_debug_ui.app`` — FastAPI application factory.
...
uvicorn 설정은 ``DebugUiSettings``(``KRTOUR_MAP_DEBUG_UI_*`` env) 또는 호출자가
...
def create_app(settings: DebugUiSettings | None = None) -> FastAPI:
    """FastAPI application factory.

    Parameters
    ----------
    settings
        ``DebugUiSettings`` instance. ``None``이면 env에서 자동 로드.
    ...
    라우터/DTO 변경 시 ``packages/krtour-map-debug-ui/openapi.json`` drift
...
``uvicorn krtour.map_debug_ui.app:app``로 직접 실행, ``scripts/export_openapi.
