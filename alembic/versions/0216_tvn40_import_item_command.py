"""T-VN-40B import item/provenance named command.

Revision ID: 0216_tvn40_import_item_cmd
Revises: 0215_tvn40_import_plans
"""
...
revision: str = "0216_tvn40_import_item_cmd"
down_revision: str | Sequence[str] | None = "0215_tvn40_import_plans"
...
CREATE TABLE ops.curation_import_collection_touches (
  ...
  FOREIGN KEY (command_id, collection_id)
    REFERENCES ops.curation_import_collection_effects(command_id, collection_id)
    ON DELETE RESTRICT
);
CREATE TRIGGER trg_curation_import_collection_touches_immutable
BEFORE UPDATE OR DELETE ON ops.curation_import_collection_touches
FOR EACH ROW EXECUTE FUNCTION ops.reject_curation_import_collection_effect_mutation();
CREATE TRIGGER trg_curation_import_collection_touches_no_truncate
BEFORE TRUNCATE ON ops.curation_import_collection_touches
...
CREATE OR REPLACE PROCEDURE feature.touch_curation_import_collection_command(
...
CREATE PROCEDURE feature.apply_curation_import_items_command(
  IN p_items jsonb,
  IN p_content_sha256 text,
  IN p_batch_kind text,
  IN p_command_id bigint,
  IN p_principal text,
  OUT o_import_batch_id uuid,
  OUT o_inserted integer,
  OUT o_updated integer,
  OUT o_removed_item_ids uuid[]
)
LANGUAGE plpgsql
SECURITY DEFINER
...
def upgrade() -> None:
    _execute_commands(_SQL)
    op.execute(
        "GRANT SELECT, INSERT ON ops.curation_import_collection_touches "
        "TO ktm_curation_command_owner"
    )
    op.execute(
        "GRANT SELECT, INSERT ON feature.curation_import_batches, "
        "feature.curation_import_rows, feature.curation_link_decisions "
        "TO ktm_curation_command_owner"
    )
    op.execute("SET ROLE ktm_curation_command_owner")
    for signature in (_TOUCH_SIGNATURE, _APPLY_SIGNATURE):
        op.execute(
...
def downgrade() -> None:
    raise RuntimeError("0216_tvn40_import_item_cmd is forward-only; rebuild with the T-VN-40 release head")
