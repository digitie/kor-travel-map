...
## 2026-09-13 — 분자를 세기 시작했고, 적대 리뷰 두 판이 각각 blocker를 냈다
...
수천 건을 쓰면서 `upstream_requests_min: 0`을 냈다 — 하필 **호출량이 가장 크면서
일일 한도는 아직 실측하지 못한** provider다(data.go.kr 포털에 없다). 분자가 유일한
가시성인데 그것이 0이었다. 그리고 그때 내가 갖고 있던 구조 검사는 "asset이 계수 범위 안에서
