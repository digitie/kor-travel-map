...
- **TM-12 — admin 표면 축소** ✅ D-06 **수정 승인됨 (2026-06-10)**
  ...
  - `/admin/category-mapping` 제거 또는 `GET /v1/categories` read-only 뷰로 대체
    (카테고리 정본 이원화 방지).
- **TM-13 — FeatureSuggestion 릴레이 경로** ✅ D-02 **(a) 확정 (2026-06-10)**
  - krtour-map이 서비스용 수신 API `POST /v1/features/suggestions`(가칭)를 신설한다
    (krtour ADR-051 + T-217c). TripMate는 제보 발생 시 이 API로 릴레이 — krtour 측
