...
- [/] T-VN-M05-ROLE-CATALOG-RESET — `31fe73ad…`·`b22bfb8c…` n150 candidate는 각각 `target_not_isolated`·`foreign_membership` terminal로 보존하며 재시도하지 않는다. PinVi draft PR [#500](https://github.com/digitie/pinvi/pull/500)의 immutable `a619d037…`은 target 네 role 내부 membership edge를 수용하되 roleid/member가 target 밖에 닿으면 계속 fail-close한다.
- [/] T-VN-M05-MANAGER-PIN-ROTATION — Manager draft PR [#243](https://github.com/digitie/kor-travel-docker-manager/pull/243)은 PinVi result receipt의 transaction·pinset·inode binding과 세분 fixed enum을 strict parse하고 durable terminal journal에 보존한다. PinVi `a619d037…`·Map `9c64e862…`의 다음 pinset은 `89330403…`이다.
- [ ] T-VN-FINAL-REBUILD — 두 PR의 CI·전문 리뷰를 통과한 새 pinset Manager release를 n150에 공식 설치하고 `rebuild-pinned --confirm --json`을 정확히 한 번 실행해 seven-runtime과 v6/v8 committed 증적을 만든다. historical `cbb`·`52`·`06045`·`68d99705`·`285618c0`·`37932169`·`31fe73ad`·`b22bfb8c` candidate는 재시도하지 않는다.
