...
| 7 | test-strategy.md §5 | e2e 경로를 `tests/e2e/`로 표기 (실제 없음) | `packages/kor-travel-map-admin/tests/`로 정정 + 실 DB는 메인 `tests/integration/` |
| 8 | backend-package.md §1 | `AsyncKorTravelMapClient`를 구현된 API처럼 서술 (미구현) | "구현 현황" 주석 — 현재는 `feature_repo` 직접 호출 단계 |
