...
**진행 중**: ADR-045 독립 프로그램화 후속. main은 PR#184(T-200 Batch DAG +
정합성 게이트)까지 merged. 코드 리뷰 후속 HIGH 선반영 흐름으로, 이번 PR은
`T-RV-09`(offline upload 업로드 크기 상한/무제한 read 차단)를 닫는다. 다음 작업은
`T-RV-27`(admin API bind 노출 정정)이다. T-RV HIGH 중 운영 보안 항목을 먼저 닫은 뒤
ADR-045 잔여(T-201b 정합성 Phase 2, T-209b Dagster metadata DB 분리/init,
T-209 Docker/daemon polish)를 이어서 닫고, ADR-045 관련 잔여 task가 닫히면 T-212
전체점검 묶음으로 넘어간다. T-207b는 사용자 결정에 따라 구현하지 않는다.
