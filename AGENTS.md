...
| ADR accepted | 001~056 전부 (text on main). 029는 ADR-043으로 supersede. ADR-045는 Docker 독립 프로그램 + OpenAPI 연동으로 ADR-003 운영 모델을 supersede. ADR-046은 ADR-045 이행 시 구 모델 호환 shim 금지. ADR-047은 docker-manager 로컬 포트 기준(API 12701, admin UI 12705, Dagster 12702, kor-travel-geo API 12501). ADR-048은 `/v1` 전 표면 clean cut + REST envelope/parameter/error/명명 정합성 표준. ADR-053은 `kor-travel-concierge` provider identity clean cut과 TripMate 직접 연동 제거 경계. ADR-054는 `kor-travel-map` / `kortravelmap` package identity clean cut. ADR-055는 `kor-travel-map-api` Python backend와 `kor-travel-map-admin` frontend 분리. ADR-056은 `pinvi` T-108에서 streaming replication을 제외하고 N150/Odroid multi-platform Docker build를 운영 자동화로 고정. |
