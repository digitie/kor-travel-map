...
- **다음 (우선순위 순)**
  ...
  - [ ] T-226 — **배포명/임포트명 재정의: `kor-travel-map` / `kortravelmap`**.
        ...
        DB는 `kor_travel_map`, RustFS bucket/prefix는 `kor-travel-map` 계열로 둔다.
        구현은 대규모
        package identity clean cut이므로 별도 PR에서 진행한다. 형제 프로젝트 표시는
        `kor-travel-geo`, `kor-travel-concierge`, `kor-travel-docker-manager`
        기준으로 맞춘다.
    - [x] T-226a — ADR/문서 정본 작성(완료: 2026-06-12, Codex):
          ...
          `kor_travel_map`, `ktmctl`, `kor-travel-geo`, `kor-travel-concierge`)와
    ...
    - [x] T-226d — runtime/deployment identity 전파:
          `KOR_TRAVEL_MAP_*`, DB 기본값 `kor_travel_map`,
          Dagster metadata DB `kor_travel_map_dagster`, RustFS bucket/prefix 표시명,
          Docker/image/service 표시명, advisory lock/log/metric service label을 clean cut했다.
    - [x] T-226e — 소비자 문서/client/migration guide:
...
- **최근 완료**
  ...
  - [x] **T-223 — curated_features + TripMate curated_trip_plans import 계약/구현.
        ...
        provenance, upsert/refresh, `KOR_TRAVEL_CONCIERGE_API_BASE_URL` 제거가 머지됐다.
  ...
  - [x] **T-222b — 공개 해수욕장/축제 뷰 API 구현. 완료(2026-06-12, Codex)**:
        ...
        추가하고 admin/user OpenAPI, `@kor-travel-map/map-user-client` 타입을 갱신했다.
  ...
  - [x] **T-224 — `kor-travel-concierge` provider 경계 재정의 + 상세 구현. 완료
        (2026-06-12, Codex)**: ADR-053을 추가하고, canonical provider를
        `kor-travel-concierge-youtube`로 clean cut했다. `providers.kor_travel_concierge`,
        `KOR_TRAVEL_MAP_KOR_TRAVEL_CONCIERGE_*`, Dagster `kor_travel_concierge_youtube_features` /
        `feature_place_kor_travel_concierge_youtube*`, raw payload
        `detail.payload.kor_travel_concierge` 기준으로 코드·테스트·계약 문서를 정렬했다.
        TripMate ↛ kor-travel-concierge 직접 연동 없음, curated trip plan 생성 flow 제외,
        구 `KOR_TRAVEL_CONCIERGE` env/provider alias 없음. 검증:
  ...
  - [x] **T-217a~g 전부 완료(2026-06-10~11)** — Phase 6.9 cross-repo 정합성 종결:
        ...
        제안 연동 합의 5건(c) · integration-map+분기 audit(d) · RustFS=kor-travel-docker-manager
  ...
  - [x] TripMate-agent provider 추가/연동 후속 작업 추적 — kor-travel-map 쪽
        `kor-travel-concierge-youtube` provider 변환, Dagster REST fetch/resource/asset/schedule
...
- **Phase 6 — TripMate 연계/정리 (대부분 TripMate repo 외부)**
  - [x] T-210e — `openapi-typescript` client 생성. **완료(2026-06-11, 사용자 지시로
        ...
        `packages/kor-travel-map-user-client/`(`@kor-travel-map/map-user-client`) —
...
- **Phase 6.9 — cross-repo 정합성 (2026-06-10 검토, ADR-050~052 — 결정 반영분)**
  - [x] T-217a — kor-travel-concierge fetcher 경로 중립화 정렬 (ADR-050 #1). **완료
        (2026-06-10)**: fetcher path + 테스트/docstring 7곳 `/api/v1/features/*`
  ...
  - [x] T-217b — `reject`/`tombstone` operation → feature **inactive 전환**. **완료
        (2026-06-10, ADR-050 #4)**: 변환부 `kor_travel_concierge_inactive_entity_ids`(skip
  ...
  - [x] T-217d — cross-repo 연동 정본 문서. **완료(2026-06-11)**:
        ...
        계약 정본 위치, kor-travel-docker-manager 포함) + 분기 audit
  ...
  - [x] T-217e — RustFS 관리 주체 확정. **완료(2026-06-11, 사용자 재정의)**:
        prefix 공유 정책 명문화 대신 **RustFS를 kor-travel-docker-manager에서 일괄 관리**하는
        것으로 갈음 — ADR-052 Amendment(인프라 소유=kor-travel-docker-manager
        단일 PostGIS :5432 + RustFS :12101/12105, kor-travel-map·kor-travel-concierge는 사용자,
  ...
  - [x] T-217f — YouTube evidence의 feature detail 노출 형태 확정. **완료(2026-06-10)**:
        ...
        **confidence_score(0~100, 신규 추가)**) + 원본 보존 `detail.payload.
        kor_travel_concierge.{youtube,evidence}`. TripMate 출처 배지 UX(TM-08)는
...
`kor-travel-concierge`는 TripMate와 직접 연동하지 않고, TripMate curated plan 생성 flow에도
...
3. **ADR-045 독립 프로그램 전환 반영** — kor-travel-map은 Docker 독립
...
4. **문서 최신성 유지** — provider 주소 보강용 geocoding endpoint 정본은
   kor-travel-geo REST v2 `POST /v2/reverse`, `POST /v2/geocode`, 로컬 포트는
   `http://127.0.0.1:12201`. geocoding 전용 디버깅 화면은 kor-travel-geo 프로젝트에서
   관리하고, kor-travel-map-admin에는 두지 않는다. frontend 현재 기준은 Next.js 16 +
   `maplibre-vworld-js#v0.1.3`.
5. **고정 포트** — kor-travel-map 독립 프로그램 로컬/standalone 포트는 API `12301`,
...
8. **feature update request 큐** — T-205a에서 `ops.feature_update_requests` 테이블과
   ...
   `AsyncKorTravelMapClient` 표면을 추가했다. T-206d는 runner 주입형 실행 본체이며,
...
- ~~**T-RV-09** offline-upload 업로드 크기 상한 없음 + 전체 버퍼링(OOM/DoS).~~
  ✅ `KOR_TRAVEL_MAP_OFFLINE_UPLOAD_MAX_BYTES` + `413` + bounded read 회귀 테스트 반영.
...
- ~~**T-RV-27** admin API `0.0.0.0` 노출(ADR-005/.env 모순)~~ ✅
  Docker compose host publish 기본값을 `KOR_TRAVEL_MAP_DOCKER_BIND_HOST=127.0.0.1`로
  제한했다. 컨테이너 내부 listen은 유지하되 host 모든 interface 노출은
  `KOR_TRAVEL_MAP_DOCKER_BIND_HOST=0.0.0.0` 명시 opt-in과 네트워크 보호 전제 문서로만
  허용한다.
...
- ~~**T-RV-18** router substring 기반 status 매핑.~~
  ...
  feature update request 라우터는 `SigunguResolverUnavailable`으로 kor-travel-geo 설정
...
- ~~**T-RV-36** Dagster package dependency hygiene.~~
  ...
  `kor-travel-map==0.2.0-dev` 핀과 `boto3`/`botocore` 직접 의존성을 추가했다.
...
- ~~**T-RV-24** offline upload 상태 집합 단일화.~~
  ✅ `kortravelmap.core.offline_upload_states`를 단일 source로 두고 router,
...
- [x] **T-014 — 코드 작성 단계 진입** (사용자 승인 2026-05-25, 본 PR#16)
  ...
  - 후속 Sprint 1 scaffolding PR로 실제 코드 작성:
    - [x] PR#17 `src/kortravelmap/` PEP 420 scaffolding + `settings.py` +
          6개 layer placeholder + smoke 테스트
    - [x] PR#18 `src/kortravelmap/category/` 144건 (kraddr-base 이전 +
          ADR-027 3건 + tests/unit/test_category.py 16 cases)
    - [x] PR#19 `src/kortravelmap/dto/` Feature + 5 detail (place/event/
    ...
    - [x] PR#20 `src/kortravelmap/core/` exceptions 7종 (ADR backend-package.md §5)
          + `make_feature_id` (ADR-009 결정적 SHA1) + tests 42건. scoring stub은
          dto Coordinate 의존 위해 후속 PR로.
    - [x] PR#21 `src/kortravelmap/infra/crs.py` (pyproj.Transformer 4326↔5179
...
- [x] T-017 — **공통 maki marker / category 매핑 npm 패키지 추출** (2026-06-09)
  ...
  - **ADR-029 (proposed, PR#10 merged)** — `@kor-travel-map/map-marker-react` (MIT
...
- [x] T-018 — **`python-knps-api` provider 등록 / KNPS 적재 준비** — **완료**
  ...
  - `kortravelmap.providers.knps` 모듈 신설은 Sprint 3 (ADR-034 7단계).
...
- [ ] T-019 — **TripMate 측 후속 작업 추적** (ADR-026 + ADR-029 후속, 본
  ...
  - TripMate `apps/web` Kakao Maps → maplibre-vworld 교체 PR (TripMate
    저장소). Next.js stack 유지, 마커 import만 `@kor-travel-map/map-marker-react`
    교체.
  - SPEC V8 v8_3 Kakao Maps 섹션에 "superseded by kor-travel-map ADR-026"
    표기 (SPEC 저장소)
...
- [x] T-102 — **pg_prewarm 부팅 후 warm-up** (2026-06-09 — mechanism 구현)
  - migration `0022_pg_prewarm_extension`: `CREATE EXTENSION pg_prewarm SCHEMA x_extension`
    (ADR-008). 명시적 warm-up 헬퍼 `kortravelmap.infra.prewarm.prewarm_relations`
...
- [ ] T-103 — **streaming ETL (Kafka/Redpanda) 대응** — 본 라이브러리는
  ...
  - streaming consumer가 실제로 필요해지면 kor-travel-map 독립 프로그램 영역
    (`packages/kor-travel-map-dagster` 또는 별도 worker)이 소유한다. TripMate
    `apps/etl`이 kor-travel-map provider 적재 consumer를 소유하지 않는다(ADR-045/046).
...
## Sprint 5 운영 진입 직전 (kor-travel-geo 패턴 미러)

- [x] T-200 — **Batch DAG + 정합성 게이트** (kor-travel-geo ADR-017 미러)
  - **ADR-045**: Dagster는 **kor-travel-map 소유**(TripMate 아님). asset 작성은
    kor-travel-map, TripMate는 OpenAPI queue 제어만. 상세 T-208(`adr045-standalone-plan.md`).
...
- [x] T-201 — **`ops.feature_consistency_reports` 도입** (T-201a Phase 1 ✅ 2026-05-29: alembic 0003 + `infra/consistency.py` F1~F3 관측; T-201b Phase 2 ✅ 2026-06-06: F4~F8 + Dagster 게이트 + dry-run report CLI)
  ...
  - 케이스 F1~F8 (`kor-travel-map-spec.docx` B.18 참고)
  ...
  - T-201b 진행: F4 dedup backlog WARN은 Sprint 4b에서 추가됨. 2026-06-05 Codex는
    ...
    비교로 추가했다. 이번 PR은 `ktmctl consistency-report` dry-run CLI와
...
- [x] T-210e — `openapi-typescript` client 생성. **완료(2026-06-11)** —
      `packages/kor-travel-map-user-client/` 신설(상단 인덱스 항목 참조). T-212e 후
...
- [ ] T-210c — TripMate `apps/etl` 레거시 Dagster 문서/스켈레톤을 kor-travel-map-owned Dagster
...
- [x] T-212b — admin UI 완결성 보강.
      ...
      - [x] Windows 호스트 Playwright 확인(2026-06-07 Codex):
        `E2E_BASE_URL=http://127.0.0.1:9014 npm -w packages/kor-travel-map-admin/frontend run e2e -- e2e/admin-ops.spec.ts --reporter=line`
        9 passed.
...
- **accepted (text on main)**: ADR-001 ~ ADR-046 (전부). PR#16에서 027~034,
  ...
  조회 + 정합성 책임) PR#54. **ADR-045** (kor-travel-map Docker 독립 프로그램 + 독립
...
- **001~049 accepted**. **다음 후보 번호 = ADR-050**:
  ...
  - 후속 `@kor-travel-map/map-marker-react` npm 게시 자동화 ADR (현재 ADR-043 보류)
  - (필요 시) ADR — Sprint 3 SHP/GeoJSON parsing 위치 결정 (`kortravelmap.
