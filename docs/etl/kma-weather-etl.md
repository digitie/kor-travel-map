...
격자가 많으면 (전국 ~5000) 호출 한도 고려. **한도는 2026-09-13에 실측했다** —
data.go.kr 마이페이지 기준 `기상청_단기예보 조회서비스`의 네 오퍼레이션이 **각각**
10,000/일이고 서로 공유하지 않는다. 종전 이 자리의 "보통 일 ~10,000 호출"은 근거
없는 어림이었고, 하필 값이 맞았지만 "서비스당"인지 "오퍼레이션당"인지를 말하지
않아 3배 오해를 살 수 있었다. 분모·분자·G 실측은 `docs/etl/upstream-quota.md`가
정본이다.
