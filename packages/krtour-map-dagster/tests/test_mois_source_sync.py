...
from krtour.map_dagster.mois_source_sync import (
    MoisSourceSyncSummary,
    mois_localdata_source_sync_op,
    sync_mois_source_db,
)
from krtour.map_dagster.provider_fetchers import ProviderCredentialMissing
...
def test_sync_passes_custom_slugs_org_and_batch(
    ...
    calls = _install_fake_mois(monkeypatch)
    settings = KrtourMapSettings(mois_source_db_path=str(tmp_path / "src.sqlite"))
...
def test_sync_creates_parent_directory(
    ...
    settings = KrtourMapSettings(mois_source_db_path=str(nested))
...
def test_op_runs_sync_and_emits_metadata(
    ...
    monkeypatch.setenv("KRTOUR_MAP_MOIS_SOURCE_DB_PATH", str(db_file))
