...
- [ ] T-VN-M05-ACTIVATION — `a3f6a8f3…`·`22563762…`에 이어 Map `bbb29d17…`·PinVi `663e21b4…`·pinset `c700bd2e…`도 registry/public-copy gate 뒤 n150 isolated M04/M05 launcher를 정확히 한 번 실행했고 `map_health_http_failed` terminal로 차단됐다. cleanup은 통과했으며 raw HTTP·컨테이너·환경 출력은 읽지 않는다. 세 pinset·각 Manager source·output leaf는 어떤 이유로도 재실행하지 않는다. 다음 실행은 이 terminal 기록을 포함한 **새** Map revision, 새 PinVi provenance revision, 새 Manager source를 atomic `pin rotate-pair`로 함께 결박한 fresh pinset에서만 허용된다. terminal current의 role별 회전은 거부되고 source pair preflight도 ledger claim보다 먼저 실행된다. 최신 CI와 전문 적대 리뷰 두 건 뒤에만 n150 isolated M04/M05 live mutating E2E와 activation attestation을 정확히 한 번 실행한다.
