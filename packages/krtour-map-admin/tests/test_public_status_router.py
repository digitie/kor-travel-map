...
def test_public_health_liveness(client: TestClient) -> None:
    ...
    assert body["data"]["service"] == "krtour-map"
...
def test_public_version(client: TestClient) -> None:
    ...
    assert data["version"]
    assert isinstance(data["krtour_map_version"], str)
    assert data["krtour_map_version"]
...
def test_public_version_commit_from_env(
    ...
    monkeypatch.setenv("KRTOUR_MAP_GIT_COMMIT", "abc1234")
    r = client.get("/version")
