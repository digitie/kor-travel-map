...
> `v1` 브랜치에 보존되어 있다. krtour-map은 TripMate와 별개로 **Docker 독립
...
> WSL에서 `/mnt/f/dev/python-krtour-map-<agent>` 경로로 실행합니다.
...
WSL 실행 경로:   /mnt/f/dev/python-krtour-map-codex/     ← Git 외 작업 기본 위치
WSL ext4 mirror: ~/dev/python-krtour-map/                ← 성능·격리 필요 시 선택
