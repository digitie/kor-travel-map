...
이 저장소(GitHub 이름 `python-krtour-map`, Python 패키지 `krtour_map`)는
**TripMate의 지도 데이터 정규화·저장 하부 라이브러리**다. 한국 공공 API
(`python-*-api`)의 결과를 단일 `Feature` 계약으로 정규화하고 PostgreSQL + PostGIS에
저장한다.

TripMate ↔ `python-krtour-map`은 **함수 직접 호출**로 연결된다. 라이브러리가 자체
제공하는 디버그용 FastAPI 라우터는 인증 없이 내부망에서만 사용한다.
...
| 디버그 UI 패키지 | `krtour-map-debug-ui` (옵션) |
...
13. **작업 큐 상태를 in-memory만 신뢰 금지** — 적재 작업은 `import_jobs` 테이블
    ...
    워커 안전.
14. **디버그 API에 인증 추가 금지** — 내부망 전제. 외부 노출이 필요해지면 네트워크
    계층(SSO 게이트웨이 / IP allowlist / Cloudflare Tunnel)에서 보호.
15. **데이터/원천 파일을 git에 커밋 금지** — `data/`는 `.gitignore`. NTFS 보관.
16. **시간 직접 사용 금지** — 모든 datetime은 KST aware (Asia/Seoul). naive
    datetime을 DTO에 넣지 않는다. `kst_now()` 사용.
17. **`Feature.detail`을 자유 dict로 사용 금지** — 항상 `PlaceDetail`/`EventDetail`
    등 Pydantic 모델 인스턴스 → `.model_dump()`. 자유 dict 우회 path 금지.
18. **TripMate 도메인 모델을 본 라이브러리에 정의 금지** — 사용자/여행계획/POI는
    TripMate.
